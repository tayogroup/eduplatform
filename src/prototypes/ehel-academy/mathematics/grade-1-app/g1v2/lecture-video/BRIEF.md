# Brief: the Grade 1 Mathematics unit lecture films

7 films, one per lesson, written at the same time by 7 agents,
...
| file | what it is |
| --- | --- |
| `src/prototypes/ehel-academy/mathematics/grade-1-app/g1v2/lecture-video/<slug>.json` | the storyboard: the script and its cues |
| `tools/lib/film-scenes/math-g1/<slug>.js` | your pictures (more parts if needed: `<slug>-2.js`, `<slug>-3.js`) |

`<slug>` is your lesson's slug, for example `counting-to-twenty`. Everything else is
...
**`src/prototypes/ehel-academy/mathematics/grade-1-app/g1v2/add-lesson-opener.py`** — your lesson's entry has
...
**That is your starting point, and it is thin on purpose: about 350
characters.** A film of 1,700 to 2,000 characters cannot come out of it alone, so
read the lesson page itself — `src/prototypes/ehel-academy/mathematics/grade-1-app/g1v2/<slug>.html` — for the examples,
...
a paragraph: 16 words a sentence here, up to 26. A film line is
...
```json
{
  "_comment": "Storyboard for the Grade 1 Mathematics unit lecture film, <Title>. Rendered by tools/create-ehel-unit-lecture.js. See lecture-video/BRIEF.md.",
  "slug": "<slug>",
  "lesson": N,
  "grade": 1,
  "stage": 1,
  "title": "<the lesson's title, exactly as app.config.json has it>",
  "subtitle": "Grade 1 Mathematics",
  "framework": "Cambridge Primary Mathematics 0096",
  "renderer": {
    "art": ["math"],
    "skin": "brown",
    "scenes": ["tools/lib/ehel-film-marks.js", "tools/lib/film-scenes/math-g1/<slug>.js"]
  },
  "objectives": [["1Gg.01", "<its official wording>"]],
  "scenes": [
    { "id": "title", "kind": "title", "codes": [], "beats": [ ... two beats ... ] },
    { "id": "<chapter>", "kind": "<chapter>", "heading": "<a few words>", "codes": ["1Gg.01"], "beats": [ ... ] },
...
- **`objectives`** are the Cambridge Primary Mathematics 0096 codes the film
  ...
  ```bash
  node -e "const f=require('./src/curriculum/cambridge-mathematics-0096.json');const all=[].concat(...Object.values(f.objectivesByStage));for(const c of process.argv.slice(1)){const o=all.find(o=>o.code===c);console.log(c,o?o.text:'NOT FOUND')}" 1Gg.01
  ```
...
**1,700 to 2,000 characters of narration**, in **26 to 34 beats**: about 2:20 to 2:40 of film.
...
### Language: a Grade 1 child (about six), and the lesson's own words

- Keep sentences at **8 words or under on average, and none over
  14**.
...
```bash
T="node tools/create-ehel-unit-lecture.js --app src/prototypes/ehel-academy/mathematics/grade-1-app/g1v2 --slug <slug>"
...
```bash
node tools/check-ehel-film-cues.js mathematics 1
```
