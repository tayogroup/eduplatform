  ...
  /* ---- a physical computing device (3P.06, 3P.10, 3CS.05) ---------------
     Bitsy: a small board with a 5-by-5 light grid, a speaker, a motor and
     a bell, and three INPUTS on it - button A, a shake, a clap. A program
     starts with a "when ..." hat block naming its input, then output
     blocks. The child builds it, then presses the input on the board:
     only the input the hat names makes the outputs happen. */
  const DEVICE_BLOCKS = {
  ...
  const devBlock = (id, attr) => { const b = DEVICE_BLOCKS[id]; return '<button type="button" class="block ' + (b.cat === "hat" ? "control" : "look") + '" ' + (attr || "") + '><span class="bicon" aria-hidden="true">' + b.icon + "</span>" + esc(b.label) + "</button>"; };
  function deviceProgram(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, script = [], running = false, firstRun = true, score = 0, leds = LED.dark, motorOn = false, bellOn = false;
    const round = () => rounds[r];
    ...
    function board() {
      return '<div class="board"><div class="ledgrid" role="img" aria-label="Bitsy\'s lights">' + leds.map((row) => row.split("").map((c) => '<i class="led' + (c === "1" ? " on" : "") + '"></i>').join("")).join("") + "</div>" +
        ...
        '<div class="inputs"><button type="button" class="big small" data-in="A"' + (running ? " disabled" : "") + '>\u{1F170}️ Press A</button><button type="button" class="big small" data-in="shake"' + (running ? " disabled" : "") + '>\u{1FAE8} Shake it</button><button type="button" class="big small" data-in="clap"' + (running ? " disabled" : "") + '>\u{1F44F} Clap</button></div></div>';
    }
    function draw(now) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The algorithm: <b>' + esc(rd.algorithm.join(", ")) + "</b></p>" + board() +
        '<div class="script">' + (script.length ? script.map((id, k) => devBlock(id, 'data-k="' + k + '"' + (running ? " disabled" : ""), k === now)).join("") : '<span class="hint">Start with a "when" block, then the outputs</span>') + "</div>" +
    ...
    function start() {
      script = []; running = false; firstRun = true; leds = LED.dark; motorOn = false; bellOn = false;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      draw(-1);
    ...
    function trigger(input) {
      ...
      running = true; leds = LED.dark; motorOn = false; bellOn = false; draw(0);
      let i = 1;
      const step = () => {
        if (i >= script.length) return finished();
        const id = script[i];
        if (LED[id]) leds = LED[id];
        if (id === "beep") SOUND.play("beep", 0.5);
        if (id === "motor") { motorOn = true; SOUND.play("robot", 0.4); }
        if (id === "bell") { bellOn = true; SOUND.play("bell", 0.5); }
        draw(i); i++;
        setTimeout(step, 800);
      };
    ...
    function finished() {
      ...
      if (same) {
        if (firstRun) score++;
        SOUND.play("tada", 0.5);
        const line = cheer() + " " + DEVICE_BLOCKS[script[0]].label.replace(/^when /, "When ") + ", Bitsy did: " + script.slice(1).map((id) => DEVICE_BLOCKS[id].label).join(", ") + ". An input made an output.";
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        $(el.stage).querySelectorAll("button").forEach((b) => { b.disabled = true; });
        r++;
        setTimeout(() => {
        ...
        firstRun = false; SOUND.play("error", 0.3);
        const line = "Bitsy did " + script.slice(1).map((id) => DEVICE_BLOCKS[id].label).join(", ") + ". The algorithm says " + rd.algorithm.join(", ").toLowerCase() + ". Change the blocks and try the input again.";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    }
    $(el.stage).addEventListener("click", (e) => {
  ...
  /* ---- a simple cipher: 1 = a, 2 = b (3DC.04, 3DC.05) -------------------
     The alphabet strip with its numbers stays on screen. A decode round
     shows numbers and the child taps the letters; an encode round shows a
     word and the child taps the numbers. codeWord / decodeCode decide,
     mirrored in _rules.py. */
  function cipher(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, got = [], right = 0, wrongHere = false, lock = false;
    const strip = () => '<div class="alpha" aria-label="The code: 1 is a, 2 is b, and so on">' + ALPHA.map((c, k) => '<span class="al"><b>' + c + "</b><small>" + (k + 1) + "</small></span>").join("") + "</div>";
    function draw() {
      const rd = rounds[r], decode = rd.kind === "decode";
      const target = decode ? rd.code : codeWord(rd.word);
      const shown = decode ? rd.code.map((n, k) => '<span class="codecell' + (k < got.length ? " done" : k === got.length ? " now" : "") + '">' + n + "<small>" + (k < got.length ? got[k] : "?") + "</small></span>").join("")
        : rd.word.split("").map((c, k) => '<span class="codecell' + (k < got.length ? " done" : k === got.length ? " now" : "") + '">' + c + "<small>" + (k < got.length ? got[k] : "?") + "</small></span>").join("");
      $(el.stage).innerHTML = '<div class="stagewide">' + strip() + '<p class="goal">' + (decode ? "Decode this message:" : "Write <b>" + esc(rd.word) + "</b> in code:") + '</p><div class="coderow">' + shown + "</div>" +
        '<div class="keys">' + (decode ? ALPHA.map((c) => '<button type="button" class="key" data-key="' + c + '"' + (lock ? " disabled" : "") + ">" + c + "</button>").join("")
          : Array.from({ length: 26 }, (_, k) => '<button type="button" class="key num" data-key="' + (k + 1) + '"' + (lock ? " disabled" : "") + ">" + (k + 1) + "</button>").join("")) + "</div></div>";
      $(el.score).textContent = "Message " + (r + 1) + " of " + rounds.length + " · " + got.length + " of " + target.length;
    }
    function start() {
      got = []; wrongHere = false; lock = false; draw();
      const rd = rounds[r];
      $(el.ask).innerHTML = rd.kind === "decode" ? "Each number is a letter: 1 is a, 2 is b. Tap the letters to decode <b>" + esc(rd.code.join(" ")) + "</b>." : "Each letter is a number: a is 1, b is 2. Tap the numbers to write <b>" + esc(rd.word) + "</b> in code.";
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      sayHere(o.finish, plain($(el.ask).innerHTML));
    }
    $(el.stage).addEventListener("click", (e) => {
      const k = e.target.closest(".key"); if (!k || lock) return;
      const rd = rounds[r], decode = rd.kind === "decode";
      const target = decode ? decodeCode(rd.code).split("") : codeWord(rd.word).map(String);
      ...
      if (v === target[got.length]) {
        ...
        if (got.length >= target.length) {
          ...
          const line = decode ? cheer() + " " + rd.code.join(" ") + " says " + decodeCode(rd.code).toUpperCase() + "." + (rd.why ? " " + rd.why : "") : cheer() + " " + rd.word + " in code is " + codeWord(rd.word).join(" ") + "." + (rd.why ? " " + rd.why : "");
          $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
          r++;
          setTimeout(() => {
        ...
        const line = decode ? "Look at the strip: " + rd.code[got.length] + " is the letter " + target[got.length] + "." : "Look at the strip: " + rd.word[got.length] + " is number " + target[got.length] + ".";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
