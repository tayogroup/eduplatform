# Brief: the Grade 3 Mathematics unit lecture films

8 films, one per lesson, written at the same time by 8 agents,
...
| file | what it is |
| --- | --- |
| `src/prototypes/ehel-academy/mathematics/grade-3-app/lecture-video/<slug>.json` | the storyboard: the script and its cues |
| `tools/lib/film-scenes/math-g3/<slug>.js` | your pictures (more parts if needed: `<slug>-2.js`, `<slug>-3.js`) |

`<slug>` is your lesson's slug, for example `up-to-a-thousand`. Everything else is
...
**`src/prototypes/ehel-academy/mathematics/grade-3-app/src/add-lesson-opener.py`** — your lesson's entry has
...
**That is your starting point, and it is thin on purpose: about 435
characters.** A film of 1,950 to 2,300 characters cannot come out of it alone, so
read the lesson page itself — `src/prototypes/ehel-academy/mathematics/grade-3-app/<slug>.html` — for the examples,
...
a paragraph: 20 words a sentence here, up to 35. A film line is
...
```json
{
  "_comment": "Storyboard for the Grade 3 Mathematics unit lecture film, <Title>. Rendered by tools/create-ehel-unit-lecture.js. See lecture-video/BRIEF.md.",
  "slug": "<slug>",
  "lesson": N,
  "grade": 3,
  "stage": 3,
  "title": "<the lesson's title, exactly as app.config.json has it>",
  "subtitle": "Grade 3 Mathematics",
  "framework": "Cambridge Primary Mathematics 0096",
  "renderer": {
    "art": ["math"],
    "skin": "brown",
    "scenes": ["tools/lib/ehel-film-marks.js", "tools/lib/film-scenes/math-g3/<slug>.js"]
  },
  "objectives": [["3Gg.01", "<its official wording>"]],
  "scenes": [
    { "id": "title", "kind": "title", "codes": [], "beats": [ ... two beats ... ] },
    { "id": "<chapter>", "kind": "<chapter>", "heading": "<a few words>", "codes": ["3Gg.01"], "beats": [ ... ] },
...
- **`objectives`** are the Cambridge Primary Mathematics 0096 codes the film
  ...
  ```bash
  node -e "const f=require('./src/curriculum/cambridge-mathematics-0096.json');const all=[].concat(...Object.values(f.objectivesByStage));for(const c of process.argv.slice(1)){const o=all.find(o=>o.code===c);console.log(c,o?o.text:'NOT FOUND')}" 3Gg.01
  ```
...
**1,950 to 2,300 characters of narration**, in **28 to 36 beats**: about 2:35 to 3:00 of film.
...
- Keep sentences at **10 words or under on average, and none over
  18**.
...
```bash
T="node tools/create-ehel-unit-lecture.js --app src/prototypes/ehel-academy/mathematics/grade-3-app --slug <slug>"
...
```bash
node tools/check-ehel-film-cues.js mathematics 3
```
