...
THE BOOKS VS APP COMPARISON NAMED THIS GAP: Stage 3 is where Cambridge's own
apparatus shifts hard - arrays and the 100-grid already established at Stage 2,
base-10 blocks and bar models arriving for real, a fraction wall new to this
stage - and Grade 1 and Grade 2's standalone builds already say which of it to
have to hand before a child starts. Grade 3 never did. This is the same reframe
those two shipped, not a new feature: a screen cannot be a fraction wall, so the
honest thing it can do is tell the adult which real one to fetch.

WHY IT PATCHES THE BUILT PAGES, not a fragment under src/. Grade 3's own README
is explicit that these lessons are GENERATED - src/build-all.sh reproduces the
eight committed .html files byte-identically from src/l*-content.js - and every
tool that has added teaching here since (add-spot-the-mistake.py,
add-twm-stamps.py, add-differentiation.py, add-self-check.py) patches the BUILT
pages directly, because none of that content lives in the fragments either. A
rebuild from src/ would discard all of it, this note included. Same trap the
root CLAUDE.md documents for the shell course's repair-ehel-math-* tools,
inherited by this build's own architecture.

ONE SOURCE. The text is read from app.config.json :: lessons[].materials, the
same field build-grownup-section.py already renders in the hub's
"What to have to hand" panel - unlike Grade 2 when its own version of this tool
was written, Grade 3 already has that hub section, so this is the second of two
readers rather than the first.

WHY FOLDED, AND WHY STEP 1 ONLY: identical reasoning to Grade 1 and Grade 2.
The sentence is addressed to the adult, so a child's first step should not open
with forty words they are not meant to read; nearly every step here has some
physical equivalent, and a note on each would be noise a reader learns to skip.
