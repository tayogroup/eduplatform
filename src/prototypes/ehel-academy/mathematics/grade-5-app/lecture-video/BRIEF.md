# Brief: the Grade 5 Mathematics unit lecture films

7 films, one per lesson (every lesson but Shape and Measures, which has its own film already), written at the same time by 7 agents,
...
## Before anything else: GRADE 5 IS NOT A DECK, and that changes where your film goes

Grades 1-4 are a slide deck: 28 to 40 slides walked through one at a time, each
ending in a `finish()` that ticks a step, and the Unit lecture is a narrated
click-through that the film REPLACES.

**Grade 5 is a page a learner scans.** A lesson is five `<section class="step">`
blocks. No slides, no deck, no `finish()`. That is CLAUDE.md's hard rule - the
deck stops at Stage 4 because by Grade 5 a learner scans a page rather than
being walked through it - and this build kept the page on purpose.

So:

- **Your film sits ABOVE the three static lecture parts, and they stay.**
  It does not replace them. Stripping the scannable text to make room for a
  video would undo the thing that makes this build Grade 5.
- **There is no `finish()` to tick when the film ends.** Progress here is
  "section reached", by an IntersectionObserver over each `<section class="step">`
  (see `app.config.json`'s comment and `check-platform-wiring.py`). The film
  needs no completion hook and must not invent one.
- **This grade has 5 lessons with a Unit lecture**, so 5 films.
  `check-what-you-know` is an assessment and has no lecture step - it gets no
  film.

**WHAT YOUR FILM MUST TEACH** is that lesson's WORK entry in
`add-lesson-opener.py` - three named parts, each a title and a paragraph - plus
the lesson page itself where you need the worked detail. The three parts are the
spine: one chapter each is the natural shape, with a title and a recap around
them.

**Each lesson has THREE DISTINCT parts and they do not overlap.** An earlier
draft of this brief claimed the third part of one lesson reopened as the first
part of the next. That was wrong - it came from a regex that ran past a WORK
entry's boundary into the following entry, so the next lesson's first part
looked like a fourth part of the current one. The five lessons' parts are:
squares and roots / cubes and cube roots / triangular numbers; sequences and
rules / linear sequences / patterns in shapes; odd, even and order of operations
/ factors and multiples / divisibility and primes; place value with decimals /
negative numbers and rounding / fractions, percentages and ratio; choosing a
method / estimating first / reading the story. Teach your own three.

## Length, measured for THIS subject

**2,050 to 2,400 characters of narration in 28 to 36 beats.** Those two numbers
are the rule, and they are this SUBJECT's - an earlier draft of this section
carried Computing's band (1,950 to 2,300) by mistake.

Measured over all 32 finished Mathematics films on 2026-09-25, this subject's
voice runs **12.47 characters a second** (spread 10.81 to 13.26). So the band
above means about **2:44 to 3:12**, and at the slow end with a technical film it
can reach 3:42. The older "about 2:45 to 3:05" below was derived from the
estimator rather than measured, and its top end is 7 seconds short. A film inside the character and beat bands is the right length
whatever the clock says. **Do not trim a film to hit a time** - `--dry`'s
estimate assumes 13.96 a second and runs 10 to 12% short of the real thing here.


## What you are making

A film of **about 3 minutes** that plays in your lesson's **Unit lecture** step, in
...
**The Maths film that exists already** is Grade 5's Shape and Measures
(`mathematics/grade-5-app/lecture-video/shape-and-measures.json`, drawn by
...
| file | what it is |
| --- | --- |
| `src/prototypes/ehel-academy/mathematics/grade-5-app/lecture-video/<slug>.json` | the storyboard: the script and its cues |
| `tools/lib/film-scenes/math-g5/<slug>.js` | your pictures (more parts if needed: `<slug>-2.js`, `<slug>-3.js`) |

`<slug>` is your lesson's slug, for example `big-numbers-below-zero`. Everything else is
...
**`src/prototypes/ehel-academy/mathematics/grade-5-app/add-lesson-opener.py`** — your lesson's entry has
...
**That is your starting point, and it is thin on purpose: about 385
characters.** A film of 2,050 to 2,400 characters cannot come out of it alone, so
read the lesson page itself — `src/prototypes/ehel-academy/mathematics/grade-5-app/<slug>.html` — for the examples,
...
a paragraph: 13 words a sentence here, up to 33. A film line is
...
```json
{
  "_comment": "Storyboard for the Grade 5 Mathematics unit lecture film, <Title>. Rendered by tools/create-ehel-unit-lecture.js. See lecture-video/BRIEF.md.",
  "slug": "<slug>",
  "lesson": N,
  "grade": 5,
  "stage": 5,
  ...
  "renderer": {
    "art": ["math"],
    "skin": "brown",
    "scenes": ["tools/lib/ehel-film-marks.js", "tools/lib/film-scenes/math-g5/<slug>.js"]
  },
  "objectives": [["4Gg.01", "<its official wording>"]],
  "scenes": [
    { "id": "title", "kind": "title", "codes": [], "beats": [ ... two beats ... ] },
    { "id": "<chapter>", "kind": "<chapter>", "heading": "<a few words>", "codes": ["4Gg.01"], "beats": [ ... ] },
...
- **`objectives`** are the Cambridge Primary Mathematics 0096 codes the film
  ...
  ```bash
  node -e "const f=require('./src/curriculum/cambridge-mathematics-0096.json');const all=[].concat(...Object.values(f.objectivesByStage));for(const c of process.argv.slice(1)){const o=all.find(o=>o.code===c);console.log(c,o?o.text:'NOT FOUND')}" 4Gg.01
  ```
...
**2,050 to 2,400 characters of narration**, in **28 to 36 beats**: about 2:45 to 3:05 of film.
...
### Language: a Grade 5 child (about nine), and the lesson's own words

- Keep sentences at **11 words or under on average, and none over
  20**.
...
```bash
T="node tools/create-ehel-unit-lecture.js --app src/prototypes/ehel-academy/mathematics/grade-5-app --slug <slug>"
...
looking at your sample sheets does not catch this. It was found in a Grade 5
...
```bash
node tools/check-ehel-film-cues.js mathematics 5
```
