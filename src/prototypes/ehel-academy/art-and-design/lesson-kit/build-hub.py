...
TRADITIONS = {
    ...
    "blockprint": "The printed cloth is drawn in the manner of the hand block printing of Rajasthan, in north-west India, where printers press blocks of teak or sheesham wood, carved by hand, onto cloth again and again - one block and one colour at a time, so a two-colour cloth is printed twice and the second block has to line up with the first. Whole families of printers work at it, and towns such as Bagru and Sanganer are known for their own colours and patterns. The flower on the page is our own, not a copy of any printer's block.",
    "amphora": "The jar is drawn in the manner of ancient Greek black-figure pottery, made around 2,500 years ago, where the potter threw or coiled the vessel and a painter put the figures on in a slip that fired black, then scratched the details through it. Jars like this held oil, water or wine, and many carry scenes from stories that the people who used them knew well. The shape and the figures on the page are our own drawing in that manner, not a copy of a real jar.",
    "bronzehead": "The cast head is drawn in the manner of the brass heads made in Benin City, in what is now Nigeria, by the guild of casters who worked for the Oba's court. They were cast by the lost-wax method, which means the first model was made in wax, covered in clay, then melted out so the metal could take its place, and each head is a portrait of a ruler kept in the palace. Many of these works, the Benin Bronzes, were taken by a British force in 1897 and are now in museums far from Benin City, and some have been returned; whose they are is still being decided. The head on the page is our own drawing in that manner and copies no real head.",
    "profile": "The wall painting is drawn in the manner of ancient Egyptian tomb and temple painting, made over three thousand years ago, where a figure is drawn so that every part is shown from its clearest side: the head and legs from the side, the eye and the shoulders from the front. Real paintings carry hieroglyphs, which are writing; the band of signs on the page is our own pattern and says nothing, because writing that means nothing should not be put in a child's mouth as though it did.",
}
...
%(css)s
  ...
  /* :not(.eyebrow) because .hubhead holds TWO paragraphs and this rule is for
     ...
     0,1,0) and draws the 13px teal label at 19px muted - 455px of nowrap
     text in a 335px box, which overflowed the page by 100px at 375 wide. */
  ...
  /* The hub bar could not fit a phone. `flex: 0 0 auto` was right when this was
     written - a Back button, a brand, a progress track and a picker - and then
     add-lesson-search.py injected a 141px search field INTO this group rather
     than drawing its own bar. A no-shrink flex item sizes to max-content
     whatever the viewport does, so the group ran off the right: measured 45px
     at 375x812 on a built hub, 2026-09-17.

     `min-width: 0` is load-bearing beside the flex - a flex item's floor is its
     min-content width, so the flex alone shrinks nothing. And the flex alone is
     not a fix either: it clears the overflow by crushing the search field to
     nothing. So the picker goes below 480px, which is the one control here that
     is pure duplication - THIS PAGE IS the list of lessons. Same change as
     computing/lesson-kit/build-hub.py and the shared add-header-bars.py. */
