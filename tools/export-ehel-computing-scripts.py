"""Export every line of learner-facing Computing narration to one workbook.

Matches the layout of ehel-math-scripts-complete.xlsx and
ehel-science-scripts-complete.xlsx so all three subjects can be reviewed and
voiced the same way:

    one sheet per stage ("Grade 1" … "Grade 7")
    columns: Unit | Category | Item ID | Title / Prompt | Script Text
    rows grouped by unit, then by category in teaching order

Computing carries modules the other subjects do not — the free-tool setup
guides, code listings, the debugging table, the online-safety rules, the AI
tutor prompts and the end-of-unit project. Those follow the same row shape and
sit in the order the learner meets them, so the sheet stays complete without
departing from the model.

Code listings keep their line breaks: the indentation and block order are part
of what is being taught, and a reviewer needs to see the listing as the learner
sees it. Those rows are marked "(code — read for sense, do not voice verbatim)"
so whoever records the narration knows not to read the punctuation aloud.

Usage:
    python tools/export-ehel-computing-scripts.py [--out <path.xlsx>]
"""
...
# openpyxl agrees: it writes `= stores, == compares` (a real Python reference
# card in Stage 7) into the file as <f> stores, == compares</f>, which Excel
# cannot parse and reports as corrupt content on open. Computing is the first
# subject whose scripts quote code, so it is the first to hit this.
FORMULA_LEADS = ("=", "+", "-", "@")


def write_row(ws, values: list) -> None:
    """Append a row, forcing every cell to be stored as text."""
    ws.append(values)
    row = ws.max_row
    for column, value in enumerate(values, 1):
        if isinstance(value, str) and value[:1] in FORMULA_LEADS:
            cell = ws.cell(row=row, column=column)
            # Store as a shared string and mark it quote-prefixed, so Excel
            # shows the text exactly as written and never evaluates it.
            cell.data_type = "s"
...
def listing(value) -> str:
    """Code lines, one per line — the layout is part of the teaching."""
    if not value:
        return ""
    return "\n".join(str(v).rstrip() for v in value)
...
def unit_rows(u: dict) -> list[list[str]]:
    ...
    for i, t in enumerate(u.get("toolkit") or [], 1):
        add("Tool Setup", f"tool-{i}", short(t.get("name")),
            block(("Tool", t.get("name")), ("Where", t.get("url")),
                  ("Steps", steps(t.get("steps"))), ("Note", t.get("note"))))

    for c in u.get("concepts") or []:
        add("Concept", c.get("id"), short(c.get("title")),
            block(("", c.get("explanation")), ("Example", c.get("example")),
                  ("Check yourself", c.get("checkYourself"))))

    for c in u.get("codeExamples") or []:
        add("Code Example", c.get("id"), short(c.get("title")),
            block(("Listing", f"{c.get('title')} ({c.get('language')})"),
                  ("", c.get("intro")),
                  ("Code (code — read for sense, do not voice verbatim)", listing(c.get("lines"))),
    ...
    for i, d in enumerate(u.get("debugging") or [], 1):
        add("Debugging", f"bug-{i}", short(d.get("symptom")),
            block(("Symptom", d.get("symptom")), ("Cause", d.get("cause")), ("Fix", d.get("fix"))))
    ...
    for i, s in enumerate(u.get("esafety") or [], 1):
        add("Online Safety", f"safety-{i}", short(s.get("title")),
            block(("Rule", s.get("title")), ("", s.get("text"))))
    ...
    for i, p in enumerate(u.get("tutorPrompts") or [], 1):
        add("AI Tutor Prompt", f"tutor-{i}", short(p), block(("", p)))

    project = u.get("project") or {}
    if project.get("title"):
        add("Unit Project", "project", short(project.get("title")),
            block(("Project", project.get("title")), ("", project.get("brief")),
                  ("Steps", steps(project.get("steps"))),
                  ("Success criteria", steps(project.get("successCriteria")))))
    ...
    for i, pair in enumerate(ref.get("terms") or [], 1):
        if len(pair) >= 2:
            add("Reference", f"term-{i}", short(pair[0]),
                block(("Computing word", pair[0]), ("Meaning", pair[1])))
...
def capstone_rows(path: Path) -> list[list[str]]:
    ...
    for i, r in enumerate(project.get("rubric") or [], 1):
        add(f"capstone-rubric-{i}", short(r.get("criterion")),
            block(("Criterion", r.get("criterion")), ("Secure", r.get("secure"))))
...
def reviewed_texts(path: Path) -> set[str]:
    """Every Script Text already signed off in a previous reviewed workbook.

    Matched on the text itself, not on row position: a builder fix that drops a
    junk row shifts every index below it, and the point of this column is to
    ...
    for sheet in wb.sheetnames:
        for row in wb[sheet].iter_rows(min_row=2, values_only=True):
            if len(row) > 4 and isinstance(row[4], str) and row[4].strip():
                seen.add(" ".join(row[4].split()))
    return seen
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path,
                        default=ROOT / "outputs" / "ehel-computing-scripts-complete.xlsx")
    # Adds a "Review Status" column marking each row against a previous
    # reviewed workbook, so a re-export after a builder fix does not ask for
    # 11,000 rows to be read again — only the ones whose text actually moved.
    parser.add_argument("--since", type=Path, help="previous reviewed workbook")
    args = parser.parse_args()

    previous = reviewed_texts(args.since) if args.since else None
    headers = HEADERS + (["Review Status"] if previous is not None else [])
