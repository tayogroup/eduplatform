# Stage 8 Computing — course syllabus

Narrative copy for the syllabus authoring form at
`local/hubredirect/syllabus.php`, for the Ehel Academy **Stage 8 Computing**
course (Grade 8; this subject labels its years *Stage*). Every figure is read
from `src/prototypes/ehel-academy/computing/grade-8/data/`.
...
1. **Curriculum review is outstanding.** The package records a self-teaching
   content pass as complete, but all 4 units still read
   `Imported - curriculum review required`.
2. **No lecture video exists.** All 4 units report `Video pending`.
3. **The narration is not in the repository.** 722 clips are needed for
   this stage and 722 are present on this machine — but
...
4. **This stage was built from **student lesson books**, carried across as written.** Stages 5–8 work this way; Stages 1–4 are rewritten from adult guides instead. This stage's prose is Cambridge's own, addressed to the learner from the start.

---

## Overview

Stage 8 Computing is the last computing course Ehel Academy offers. It returns to the four-strand structure and takes the programming to **pseudocode** — designing a program before writing it. It follows **Cambridge Lower Secondary Computing 0868, Stage 8**.

Four units: computational thinking and programming, managing data, networks and digital communication, computer systems.

Across the year: **4 units**, **40 learning outcomes**,
**49 concepts**, **18 code examples**, **32 hands-on activities**,
**64 practice questions**, **48 fluency questions**, **19 debugging
cases**, **15 e-safety notes**, **48 games** and **4 unit projects** — one
...
**debugging section** — a symptom, its cause and its fix, 19 of them across the
year — which teaches that finding the fault is part of the work rather than a
sign of failure. And every unit carries **e-safety notes**, 15 across the year,
...
> {{Teacher name}} supports Stage 8 Computing at {{school name}}. {{One or two
...
Stage 8 assumes Stages 1 to 7. There is no foundation unit inside this course.

Before Unit 1 a learner sits the **Stage 8 Computing Placement Exam**: 36 questions, 36 marks, about 35 minutes. Attempts are unlimited and it is never a fail.

Sections:

- **Stage 7 skills** (22 questions)
- **Stage 6 skills** (8 questions)
- **Foundations (Stages 4–5)** (6 questions)
...
- Materials for the hands-on activities — 32 of them across the year, many
...
- **The unit challenge**, pass mark 80% — 4 across the year, 12 questions each.
- **Games**, mastery score 3.
- **Self-assessment** — 40 *I can…* statements across the year, no marks.
- **4 unit projects**, each with success criteria written into the brief.
...
- **Read the e-safety notes properly.** 15 across the year, inside the
...
**If a learner is struggling**, the placement check can be retaken at any time. Stage 7 Computing remains available for a learner who needs the ground under this one.
...
|---|---|---|---|---|---|---|
| 1 | Computational Thinking and Programming | 10 | 10 | 10 | 8 | 4 |
| 2 | Managing Data | 10 | 14 | 2 | 8 | 3 |
| 3 | Networks and Digital Communication | 10 | 14 | 5 | 8 | 4 |
| 4 | Computer Systems | 10 | 11 | 1 | 8 | 4 |
| | **Total** | **40** | **49** | **18** | **32** | **15** |

Varying between units: concepts (10–14), outcomes (10),
code examples (1–10), activities (8), e-safety notes
(3–4), debugging cases (1–8), practice (16)
...
**Stages 5, 6 and 8 share these four unit titles**, because they are Cambridge's
four strands rather than lesson names. The content behind them differs — Stage 5
teaches variables, Stage 6 flowcharts, Stage 8 pseudocode, each from its own
source documents — so the repetition is a naming convention, not duplication.

Framework: Cambridge Lower Secondary Computing 0868, Stage 8.
Source audience: student.
Narration: 722/722 clips present locally, 0 tracked in git.

Source: `course-manifest.json` and `units/unit-*.json` under
`src/prototypes/ehel-academy/computing/grade-8/data/`.
