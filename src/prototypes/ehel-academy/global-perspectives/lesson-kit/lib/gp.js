  ...
  function pictogramRead(o) {
    ...
    const body = o.display === "table"
      ...
      : o.display === "bars"
        ? '<div class="picto bars"><p class="pictotitle">' + esc(o.title) + "</p>" + o.rows.map((r) =>
          ...
          '<p class="sub">A bar chart: the longer the bar, the more people.</p></div>'
    ...
    function draw() {
      ...
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      const what = o.display === "table" ? "table" : o.display === "bars" ? "bar chart" : "pictogram";
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      setTimeout(() => {
        if (i >= o.items.length) {
          ...
          endStep(o, "You answered " + right + " of " + o.items.length + " from the " + (o.display === "table" ? "table" : o.display === "bars" ? "bar chart" : "pictogram") + ". " + o.done);
        } else draw();
      }, 2700);
    });
    draw();
  ...
  function opinion(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, stance = null, said = 0, lock = false, given = [];
  ...
  function teamBuild(o) {
    ...
    logs[o.finish] = log;
    let r = 0, grown = 0, lock = false, missed = false, score = 0, taskDone = [], taskOrder = [];
    ...
    function next() {
      ...
      setTimeout(() => {
        ...
        else if (rounds[r].kind === "task") { start(); }
        else start();
      }, 3400);
    }
    function friendDone() {
    ...
    $(el.stage).addEventListener("click", (e) => {
      ...
      if (k === taskDone.length) {
        ...
        if (taskDone.length === rd.steps.length) {
          lock = true; if (!missed) score++;
          log.push({ who: "you", text: rd.log || ("You " + lower1(rd.job)), pic: rd.pic || "\u{1F6E0}\ufe0f" });
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      if (rd.kind === "share") {
        ...
        if (opt.good) {
          lock = true; if (!missed) score++;
          $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
          log.push({ who: "you", text: rd.log || opt.log || opt.t, pic: rd.pic || (rd.kind === "idea" ? "\u{1F4A1}" : "\u{1F91D}") });
          grow(cheer() + " " + rd.why);
          next();
        } else {
  ...
  function lookBack(o) {
    ...
    $(el.stage).addEventListener("click", (e) => {
      ...
      if (lc && phase === "learned" && !lock) {
        ...
        if (c.ok) {
          ...
          if (got.size >= pick) {
            lock = true;
            setTimeout(() => { lock = false; phase = "liked"; draw(); $(el.fb).textContent = ""; const q = o.mode === "helped" ? "Now: which part <b>helped you learn</b> most? Tap it." : "Now: which part did you <b>like</b> best? Tap it."; $(el.ask).innerHTML = q; sayHere(o.finish, plain(q)); }, 2600);
          }
        } else {
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
      SOUND.play("tada", 0.4);
      const sentence = (o.mode === "helped" ? "The part that helped me learn most was " : "I liked ") + o.liked[liked].title + ", " + b.dataset.why + ".";
  ...
  function textSource(o) {
    ...
    $(el.stage).addEventListener("click", (e) => {
      ...
      if (k === rd.line) {
        ...
        r++;
        setTimeout(() => {
          if (r >= rounds.length) { reportScore(o.finish, right, rounds.length); endStep(o, "You found the sentence that answers " + rounds.length + " questions. " + o.done); }
          else start();
        }, 3400);
      } else {
        missed = true; b.classList.add("wrong"); SOUND.play("error", 0.3);
