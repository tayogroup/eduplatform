/* Raise a hand, the class chat, and the Join class pill — Professor Adow TVET.
 *
 * A VERBATIM PORT of the Ehel Academy standalone control
 * (ehel-academy/mathematics/grade-3-app/learner-controls.js), changed only in
 * its two import lines. It is copied rather than rewritten because every line
 * of it is a wire contract with local_hubredirect — the poll intervals, the
 * unread dot, the private-message flag, the attachment expiry — and a
 * reimplementation would diverge from the server in ways no local test here
 * could catch.
 *
 * THESE CONTROLS ARE GATED AND WILL NOT APPEAR YET. They mount only when the
 * page was opened from Moodle with a signed launch (?pwsToken and a
 * cross-origin ?pwsEndpoint) AND the server says a teacher is watching. Today
 * Professor Adow TVET has no consumer record, no synced courses and no
 * enrolments, so nothing shows — which is correct, not broken. A button that
 * cannot reach a teacher is worse than no button. They light up on their own
 * the day a learner launches a TVET lesson from Moodle.
 *
 * A SEPARATE COPY IS THE ESTABLISHED PATTERN, not an accident. The upstream
 * header warns that giving a page its own copy "is the one thing that must not
 * happen" — that warning is about TWO copies on ONE page, because both controls
 * are singletons owning polling state and would disagree about whether a hand
 * is up. A different school's page loading one copy is exactly what the Grade 3
 * standalone app does.
 *
 * Keeping it in step with upstream is manual. If the hubredirect contract
 * changes, this file has to be re-copied.
 */
import { platformUrl, platformHeaders } from "./platform.js";
import { escapeHtml as sharedEscapeHtml } from "./platform.js";
