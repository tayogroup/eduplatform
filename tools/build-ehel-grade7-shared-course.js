...
const GRADE_DIR = path.join(ROOT, "src", "prototypes", "ehel-academy", "english", "grade-7");
const DATA_DIR = path.join(GRADE_DIR, "data");
const UNIT_DIR = path.join(DATA_DIR, "units");
const SOURCE_FILE = path.join(ROOT, "inputs", "ehel-grade7-source", "grade7-source-extracted.json");
...
const gradeProfile = templateConfig.gradeProfiles.g07;
if (!gradeProfile || gradeProfile.readingWordsPerText !== "500-1100" || gradeProfile.speakingTarget !== "4-5 minutes") throw new Error("Grade 7 neutral profile is missing or invalid.");
...
const visualMap = {
  1: ["unit-8-home.png", "A Grade 7 learner exploring family identity, migration and oral history"],
  2: ["unit-3-ready-steady-go.png", "Learners comparing celebrations, festivals and community traditions"],
  3: ["unit-4-big-sky.png", "Learners investigating clothing, fashion choices and sustainability"],
  4: ["unit-9-city.png", "Learners evaluating transport, cities and practical urban solutions"],
  5: ["unit-4-big-sky.png", "Learners planning outdoor adventures and environmental protection"],
  6: ["unit-3-ready-steady-go.png", "Learners analysing sport, Olympic achievement and inclusion"],
  7: ["unit-2-neighbours-jobs.png", "Learners discussing success, careers and personal growth"],
  8: ["unit-9-city.png", "Learners verifying media, technology and news sources"],
  9: ["unit-5-measure.png", "Learners debating digital innovation and responsible futures"],
  10: ["capstone-my-english-world.png", "A Grade 7 learner defending a researched English capstone"],
};
...
function unitId(unitNo) {
  return `eng-g07-${termFor(unitNo).id}-u${String(unitNo).padStart(2, "0")}`;
}
...
function doc(unitNo, type) {
  ...
  if (!record) throw new Error(`Missing Grade 7 source: Unit ${unitNo} ${type}`);
  return record;
}
...
function sourcePath(unitNo, type) {
  return `inputs/ehel-grade7-source/Year 7/${doc(unitNo, type).path}`;
}
...
function extractVocabulary(unitNo) {
  ...
  return tables.map((table, groupIndex) => ({
    ...
    words: table.slice(1).filter((row) => {
      ...
      if (!key || seen.has(key) || total >= 50) return false;
      seen.add(key);
      total += 1;
      return true;
...
function extractReadings(unitNo) {
  ...
  const cleanLines = (paragraphs) => paragraphs.filter((line) => !/^(Year 7 English|After Reading|Understanding the Story|Comprehension|Reflection Questions|Activity \d+|Answer Key|Ask Your AI Tutor|Self-)/i.test(line) && !line.endsWith("?") && line.split(/\s+/).length > 5);
  const chunk = (paragraphs, target = 500) => {
    const chunks = [];
    let current = [];
    let words = 0;
  ...
  const lessonChunks = chunk(cleanLines(lesson.paragraphs.slice(2)), 540).slice(0, 2);
  ...
  const storyChunks = chunk(cleanLines(story.paragraphs.slice(storyStart, storyEnd < 0 ? story.paragraphs.length : storyEnd)), 540).slice(0, 3);
  const readings = [
...
function grammarRecord(unitNo, block, sequence) {
  ...
  return {
    ...
    origin: "Authored Grade 7 source",
...
const extensionTasks = {
  speaking: [
    ...
    ["Five-Minute Unit Talk", "Plan and record a five-minute talk using an opening, three connected claims with evidence, a considered viewpoint, a response to an alternative view, and a purposeful closing."],
  ],
  writing: [
...
function buildTasks(unitNo, kind) {
  ...
  return tasks.slice(0, 6).map(([title, body], index) => {
    ...
    const common = { unitId: unitId(unitNo), sequence, title: `${kind[0].toUpperCase() + kind.slice(1)} ${sequence}: ${title}`, outcomeId: `${unitId(unitNo)}-lo${String(Math.min(sequence, 6)).padStart(2, "0")}`, origin: index < sourceTasks.length ? "Authored Grade 7 source" : "Ehel English Content Template v1.2 curriculum expansion", reviewStatus: "Approved v1.2", sourceFile: index < sourceTasks.length ? sourcePath(unitNo, "Lesson") : "Ehel English Content Template v1.2" };
    ...
    if (kind === "writing") return { writingId: id, ...common, promptAndInstructions: body, modelText: `A strong Grade 7 response uses cohesive paragraphs, precise unit words, relevant evidence, source awareness and a purposeful conclusion about ${title.toLowerCase()}.`, sentenceStarter: "I would like to argue or explain...", expectedLength: sequence <= 2 ? "Four organised paragraphs" : gradeProfile.writingExpectation, successCriteria: "I addressed the prompt and audience; organised ideas into cohesive paragraphs; used precise Unit vocabulary; supported claims with relevant evidence; considered viewpoint and source reliability; checked cohesion and punctuation; revised my work", support: "Plan claims, topic sentences and supporting evidence, then use the word bank and source models.", extension: "Synthesize details from more than one text, acknowledge an alternative viewpoint, vary sentence structures and strengthen the conclusion.", rubricId: "rub-writing-v1" };
...
function extractOutcomeText(lesson, title) {
  ...
  const extensions = [
    ...
    `Use Grade 7 vocabulary accurately in discussion and writing.`,
    ...
    `Present a clear four- to five-minute response and defend ideas in follow-up questions.`,
  ];
...
function buildRegularUnit(unitNo, dictionaryEntries) {
  ...
  groups.forEach((group, groupIndex) => {
    ...
    group.words.forEach((word, wordIndex) => {
      ...
      const practiceSentences = [word.example, `The word ${word.word} belongs to our ${group.title.toLowerCase()} vocabulary.`, `I can explain ${word.word} in my own words.`, `I can connect ${word.word} to this unit's texts.`, `I can use ${word.word} in a clear Grade 7 sentence.`];
      dictionaryLinks.push({ vocabularyId, dictionaryEntryId, senseId: `${dictionaryEntryId}-sense-01`, gradeId: "g07", termId: term.id, unitId: id, groupId, groupTitle: group.title, sequence: wordIndex + 1, childMeaning: word.meaning, exampleSentence: word.example, practiceSentences, sentenceAudio: practiceSentences.map((_, sentenceIndex) => pendingAudio(`./unit-${unitNo}/media/audio/vocabulary/${vocabularyId}-sentence-${sentenceIndex + 1}.mp3`)), sentenceStarter: starterFor(word), spellingPractice: word.word.split("").join(" - "), aiTutorPrompt: `Ask me to define '${word.word}', use it in a sentence, and connect it to ${title}.`, reviewStatus: "Approved v1.2" });
...
function buildCapstone(regularUnits, dictionaryEntries) {
  ...
  const id = unitId(unitNo);
  const term = termFor(unitNo);
  const selectedLinks = regularUnits.flatMap((unit) => unit.dictionaryLinks.slice(0, 4)).slice(0, 30).map((link, index) => ({ ...link, vocabularyId: `u10-g1-${index + 1}-${slug(dictionaryEntries.get(link.dictionaryEntryId).displayWord)}`, unitId: id, termId: term.id, groupId: "u10-g1-grade-7-review-words", groupTitle: "Grade 7 Review Words", sequence: index + 1 }));
  const title = "My English Voice: Grade 7 Capstone";
  const outcomes = ["Select and synthesize strong evidence from Units 1-9 to explain progress.", "Use Grade 7 vocabulary accurately across speaking and writing.", "Analyse and compare passages through language, structure, viewpoint and source reliability.", "Create an evidence-led inquiry product for a defined audience using responsible source attribution.", "Present and defend conclusions clearly for five minutes, responding thoughtfully to challenges.", "Evaluate progress honestly and set a specific Grade 8 English goal."].map((learningOutcome, index) => ({ outcomeId: `${id}-lo0${index + 1}`, unitId: id, sequence: index + 1, learningOutcome, bloomLevel: index < 2 ? "Apply" : index < 4 ? "Analyse and create" : "Evaluate", evidenceOfLearning: "Capstone portfolio, inquiry product, source record, oral defence and reflection", origin: "Ehel Grade 7 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  const makeSix = (kind, items) => items.map((item, index) => {
    ...
    const common = { unitId: id, sequence: index + 1, title: item[0], outcomeId: outcomes[index].outcomeId, origin: "Ehel Grade 7 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" };
  ...
  while (comprehension.length < 12) comprehension.push({ ...comprehension[comprehension.length % 10], questionId: `${id}-cq${String(comprehension.length + 1).padStart(3, "0")}`, sequence: comprehension.length + 1, question: comprehension.length === 10 ? "Compare two review texts. What important idea do they share?" : "Which Grade 7 text influenced your capstone most, and why?" });
  const quizzes = regularUnits.flatMap((unit) => unit.quizzes.slice(0, 2)).slice(0, 10).map((quiz, index) => ({ ...quiz, quizId: `${id}-quiz${String(index + 1).padStart(2, "0")}`, questionId: `${id}-quiz${String(index + 1).padStart(2, "0")}-q01`, unitId: id, quizTitle: "Grade 7 capstone review checkpoint", sequence: index + 1 }));
  const liveSessions = ["Launch and portfolio curation", "Reading and vocabulary conference", "Project planning and first draft", "Grammar editing and peer feedback", "Presentation rehearsal", "Capstone exhibition and reflection"].map((title, index) => ({ liveSessionId: `${id}-live0${index + 1}`, unitId: id, week: index < 3 ? 1 : 2, sessionWithinWeek: (index % 3) + 1, sessionNo: index + 1, title, durationMin: 45, beforeSession: "Complete the preceding capstone milestone and bring evidence.", agenda: "5 min check-in; 10 min model; 15 min workshop; 10 min conference or rehearsal; 5 min next step", afterSession: index === 5 ? "Complete the final course quiz." : "Apply feedback and upload the improved milestone.", outcomeIds: outcomes[index].outcomeId, origin: "Ehel Grade 7 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  return { schemaVersion: "Ehel English Runtime v1.2", templateVersion: "Ehel English Content Template v1.2", dictionaryVersion: "Ehel Master Dictionary v1.2", generatedAt: GENERATED_AT, grade: { id: "g07", label: "Grade 7" }, subject: "English", term, unit: { gradeId: "g07", subject: "English", termId: term.id, unitId: id, unitNo, unitTitle: title, unitOverview: "Bring together your strongest vocabulary, reading, grammar, speaking and writing from Units 1-9. Create an evidence-led inquiry product for a defined audience, attribute and evaluate sources, defend conclusions and reflect on your Grade 7 growth.", learningPath: "Curate and annotate evidence. Form an inquiry question. Compare and verify sources. Plan and draft cohesive sections. Revise from feedback. Present and defend. Reflect toward Grade 8. Complete the final course quiz.", origin: "Ehel Grade 7 capstone specification", reviewStatus: "Approved v1.2 capstone", sourceFile: "Ehel English Content Template v1.2" }, visual: { image: "../grade-2/capstone-my-english-world.png", alt: visualMap[10][1], lectureMode: "capstone-launch" }, vocabularyGroups: [{ id: "u10-g1-grade-7-review-words", number: 1, title: "Grade 7 Review Words", vocabularyIds: selectedLinks.map((link) => link.vocabularyId) }], dictionaryLinks: selectedLinks, rubrics: baseRubrics.map((rubric) => ({ ...rubric, origin: "Ehel Grade 7 approved rubric v1.2" })), outcomes, readings, comprehension, grammar, speaking, writing, activities, assignments: [{ assignmentId: `${id}-assignment01`, unitId: id, title: "Grade 7 inquiry capstone and oral defence", instructions: "Submit the evidence-led inquiry product, attributed source record, four annotated portfolio pieces, five-minute oral defence recording and Grade 8 transition reflection.\n" + CAPSTONE_MARK_GUIDANCE, submissionType: "Portfolio + inquiry product + source record + oral defence", marks: 72, outcomeIds: outcomes.map((outcome) => outcome.outcomeId).join(" | "), rubricIds: "rub-writing-v1 | rub-speaking-v1", origin: "Ehel Grade 7 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], quizzes, liveSessions, teacherNotes: [{ teacherNoteId: `${id}-note01`, unitId: id, noteType: "Capstone moderation", note: "Conference at each milestone, verify responsible attribution and source evaluation, preserve learner ownership, assess the inquiry product and five-minute oral defence with shared rubrics, and record actionable Grade 8 transition feedback.", visibility: "Teacher", origin: "Curriculum review", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], answerKey: [...activities.map((activity) => ({ answerId: `${activity.activityId}-answer`, unitId: id, contentId: activity.activityId, contentType: "Capstone milestone", answerOrGuidance: activity.answerSummary, origin: activity.origin, reviewStatus: "Approved v1.2", sourceFile: activity.sourceFile })), ...quizzes.map((quiz) => ({ answerId: `${quiz.questionId}-answer`, unitId: id, contentId: quiz.questionId, contentType: "Quiz", answerOrGuidance: `${quiz.correctAnswer} ${quiz.explanation}`, origin: quiz.origin, reviewStatus: "Approved v1.2", sourceFile: quiz.sourceFile }))], selfAssessment: outcomes.map((outcome, index) => ({ selfAssessmentId: `${id}-self0${index + 1}`, unitId: id, sequence: index + 1, statement: outcome.learningOutcome.replace(/^(Select|Use|Analyse|Create|Present|Evaluate)/, (verb) => `I can ${verb.toLowerCase()}`), scale: "Not yet | With help | By myself", outcomeId: outcome.outcomeId, origin: "Capstone reflection", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" })) };
}

function buildFinalAssessment(units) {
  const selected = units.slice(0, 9).flatMap((unit) => unit.quizzes.slice(0, 4)).slice(0, 30);
  const sections = [
    { sectionId: "grade7-final-sec01", sequence: 1, title: "Words and Meaning", description: "Use Grade 7 vocabulary accurately in familiar and new contexts.", startQuestion: 1, endQuestion: 10, questionCount: 10 },
    { sectionId: "grade7-final-sec02", sequence: 2, title: "Reading and Language", description: "Show reading understanding and control of Grade 7 language patterns.", startQuestion: 11, endQuestion: 20, questionCount: 10 },
    { sectionId: "grade7-final-sec03", sequence: 3, title: "Communication and Application", description: "Apply English to speaking, writing and purposeful communication.", startQuestion: 21, endQuestion: 30, questionCount: 10 },
  ];
  const questions = selected.map((quiz, index) => {
    const sourceUnit = units.find((unit) => unit.unit.unitId === quiz.unitId);
    return { ...quiz, assessmentId: "eng-g07-course-final-quiz-v12", quizId: "eng-g07-course-final-quiz-v12", questionId: `eng-g07-final-q${String(index + 1).padStart(2, "0")}`, sequence: index + 1, sectionId: sections[Math.floor(index / 10)].sectionId, sourceUnitNo: sourceUnit?.unit.unitNo, sourceUnitId: quiz.unitId, sourceUnitTitle: sourceUnit?.unit.unitTitle, curriculumArea: Math.floor(index / 10) === 0 ? "Vocabulary and meaning" : Math.floor(index / 10) === 1 ? "Reading and language" : "Communication and application", reviewRoute: `Review Unit ${sourceUnit?.unit.unitNo || 1}`, audio: pendingAudio(`./media/audio/final-quiz/eng-g07-final-q${String(index + 1).padStart(2, "0")}.mp3`) };
  });
  return { schemaVersion: "Ehel English Course Final Assessment v1.2", assessmentId: "eng-g07-course-final-quiz-v12", gradeId: "g07", subject: "English", title: "Grade 7 English Final Course Quiz", description: "A cumulative assessment of vocabulary, reading, language and communication across Units 1-9.", placement: "After Unit 10 capstone", questionCount: 30, totalMarks: 30, estimatedMinutes: 30, passPercent: 80, attemptsAllowed: "Teacher or school policy", sections, questions, reviewStatus: "Approved v1.2", audioProvider: "ElevenLabs", voiceId: VOICE_ID };
}
...
const dictionary = { schemaVersion: "Ehel Master Dictionary v1.2", language: "en-GB", gradeId: "g07", entryCount: dictionaryEntries.size, entries: [...dictionaryEntries.values()] };
const finalAssessment = buildFinalAssessment(units);
const manifest = { schemaVersion: "Ehel Grade 7 English Course Manifest v1.2", grade: { id: "g07", label: "Grade 7" }, subject: "English", defaultUnit: 1, units: units.map((unit) => ({ number: unit.unit.unitNo, id: unit.unit.unitId, termId: unit.term.id, title: unit.unit.unitTitle, data: `./data/units/unit-${unit.unit.unitNo}.json`, vocabularyCount: unit.dictionaryLinks.length, reviewStatus: unit.unit.reviewStatus })), finalAssessment: { id: finalAssessment.assessmentId, title: finalAssessment.title, data: "./data/course-final-quiz.json", placement: finalAssessment.placement, questionCount: 30, passPercent: 80, reviewStatus: finalAssessment.reviewStatus } };
...
fs.writeFileSync(path.join(DATA_DIR, "master-dictionary.grade7.json"), JSON.stringify(dictionary, null, 2));
fs.writeFileSync(path.join(DATA_DIR, "course-final-quiz.json"), JSON.stringify(finalAssessment, null, 2));
console.log(`Built Grade 7 English: ${units.length} units, ${dictionary.entryCount} dictionary entries, ${finalAssessment.questionCount}-question final quiz.`);
