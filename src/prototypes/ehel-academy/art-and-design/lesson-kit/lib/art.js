  ...
  /* ==================================================================
     ...
     Every line this page speaks goes through VOICE.speak or VOICE.follow
     (say() is one line in voice.js, the Explain button and the praise
     banks call VOICE directly). So the recorded voice is ONE wrapper round
     those two methods, here, and voice.js stays byte-identical to the
     other kits' copy.

     A line is split into sentences and each sentence is looked up by
     cyrb53(sentence) in media/tts/index.json - the naming every other
     subject's narration uses (tools/lib/ehel-narration-hash.js). If EVERY
     sentence of the line has a clip, the clips play in order; if any does
     not, the whole line goes to the voice engine exactly as before. All or
     nothing, so a sentence is never read by two different voices. The
     clips are the platform's own ElevenLabs voice (XfNU2rGpBa01ckF309OY,
     the one quiz_tts.php speaks at runtime), so a line composed at runtime
     and a recorded one are the same speaker.

     The text functions sit between the two markers below and are the ONE
     definition: lesson-kit/narrate.mjs lifts them out of this file to
     decide what to record, and check-narration.mjs holds cyrb53 equal to
     the shared one. Change a rule here and every hash moves with it.
     ================================================================== */
  /* NARRATION-TEXT-START */
  const N_PICTO = /[\u{1F000}-\u{1FAFF}\u{2190}-\u{21FF}\u{2300}-\u{27BF}\u{2B00}-\u{2BFF}\u{FE0F}\u{200D}]/gu;
  const N_ENT = { "&amp;": "&", "&lt;": "<", "&gt;": ">", "&quot;": '"', "&#39;": "'", "&nbsp;": " " };
  /* the words of a line, SSML or not: a pause of 300ms or more stands for
     the full stop it was written in place of, as on the platform path */
  function speechText(x) {
    let s = String(x == null ? "" : x);
    s = s.replace(/<break[^>]*time="(\d+)ms"[^>]*\/?>/g, (m, ms) => (Number(ms) >= 300 ? "  " : " "));
    s = s.replace(/<[^>]+>/g, " ");
    s = s.replace(/&(amp|lt|gt|quot|#39|nbsp);/g, (m) => N_ENT[m]);
    s = s.replace(N_PICTO, " ");
    s = s.replace(/([^.!?…\s])\s*/g, "$1.").replace(//g, " ");
    return s.replace(/\s+/g, " ").replace(/\s+([.,!?…])/g, "$1").trim();
  }
  function sentencesOf(x) {
    const t = speechText(x);
    if (!t) return [];
    return (t.match(/[^.!?…]+(?:[.!?…]+["'”’)]*|$)/g) || [])
      .map((u) => u.replace(/\s+/g, " ").trim()).filter((u) => /[A-Za-z0-9]/.test(u));
  }
  ...
  /* THE INDEX A PAGE READS IS NAMED FOR ITS CONTENT. The builders replace
     this line with index.<sha10>.json, a copy narrate.mjs writes beside
     index.json. A fixed name is read once and kept: the edge does not refresh
     when storage is overwritten, and it caches each compressed variant on its
     own - on 2026-09-11 the zstd copy a browser receives was still the
     morning's after a re-upload, so every newly recorded line went to the live
     voice while a plain request read back correct. A new name is a new URL. */
  const NARRATION_INDEX = "index.json";
  const NARRATION = (function () {
    const BASE = "media/tts/";
    const el = typeof Audio === "function" ? new Audio() : null;
    let have = null, queue = [], after = [], playing = false, token = 0, line = null;
    const log = window.__artNarrationLog = window.__artNarrationLog || [];
    try {
      fetch(BASE + NARRATION_INDEX).then((r) => (r.ok ? r.json() : null))
        .then((j) => { if (j && Array.isArray(j.clips)) have = new Set(j.clips); })
        .catch(() => { /* no recordings: every line goes to the voice engine */ });
    } catch (_) { /* no fetch: the same */ }
    const ORIG = { speak: VOICE.speak, follow: VOICE.follow, stop: VOICE.stop, speaking: VOICE.speaking };
    function clipsFor(x) {
      const ss = sentencesOf(x);
      const hs = ss.map((s) => cyrb53(s));
      const ok = !!(have && el && hs.length && hs.every((h) => have.has(h)));
      if (log.length < 4000) log.push({ s: ss, ok });
      return ok ? hs : null;
    }
    function on(v) { playing = v; document.body.classList.toggle("voice-on", v); }
    function halt() { token++; queue = []; try { el.pause(); el.removeAttribute("src"); } catch (_) { /* nothing */ } on(false); }
    function run(t) {
      if (t !== token) return;
      const h = queue.shift();
      if (!h) { on(false); const next = after.shift(); if (next) next(); return; }
      el.src = BASE + h + ".mp3";
      el.onended = () => setTimeout(() => run(t), 160);
      el.onerror = () => run(t);   /* a clip that will not load must not wedge the rest */
      const p = el.play();
      /* a refused play (autoplay policy) hands the whole line to the engine */
      if (p && p.catch) p.catch(() => { if (t === token && line != null) { const x = line; halt(); ORIG.speak(x); } });
    }
    VOICE.speak = function (x) {
      const hs = clipsFor(x);
      halt(); after = [];
      if (!hs) { ORIG.speak(x); return; }
      ORIG.stop(); line = x; queue = hs.slice(); on(true); run(token);
    };
    VOICE.follow = function (x) {
      const hs = clipsFor(x);
      if (playing) { if (hs) queue.push(...hs); else after.push(() => ORIG.follow(x)); return; }
      if (hs && !ORIG.speaking()) { line = x; queue = hs.slice(); on(true); run(token); return; }
      ORIG.follow(x);
    };
    VOICE.stop = function () { halt(); after = []; ORIG.stop(); };
    VOICE.speaking = function () { return playing || ORIG.speaking(); };
    return { ready: () => !!have, count: () => (have ? have.size : 0) };
  })();
  window.__artNarration = { sentencesOf, speechText, cyrb53, ready: () => NARRATION.ready(), count: () => NARRATION.count() };
  ...
  const SCENES = {
    ...
    /* ---- added for Grade 2 (2026-09-11). Each is drawn IN THE MANNER of a
       tradition and copies no real work; build-hub.py :: TRADITIONS says
       which, for the grown-ups. ---- */
    /* stamped cloth: the same stamps pressed again and again, in squares
       ruled with a comb - the manner of adinkra cloth from Ghana */
    adinkra: { label: "A cloth printed with stamps", draw: () => {
      let g = '<rect width="320" height="240" fill="#E8D2A6"/>';
      for (const x of [104, 212]) for (let k = 0; k < 4; k++) g += '<rect x="' + (x + k * 4) + '" y="0" width="2" height="240" fill="#3B2314"/>';
      for (let k = 0; k < 4; k++) g += '<rect x="0" y="' + (116 + k * 4) + '" width="320" height="2" fill="#3B2314"/>';
      const ring = (x, y) => '<circle cx="' + x + '" cy="' + y + '" r="15" fill="none" stroke="#2B1A10" stroke-width="5"/><circle cx="' + x + '" cy="' + y + '" r="5" fill="#2B1A10"/>';
      const diamond = (x, y) => '<path d="M' + x + " " + (y - 17) + " l17 17 l-17 17 l-17-17z" + '" fill="#2B1A10"/><circle cx="' + x + '" cy="' + y + '" r="5" fill="#E8D2A6"/>';
      const flower = (x, y) => '<g fill="#2B1A10">' + [[0, -11], [11, 0], [0, 11], [-11, 0], [0, 0]].map((d) => '<circle cx="' + (x + d[0]) + '" cy="' + (y + d[1]) + '" r="6"/>').join("") + "</g>";
      const cells = [[0, 104, ring], [120, 212, diamond], [228, 320, flower]];
      for (const [x0, x1, stamp] of cells) for (const [y0, y1] of [[0, 116], [132, 240]]) {
        const cx = (x0 + x1) / 2, cy = (y0 + y1) / 2;
        for (const dx of [-24, 24]) for (const dy of [-26, 26]) g += stamp(cx + dx, cy + dy);
      }
      return g;
    } },
    /* a weaving on a card loom: straight warp threads, and weft that goes
       over one and under the next, in stripes */
    loom: { label: "A weaving on a loom", draw: () => {
      let g = '<rect width="320" height="240" fill="#C9A06B"/><rect x="40" y="10" width="240" height="220" rx="8" fill="#F7F1E3"/>';
      const warps = []; for (let x = 60; x <= 260; x += 20) warps.push(x);
      const stripes = ["#E0312B", "#E0312B", "#F6C700", "#F6C700", "#2D6CDF", "#2D6CDF"];
      for (const x of warps) g += '<rect x="' + (x - 2) + '" y="16" width="4" height="208" fill="#FFFFFF" stroke="#B8A58A" stroke-width="1"/>';
      for (let r = 0; r < 11; r++) {
        const y = 60 + r * 15, col = stripes[r % stripes.length];
        g += '<rect x="56" y="' + y + '" width="208" height="12" rx="4" fill="' + col + '"/>';
        warps.forEach((x, c) => { if ((r + c) % 2) g += '<rect x="' + (x - 2) + '" y="' + (y - 1) + '" width="4" height="14" fill="#FFFFFF"/>'; });
      }
      return g + '<g fill="#8B5A2B">' + warps.map((x) => '<path d="M' + (x - 4) + ' 10 h8 l-4 7z"/><path d="M' + (x - 4) + ' 230 h8 l-4 -7z"/>').join("") + "</g>";
    } },
    /* a clay head from long ago: fired orange clay, rows of hair, eyes cut
       as half-moons with holes - the manner of Nok terracotta from Nigeria */
    terracotta: { label: "A clay head from long ago", draw: () =>
      '<rect width="320" height="240" fill="#F3E6D2"/><rect x="0" y="206" width="320" height="34" fill="#D9C6A5"/>' +
      '<path d="M112 196 q-18-60 -6-110 q12-50 54-58 q42 8 54 58 q12 50 -6 110 q-48 22 -96 0z" fill="#B5652B"/>' +
      '<g fill="none" stroke="#7A3A1C" stroke-width="4" stroke-linecap="round"><path d="M120 76 q40-34 80 0"/><path d="M124 62 q36-30 72 0"/><path d="M132 48 q28-24 56 0"/><path d="M144 36 q16-12 32 0"/></g>' +
      '<g fill="#7A3A1C"><path d="M120 108 q18-16 36 0 q-18 12 -36 0z"/><path d="M164 108 q18-16 36 0 q-18 12 -36 0z"/></g>' +
      '<g fill="#2B1A10"><circle cx="138" cy="106" r="3.5"/><circle cx="182" cy="106" r="3.5"/><ellipse cx="160" cy="170" rx="14" ry="7"/></g>' +
      '<path d="M160 116 v26 q-8 6 0 8 q8-2 0-8" fill="none" stroke="#7A3A1C" stroke-width="4" stroke-linecap="round"/>' +
      '<g fill="#9A5122"><ellipse cx="104" cy="120" rx="8" ry="16"/><ellipse cx="216" cy="120" rx="8" ry="16"/></g>' },
    /* a leaf to draw from life: a straight middle vein, little veins off it,
       a jagged edge and a stalk */
    leaf: { label: "A leaf, looked at closely", draw: () => {
      const pts = [];
      for (let k = 0; k <= 40; k++) {
        const t = k / 40, a = t * Math.PI * 2, r = (k % 2 ? 0.9 : 1);
        pts.push([160 + Math.cos(a) * 118 * r * 0.95, 116 + Math.sin(a) * 70 * r]);
      }
      return '<rect width="320" height="240" fill="#FFF8E7"/>' +
        '<path d="M' + pts.map((p) => p[0].toFixed(1) + " " + p[1].toFixed(1)).join(" L") + 'z" fill="#3FA34D" stroke="#1F5E2A" stroke-width="3" stroke-linejoin="miter" transform="rotate(-18 160 116)"/>' +
        '<g stroke="#D6F0C8" stroke-width="3" stroke-linecap="round" fill="none" transform="rotate(-18 160 116)">' +
        '<path d="M54 116 H252" stroke-width="5"/>' +
        [88, 126, 164, 202].map((x) => '<path d="M' + x + " 116 l24 -40 M" + x + ' 116 l24 40"/>').join("") + "</g>" +
        '<path d="M58 150 q-20 30 -34 70" fill="none" stroke="#1F5E2A" stroke-width="7" stroke-linecap="round"/>';
    } },
    /* a paper cut: red paper folded in half and cut, so both halves are the
       same - the manner of Chinese paper cutting */
    papercut: { label: "A paper cut, folded and cut", draw: () => {
      const half =
        '<path d="M160 22 q-70 6 -100 60 q-26 60 12 110 q34 34 88 34 z" fill="#C8102E"/>' +
        '<g fill="#FFF4E0"><path d="M126 60 q-16 22 0 40 q16-18 0-40z"/><path d="M96 96 q-14 20 2 36 q14-18 -2-36z"/>' +
        '<circle cx="136" cy="130" r="9"/><circle cx="104" cy="160" r="7"/><path d="M140 170 q-22 8 -26 26 q22-4 26-26z"/><circle cx="150" cy="96" r="5"/></g>';
      return '<rect width="320" height="240" fill="#FFF4E0"/>' + half + '<g transform="translate(320 0) scale(-1 1)">' + half + "</g>" +
        '<path d="M160 14 V234" stroke="#8A8F94" stroke-width="1.5" stroke-dasharray="5 5"/>';
    } },
    /* ---- added for Grade 3 (2026-09-11) ---- */
    /* a street front to gather patterns from: bricks, an arched window,
       railings and a tiled roof */
    buildings: { label: "A building with patterns on it", draw: () => {
      let g = '<rect width="320" height="240" fill="#F3E6D2"/>';
      for (let r = 0; r < 9; r++) for (let c = -1; c < 9; c++) {
        const x = c * 40 + (r % 2 ? 20 : 0), y = 44 + r * 18;
        g += '<rect x="' + x + '" y="' + y + '" width="38" height="16" fill="#B5652B" stroke="#E8D2A6" stroke-width="2"/>';
      }
      for (let c = 0; c < 9; c++) g += '<path d="M' + (c * 40 - 4) + ' 44 q24 -26 48 0 z" fill="#7A3A1C" stroke="#5C2A12" stroke-width="2"/>';
      g += '<path d="M120 190 v-86 a40 40 0 0 1 80 0 v86 z" fill="#9CC8F0" stroke="#F7F1E3" stroke-width="8"/>' +
        '<path d="M160 64 v126 M120 124 h80" stroke="#F7F1E3" stroke-width="5"/>';
      g += '<rect x="0" y="196" width="320" height="44" fill="#D9C6A5"/>';
      for (let x = 8; x < 320; x += 16) g += '<path d="M' + x + ' 238 V204 M' + (x - 4) + ' 208 l4 -8 l4 8" stroke="#1B1B1B" stroke-width="3" fill="none"/>';
      return g + '<path d="M0 214 H320" stroke="#1B1B1B" stroke-width="3"/>';
    } },
    /* a pencil drawing of an apple lit from the left: a highlight, a shadow
       side and a cast shadow on the table */
    stilllife: { label: "A pencil drawing of an apple, lit from one side", draw: () =>
      '<defs><radialGradient id="appleTone" cx="0.32" cy="0.34" r="0.75"><stop offset="0" stop-color="#F2F2F2"/><stop offset="0.35" stop-color="#B8B8B8"/><stop offset="0.8" stop-color="#5E5E5E"/><stop offset="1" stop-color="#3A3A3A"/></radialGradient></defs>' +
      '<rect width="320" height="240" fill="#FAFAF7"/><rect x="0" y="170" width="320" height="70" fill="#E6E6E1"/>' +
      '<ellipse cx="200" cy="186" rx="86" ry="14" fill="#6B6B6B" opacity="0.55"/>' +
      '<path d="M160 72 q-58 -8 -62 48 q-2 58 62 66 q64 -8 62 -66 q-4 -56 -62 -48z" fill="url(#appleTone)" stroke="#3A3A3A" stroke-width="2"/>' +
      '<ellipse cx="136" cy="100" rx="12" ry="8" fill="#FFFFFF"/>' +
      '<path d="M160 74 q2 -18 10 -26" stroke="#3A3A3A" stroke-width="4" fill="none" stroke-linecap="round"/>' +
      '<g stroke="#8A8A8A" stroke-width="2"><path d="M198 132 l14 -10 M200 144 l16 -12 M198 156 l18 -14 M194 166 l16 -12"/></g>' +
      '<g fill="#F6C700" stroke="#C99700" stroke-width="2"><circle cx="30" cy="36" r="14"/></g>' +
      '<g stroke="#C99700" stroke-width="3"><path d="M50 44 l18 8 M46 56 l16 14 M36 60 l8 18"/></g>' },
    /* the colour wheel: three primaries with the secondaries between them */
    colourwheel: { label: "A colour wheel", draw: () => {
      const cols = [["#E0312B", "red"], ["#F28C28", "orange"], ["#F6C700", "yellow"], ["#3FA34D", "green"], ["#2D6CDF", "blue"], ["#7B3FA0", "purple"]];
      let g = '<rect width="320" height="240" fill="#FAFAF7"/>';
      cols.forEach(([c], k) => {
        const a0 = (k * 60 - 120) * Math.PI / 180, a1 = ((k + 1) * 60 - 120) * Math.PI / 180, R = 100, cx = 160, cy = 120;
        g += '<path d="M' + cx + " " + cy + " L" + (cx + R * Math.cos(a0)).toFixed(1) + " " + (cy + R * Math.sin(a0)).toFixed(1) +
          " A" + R + " " + R + " 0 0 1 " + (cx + R * Math.cos(a1)).toFixed(1) + " " + (cy + R * Math.sin(a1)).toFixed(1) + 'z" fill="' + c + '" stroke="#FFFFFF" stroke-width="4"/>';
      });
      return g + '<circle cx="160" cy="120" r="26" fill="#FAFAF7"/>';
    } },
    /* a clay brick with a picture raised on it: a horse and cart, and a
       border of pressed dots - the manner of Han dynasty pictorial bricks */
    relief: { label: "A clay brick with a raised picture", draw: () => {
      let g = '<rect width="320" height="240" fill="#EFE3CF"/><rect x="20" y="24" width="280" height="192" rx="6" fill="#A9552B"/>' +
        '<rect x="34" y="38" width="252" height="164" rx="4" fill="#9A4A24"/>';
      for (let x = 40; x <= 280; x += 16) g += '<circle cx="' + x + '" cy="31" r="3" fill="#6E3219"/><circle cx="' + x + '" cy="209" r="3" fill="#6E3219"/>';
      const horse = 'M70 150 q6 -30 30 -34 l6 -16 l8 4 l-2 12 q30 2 44 16 l2 28 h-8 l-2 -20 l-30 4 l-2 20 h-8 l-2 -22 q-12 2 -18 12 l-2 14 h-8z';
      const cart = 'M160 128 h70 v22 h-70z';
      g += '<g transform="translate(4 4)" fill="#6E3219" opacity="0.7"><path d="' + horse + '"/><path d="' + cart + '"/><circle cx="210" cy="164" r="18"/></g>' +
        '<g fill="#C8743F" stroke="#E09A62" stroke-width="2"><path d="' + horse + '"/><path d="' + cart + '"/><circle cx="210" cy="164" r="18"/></g>' +
        '<path d="M210 146 v36 M192 164 h36" stroke="#9A4A24" stroke-width="3"/><path d="M150 134 l10 -2" stroke="#E09A62" stroke-width="3"/>' +
        '<g fill="#C8743F" stroke="#E09A62" stroke-width="2"><circle cx="186" cy="112" r="8"/><path d="M178 122 h16 v10 h-16z"/></g>';
      return g;
    } },
    /* a quilt of running stitches: rows of small stitches and a fish drawn in
       stitches - the manner of kantha from Bengal */
    kantha: { label: "A cloth stitched with running stitch", draw: () => {
      let g = '<rect width="320" height="240" fill="#6E8F6A"/><rect x="14" y="12" width="292" height="216" rx="6" fill="#F4ECDD"/><rect x="10" y="8" width="292" height="216" rx="6" fill="#FBF6EA" stroke="#E8D9BD" stroke-width="2"/>';
      let row = 0;
      for (let y = 26; y < 220; y += 14) if (y < 70 || y > 176) g += '<path d="M22 ' + y + ' H292" stroke="' + (row++ % 2 ? "#2D6CDF" : "#C8102E") + '" stroke-width="3" stroke-dasharray="7 6"/>';
      g += '<path d="M70 124 q60 -56 130 0 q-70 56 -130 0z M200 124 l40 -26 v52z" fill="none" stroke="#C8102E" stroke-width="3" stroke-dasharray="6 5"/>' +
        '<circle cx="104" cy="118" r="5" fill="#2D6CDF"/>' +
        '<path d="M120 110 q20 14 0 28 M140 108 q20 16 0 32 M160 110 q20 14 0 28" fill="none" stroke="#2D6CDF" stroke-width="3" stroke-dasharray="5 4"/>';
      return g;
    } },
    /* a painted house wall: thick black outlines around bold coloured
       shapes - the manner of Ndebele house painting from South Africa */
    ndebele: { label: "A house wall painted with bold shapes", draw: () => {
      let g = '<rect width="320" height="240" fill="#F7F1E3"/>';
      const panel = (x, c) => '<rect x="' + x + '" y="30" width="60" height="150" fill="' + c + '" stroke="#1B1B1B" stroke-width="6"/>' +
        '<path d="M' + (x + 10) + ' 150 l10 -20 l10 20 l10 -20 l10 20 l10 -20" fill="none" stroke="#1B1B1B" stroke-width="5"/>' +
        '<path d="M' + (x + 30) + ' 50 l20 20 l-20 20 l-20 -20z" fill="#F7F1E3" stroke="#1B1B1B" stroke-width="5"/>';
      g += panel(10, "#2D6CDF") + panel(250, "#3FA34D");
      g += '<path d="M110 180 V70 h20 v-20 h60 v20 h20 v110z" fill="#F6C700" stroke="#1B1B1B" stroke-width="6"/>' +
        '<rect x="136" y="96" width="48" height="84" fill="#E0312B" stroke="#1B1B1B" stroke-width="6"/>' +
        '<path d="M78 30 v150 M242 30 v150" stroke="#1B1B1B" stroke-width="4"/>';
      for (let x = 0; x < 320; x += 20) g += '<rect x="' + x + '" y="186" width="20" height="54" fill="' + (x % 40 ? "#1B1B1B" : "#F7F1E3") + '"/>';
      return g;
    } },
    /* a night sky of swirling strokes, glowing stars and a tall dark tree -
       the manner of Vincent van Gogh's painting of a starry night */
    swirlnight: { label: "A painting of a swirling night sky", draw: () => {
      let g = '<rect width="320" height="240" fill="#1B3A75"/>';
      for (let k = 0; k < 7; k++) g += '<path d="M' + (20 + k * 40) + ' ' + (120 - (k % 2) * 16) + ' q20 -30 40 0 t40 0" fill="none" stroke="' + (k % 2 ? "#6E9DE8" : "#9CC8F0") + '" stroke-width="4" stroke-linecap="round"/>';
      g += '<path d="M120 80 q30 -40 60 -10 q20 24 -6 34 q-20 6 -24 -10 q2 -14 16 -12" fill="none" stroke="#DCE9F8" stroke-width="5" stroke-linecap="round"/>';
      for (const [x, y, r] of [[40, 40, 10], [230, 36, 9], [280, 90, 8], [90, 96, 7], [196, 108, 7]]) g += '<circle cx="' + x + '" cy="' + y + '" r="' + (r + 9) + '" fill="none" stroke="#F6C700" stroke-width="3" opacity="0.7"/><circle cx="' + x + '" cy="' + y + '" r="' + r + '" fill="#FBE99A"/>';
      g += '<path d="M270 30 a18 18 0 1 0 18 26 a14 14 0 1 1 -18 -26z" fill="#F6C700"/>';
      g += '<path d="M0 200 q60 -30 120 -10 q60 -20 120 0 q40 -12 80 0 v50 h-320z" fill="#16304F"/>';
      for (const x of [150, 180, 210, 240]) g += '<rect x="' + x + '" y="196" width="18" height="14" fill="#F3E6D2"/><path d="M' + (x - 2) + ' 196 l11 -8 l11 8z" fill="#5C3A1E"/>';
      g += '<path d="M52 240 q-8 -80 8 -150 q10 40 8 70 q12 -30 10 -60 q14 50 4 140z" fill="#0B1D2C"/>';
      return g;
    } },
    /* a huge curling wave with claw-like foam, little boats and a small
       mountain far away - the manner of Katsushika Hokusai's woodblock print */
    greatwave: { label: "A print of a great wave", draw: () =>
      '<rect width="320" height="240" fill="#EDE1C8"/>' +
      '<path d="M222 162 l30 -38 l30 38z" fill="#1B3A75"/><path d="M240 142 l12 -18 l12 18z" fill="#FFFFFF"/>' +
      '<path d="M0 240 V150 q40 -120 150 -110 q40 6 50 36 q-30 -20 -56 0 q-40 34 -30 94 q20 -30 60 -10 q40 20 70 -10 q30 -30 76 -10 V240z" fill="#1B3A75"/>' +
      '<path d="M0 240 V170 q36 -100 140 -104 q-40 20 -50 70 q-6 40 20 60 q-40 -10 -60 20z" fill="#2D6CDF" opacity="0.8"/>' +
  ...
  const CHECKS = {
    ...
    /* sharp <= 2: without it a big zigzag passed as a circle - it turns
       819 degrees and ends far from nowhere in particular (measured on the
       keyboard route's own zigzag, 2026-09-11) */
    round: (f) => f.len > 90 && f.straightness < 0.35 && f.turning > 250 && f.sharp <= 2,
    dots: (f) => f.dots >= 6,
  ...
  const TOOLS = {
    ...
    /* CHALK IS A COLOURED CHALK, not white. It was #F7F1E3 on paper that is
       #FFFDF6 - a contrast of about 1.07, so the lesson's chalk marks were
       drawn and could not be seen (found while drawing the mark previews for
       the keyboard route, 2026-09-11). */
    chalk: { label: "Chalk", pic: "\u{1F9F1}", colour: "#C2417A", width: 7, cap: "butt", dashed: "3 3", sound: "rustle" },
    ...
    /* CHARCOAL (Grade 3): thick and dark, the tool a "thick" mark is
  ...
  /* THE KEYBOARD AND SWITCH ROUTE (validation area 15, 2026-09-11). The
     paper is a pointer surface, so a child who cannot drag - a switch user,
     a keyboard user, a child whose hands are not ready yet - had no way
     through the marks step at all. Every mark the step can ask for is also
     a shape this function can draw, and the shapes are judged by the SAME
     CHECKS a hand-drawn stroke is: check-coverage runs every one of them
     through strokeFeatures, so a check tightened later cannot quietly make
     the keyboard route impossible. The child still has to KNOW the mark -
     they pick it out of three - so this is a different way to show the
     same thing, not a way round it. */
  const MARK_NAME = {
    straight: "a straight line", wavy: "a wavy line", zigzag: "a zigzag", long: "a long line", short: "a short mark",
    round: "a circle", dots: "dots", thick: "a thick line", thin: "a thin line",
  };
  const MARK_RIVALS = {
    straight: ["wavy", "zigzag"], wavy: ["straight", "zigzag"], zigzag: ["wavy", "round"], round: ["zigzag", "dots"],
    dots: ["straight", "round"], long: ["short", "dots"], short: ["long", "wavy"], thick: ["thin"], thin: ["thick"],
  };
  function markPts(want, dy) {
    dy = dy || 0;
    const line = (x0, y0, x1, y1) => {
      const n = Math.max(2, Math.round(Math.hypot(x1 - x0, y1 - y0) / 3)), out = [];
      for (let i = 0; i <= n; i++) out.push({ x: Math.round(x0 + (x1 - x0) * i / n), y: Math.round(y0 + dy + (y1 - y0) * i / n) });
      return out;
    };
    const pts = [];
    if (want === "straight") return [line(50, 110, 270, 110)];
    if (want === "long") return [line(20, 110, 300, 110)];
    if (want === "short") return [line(140, 110, 180, 110)];
    if (want === "thick" || want === "thin") return [line(60, 110, 260, 110)];
    if (want === "wavy") { for (let x = 30; x <= 290; x += 3) pts.push({ x, y: Math.round(110 + dy + 32 * Math.sin((x - 30) / 26)) }); return [pts]; }
    if (want === "zigzag") {
      const c = [[30, 165], [70, 55], [110, 165], [150, 55], [190, 165], [230, 55], [270, 165]];
      for (let i = 1; i < c.length; i++) pts.push(...line(c[i - 1][0], c[i - 1][1], c[i][0], c[i][1]).slice(i > 1 ? 1 : 0));
      return [pts.map((p) => ({ x: p.x, y: p.y }))];
    }
    if (want === "round") { for (let a = 0; a <= 372; a += 4) pts.push({ x: Math.round(160 + 62 * Math.cos(a * Math.PI / 180)), y: Math.round(110 + dy + 62 * Math.sin(a * Math.PI / 180)) }); return [pts]; }
    if (want === "dots") return [0, 1, 2, 3, 4, 5].map((i) => [{ x: 60 + 40 * i, y: 110 + dy + (i % 2 ? -22 : 22) }]);
    return [];
  }
  /* the little picture of a mark on its choice button */
  function markPreview(want, tool) {
    const t = TOOLS[tool] || TOOLS.pencil;
    const w = want === "thin" ? 3 : want === "thick" ? 16 : Math.min(t.width, 9);
    const paths = markPts(want).map((pts) => pts.length === 1
      ? '<circle cx="' + pts[0].x + '" cy="' + pts[0].y + '" r="' + Math.max(6, w) + '" fill="' + t.colour + '"/>'
      : '<path d="M' + pts.map((p) => p.x + " " + p.y).join(" L") + '" fill="none" stroke="' + t.colour + '" stroke-width="' + w + '" stroke-linecap="round" stroke-linejoin="round"/>');
    return '<svg viewBox="0 0 320 220" aria-hidden="true"><rect width="320" height="220" rx="14" fill="#FFFDF6"/>' + paths.join("") + "</svg>";
  }
  function markMaker(o) {
    ...
    const STAMPS = ["straight", "wavy", "zigzag", "round", "dots"];
    let opened = false, choices = [];
    const paperLabel = () => "Your paper, " + (strokes.length ? "with " + strokes.length + (strokes.length === 1 ? " mark" : " marks") + " on it" : "empty");
    const labelPaper = () => { const s = $(svgId); if (s) s.setAttribute("aria-label", paperLabel()); };
    function canvasSvg() {
      return '<svg viewBox="0 0 320 220" class="canvas" id="' + svgId + '" role="img" aria-label="' + paperLabel() + '" aria-describedby="' + el.stage + 'kb"><rect width="320" height="220" fill="#FFFDF6"/>' + strokes.map(strokeSvg).join("") + "</svg>";
    }
    function draw() {
      const rd = rounds[r];
      $(el.stage).innerHTML = '<div class="stagewide">' +
        ...
        '<div class="toolrow">' + tools.map((t) => '<button type="button" class="toolbtn' + (tool === t ? " on" : "") + '" data-t="' + t + '" aria-pressed="' + (tool === t) + '"><span class="cpic" aria-hidden="true">' + TOOLS[t].pic + "</span>" + esc(TOOLS[t].label) + "</button>").join("") + "</div>" +
        '<div class="sim canvasbox">' + canvasSvg() + "</div>" +
        '<p class="kbnote" id="' + el.stage + 'kb">' + (free() ? "Draw on the paper with a finger or the mouse, or add a mark with the buttons." : "Draw on the paper with a finger or the mouse, or press Choose the mark.") + "</p>" +
        (free()
          ? '<div class="markopts stamps" role="group" aria-label="Add a mark"><span class="phase">Add a mark</span>' + STAMPS.map((w) => '<button type="button" class="markopt" data-stamp="' + w + '" aria-label="Add ' + MARK_NAME[w] + '">' + markPreview(w, tool) + "</button>").join("") + "</div>"
          : '<div class="markopts" id="' + el.stage + 'opts" role="group" aria-label="Which one is ' + MARK_NAME[rd.want] + '?"' + (opened ? "" : " hidden") + ">" +
            choices.map((w) => '<button type="button" class="markopt" data-w="' + w + '" aria-label="' + cap1(MARK_NAME[w]) + '">' + markPreview(w, rd.tool) + "</button>").join("") + "</div>") +
        '<div class="bigbtns">' +
        (free() ? "" : '<button type="button" class="big small teal" data-alt="1" aria-expanded="' + opened + '" aria-controls="' + el.stage + 'opts">&#9000; Choose the mark</button>') +
        '<button type="button" class="big small ghost" id="' + el.stage + 'clear">Clean paper</button>' +
    ...
    /* draw a list of strokes onto the paper a few points at a time, the way
       a hand would, then hand the LAST one back to be judged. Every earlier
    ...
    function animate(list, toolName, then) {
      ...
      function next() {
        ...
        const p = pathEl(s); p.setAttribute("d", pathD(s.pts)); svg.appendChild(p);
    ...
    function judge(s) {
      ...
      } else if (rd.want === "dots" && f.len < 14) {
        /* A DOT ON THE WAY TO SIX IS PROGRESS, NOT A MISS. Every dot before
           the sixth used to fail the check and set `missed`, so the dots round
           could never score for anyone - found writing the keyboard route,
           which lays six dots down and had to decide what the first five
           were. */
        SOUND.play("pop", 0.2);
        $(el.fb).className = "fb"; $(el.fb).textContent = "Tap, tap. " + Math.max(1, 6 - session.dots) + " more.";
      } else {
    ...
    function startRound() {
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      if (!free()) { tool = rounds[r].tool; choices = shuffle([rounds[r].want].concat(MARK_RIVALS[rounds[r].want] || [])); }
      opened = false;
      draw();
    ...
    $(el.stage).addEventListener("click", (e) => {
      if (lock) return;
      const b = e.target.closest(".toolbtn");
      if (b) { tool = b.dataset.t; SOUND.play("click", 0.3); draw(); say(TOOLS[tool].label); return; }
      const alt = e.target.closest("[data-alt]");
      if (alt && !free()) {
        opened = !opened;
        $(el.stage + "opts").hidden = !opened; alt.setAttribute("aria-expanded", String(opened));
        if (opened) {
          SOUND.play("click", 0.3);
          const first = $(el.stage + "opts").querySelector(".markopt"); if (first) first.focus();
          say("Which one is " + MARK_NAME[rounds[r].want] + "? Tap it.");
        }
        return;
      }
      const pick = e.target.closest(".markopt[data-w]");
      if (pick && !free()) {
        const rd = rounds[r], w = pick.dataset.w;
        if (w !== rd.want) {
          missed = true; SOUND.play("error", 0.3); pick.classList.add("wrong");
          setTimeout(() => pick.classList.remove("wrong"), 700);
          const line = "That is " + MARK_NAME[w] + ". Look for " + MARK_NAME[rd.want] + ".";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          return;
        }
        lock = true; tool = rd.tool; strokes = []; session.dots = 0; opened = false; draw();
        animate(markPts(rd.want), rd.tool, (s) => { strokes.push(s); lock = false; judge(s); });
        return;
      }
      const stamp = e.target.closest(".markopt[data-stamp]");
      if (stamp && free()) {
        const w = stamp.dataset.stamp;
        lock = true;
        animate(markPts(w, rnd(-40, 40)), tool, (s) => {
          strokes.push(s); lock = false; judge(s);
          $(el.fb).className = "fb good"; $(el.fb).textContent = "You added " + MARK_NAME[w] + ".";
        });
      }
    });
    startRound();
  ...
  function patternMaker(o) {
    ...
    function checkOwn() {
      if (lock) return;
      /* four, not three: Grade 2 teaches a four-print repeat (review, 2026-09-11) */
      const p = patternPeriod(own, 4);
      if (p && p >= 2) {
  ...
  /* ---- a good place to stop (validation area 17, 2026-09-11) -------------
     A lesson here is forty minutes or more, and a five-year-old's sitting
     is about twenty. The journal is where the making and the looking back
     are done, so it is the natural half-way line, and it is offered there:
     stop now, or keep going. It is drawn INSIDE the journal step rather than
     as a step of its own on purpose - progress names every step by its
     position (step-01, step-02...), so a new step would move every step
     after it, the quiz's gradebook key included, under every record already
     written. Stopping first moves the page on one step, so the place that is
     kept is the next thing to do, not the journal just finished. */
  function pauseCard(o) {
    const el = o.el;
    const box = document.createElement("div");
    box.className = "pausecard";
    box.setAttribute("role", "group");
    box.setAttribute("aria-label", "A good place to stop");
    box.innerHTML = '<span class="cpic" aria-hidden="true">\u{1F319}</span><div class="pc-text"><p class="phase">A good place to stop</p>' +
      "<b>You made things and looked back at them. That is half the lesson.</b>" +
      '<p class="sub">You can stop now and come back later. Your place is kept.</p></div>' +
      '<div class="bigbtns"><button type="button" class="big small ghost" data-stop="1">Stop for today</button>' +
      '<button type="button" class="big small" data-go="1">Keep going &#9654;</button></div>';
    $(el.stage).appendChild(box);
    box.addEventListener("click", (e) => {
      if (e.target.closest("[data-go]")) { show(o.finish + 1, true); return; }
      if (!e.target.closest("[data-stop]")) return;
      const back = document.querySelector(".lesson-back");
      if (back && back.href) { show(o.finish + 1, false); setTimeout(() => { location.href = back.href; }, 350); return; }
      /* opened on its own, with no hub to go back to: the place kept is this
         step, so the sentence says what to press when they come back */
      box.querySelector(".bigbtns").innerHTML = '<p class="sub">Well done today. You can close this page now. When you come back, press Next to go on.</p>';
      say("Well done today. You can close this page now. When you come back, press Next to go on.");
    });
    afterVoice(() => sayHere(o.finish, "This is a good place to stop. You can stop for today, or keep going."));
  }
  ...
  function myJournal(o) {
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      setTimeout(() => { endStep(o); if (o.pause) pauseCard(o); }, 3400);
    });
    draw();
  ...
  function unitOverview(o) {
    ...
    /* ON LEAVING, never while drawing. The deck draws every step at page
       load, before the progress module exists - a module script runs only
       after the page is parsed - so a finish() here reached the dots and
       never the record: a lesson finished to the last sticker was stored
       13 of 15 steps (this one and Our world) and never complete (measured
       on the live Grade 1 page, 2026-09-11). Not "on arrival" either: the
       page arrives on this step during the load. Silent, because the next
       slide's own instruction is speaking. */
  ...
  function lecture(o) {
    const el = o.el;
    const parts = o.parts || [];
    let k = 0, furthest = 0, over = false;
    const id = el.stage + "l";
    /* the part-by-part view: the whole step when there is no video, and the
       transcript under the video when there is one */
    const host = () => $(o.video ? id + "parts" : el.stage);
    function done() {
      if (over) return;
      over = true;
    ...
    function paint() {
      const p = parts[k];
      if (!o.video) $(el.stage).className = "stagewide";
      host().innerHTML =
        '<div class="lec">' +
        ...
        "</div>" +
        (o.video ? "" : '<p class="lec-note">Read aloud by the lesson\'s voice.</p>') +
        "</div>";
    ...
    /* THE VIDEO, in the English build's shape: captions on by default, and
       the poster held back until the step is reached - every slide is in the
       DOM at once, so a `poster` attribute here would be fetched on page
       load for a step a child may never open. It rides data-poster and is
       promoted by the ONSHOW hook. The video and its captions are the same
       recorded sentences the Listen buttons below play. */
    $(el.stage).className = "stagewide";
    $(el.stage).innerHTML =
      '<div class="lec lecvid">' +
      '<video id="' + id + 'v" class="lecturevideo" controls playsinline preload="metadata" data-poster="' + esc(o.poster || "") + '">' +
      '<source src="' + esc(o.video) + '" type="video/mp4">' +
      (o.captions ? '<track kind="captions" srclang="en" label="English" src="' + esc(o.captions) + '" default>' : "") +
      "</video>" +
      '<div class="bigbtns"><button type="button" class="big small" id="' + id + 'watched">I watched it &#10003;</button></div>' +
      '<details class="lec-read"><summary>Read it part by part instead</summary><div id="' + id + 'parts"></div></details>' +
      "</div>";
    const video = $(id + "v");
    $(el.score).textContent = parts.length + " parts" + (o.seconds ? " · " + Math.max(1, Math.round(o.seconds / 60)) + " min" : "");
    video.addEventListener("play", () => { try { VOICE.stop(); } catch (_) { /* nothing */ } });
    video.addEventListener("timeupdate", () => {
      if (!o.seconds) return;
      const heard = Math.min(parts.length, Math.ceil(video.currentTime / o.seconds * parts.length));
      if (heard > furthest) { furthest = heard; reportAttempt(o.finish, furthest, parts.length, "parts"); }
    });
    video.addEventListener("ended", done);
    $(id + "watched").addEventListener("click", () => { try { video.pause(); } catch (_) { /* nothing */ } done(); });
    paint();
    ONSHOW[o.finish] = () => {
  ...
  function ourWorld(o) {
    ...
    /* on arrival, never while drawing - see unitOverview. Silent: the
       slide's own instruction is speaking. */
  ...
  /* ---- the make-at-home sheet, printed (validation area 24, 2026-09-11) --
     The projects are for a table covered in newspaper, which is not where
     the screen is. One A4 sheet per lesson: what to get, what to do, what to
     look for, and a box to stick the result in, so the page can go on the
     fridge and the making can happen away from the device. It prints the
     page itself rather than opening a window - a pop-up is blocked more
     often than not inside a school's launcher - by filling #printsheet and
     letting @media print show that alone. break-inside: avoid is safe here
     only because every project is far shorter than a page; the driver prints
     all eight to PDF and counts pages to hold that. */
  function printHomeSheet(o, items) {
    ...
    sheet.innerHTML =
      '<header class="ps-head"><p class="ps-eyebrow">Ehel Academy &middot; ' + esc(o.gradeLabel || "") + " Art &amp; Design &middot; Lesson " + esc(o.lessonNo || "") + "</p>" +
      "<h1>Make it at home: " + esc(o.lesson || "") + "</h1>" +
      '<p class="ps-tip">Make one with a grown-up. Cover the table first and roll up your sleeves. Messy is fine.</p>' +
      '<p class="ps-name">Name: <span></span></p></header>' +
      items.map((it) => '<section class="ps-proj"><h2>' + esc(it.n) + ". " + esc(it.title) + "</h2>" +
        '<p><b>You need:</b> ' + esc(it.materials) + "</p>" +
        "<ol>" + (it.steps || []).map((t) => "<li>" + esc(t) + "</li>").join("") + "</ol>" +
        '<p><b>Look for:</b> ' + esc(it.look) + "</p>" +
        '<div class="ps-box"><span>Stick it or draw it here</span></div>' +
        '<p class="ps-done">&#9744; We made this &nbsp;&nbsp; Date: <span></span></p></section>').join("");
    document.body.classList.add("printing-home");
    const off = () => { document.body.classList.remove("printing-home"); window.removeEventListener("afterprint", off); };
    window.addEventListener("afterprint", off);
    try { window.print(); } catch (_) { off(); }
  }
  ...
  function homeProjects(o) {
    ...
    function paint() {
      ...
      $(el.stage).innerHTML = '<div class="acts" id="' + id + '">' + items.map((it, k) =>
        ...
        "</div>").join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small teal" id="' + id + 'print">&#128424; Print it for home</button>' +
        '<button type="button" class="big small" id="' + id + 'fin">We made these &#10003;</button></div>';
      ...
      $(id + "print").addEventListener("click", () => printHomeSheet(o, items));
    }
    $(el.stage).addEventListener("click", (e) => {
  ...
  /* ---- the starting check (validation area 11, 2026-09-11) ----------------
     The readiness check every other subject opens with, in its own file
     shape (art-and-design/data/placement/grade-N.json - the shape
     shell/placement.js reads), drawn by this kit because this subject has no
     shell course to hang it on. Built into its own page, starting-check.html,
     linked from the hub, and deliberately NOT a lesson: it is not in the
     lesson list, so it earns no unit and reports nothing to the school's
     gradebook. The result stays on this device (the hub shows it), which is
     where the shell keeps a placement too until a teacher asks for it.
     band() is shell/placement.js's own rule, line for line, so a score means
     the same thing here as in Science. */
  function readinessCheck(o) {
    const el = o.el, exam = o.exam, lessons = o.lessons || {};
    const qs = exam.questions.slice().sort((a, b) => a.sequence - b.sequence);
    const split = (s) => String(s || "").split("|").map((x) => x.trim()).filter(Boolean);
    const q = new URLSearchParams(location.search);
    const KEY = "ehel-art-starting-check:" + (o.course || "") + ":" + (q.get("studentid") || "local");
    let i = 0, answers = {}, lock = false;
    const link = (file) => {
    ...
    function picOf(p) {
      p = String(p || "").trim();
      if (p.startsWith("mark:")) return markPreview(p.slice(5), "crayon");
      if (p.startsWith("swatch:")) return swatchSvg(p.slice(7), 64);
      return esc(p);
    }
    ...
    function results() {
      ...
      const sectionScores = exam.sections.map((s) => {
        const mine = qs.filter((x) => x.sectionId === s.sectionId);
        const got = mine.filter((x) => answers[x.questionId] === x.correctAnswer).reduce((n, x) => n + (x.marks || 1), 0);
        const of = mine.reduce((n, x) => n + (x.marks || 1), 0);
        score += got; total += of;
        return { id: s.sectionId, label: s.title, score: got, total: of, percent: of ? Math.round(got / of * 100) : 0, remediation: s.remediation || [] };
      });
      const percent = total ? Math.round(score / total * 100) : 0;
      return { score, total, percent, sectionScores, band: band(percent, sectionScores) };
    }
    function stored() { try { return JSON.parse(localStorage.getItem(KEY) || "null"); } catch (_) { return null; } }
    function intro() {
      const last = stored();
      $(el.ask).innerHTML = "Before Lesson 1: a short <b>starting check</b>. Tap the answer you think is right. It is never a fail.";
      $(el.stage).className = "stagewide";
      $(el.stage).innerHTML = '<div class="rc-intro"><span class="cpic" aria-hidden="true">\u{1F3A8}</span>' +
        "<h3>" + esc(exam.shortTitle || exam.title) + "</h3><p class=\"sub\">" + esc(exam.description) + "</p>" +
        '<p class="rc-bits"><span>' + qs.length + " questions</span><span>about " + esc(exam.estimatedMinutes) + " minutes</span><span>never a fail</span></p>" +
        (last && exam.banding[last.band] ? '<p class="rc-last">Last time: <b>' + esc(exam.banding[last.band].label) + "</b> (" + esc(last.percent) + "%)</p>" : "") +
        '<div class="bigbtns"><button type="button" class="big" id="' + el.stage + 'go">Start &#9654;</button></div></div>';
      $(el.ch).innerHTML = ""; $(el.fb).textContent = ""; $(el.score).textContent = "";
      $(el.stage + "go").addEventListener("click", () => { i = 0; answers = {}; ask(); });
    }
    function ask() {
      lock = false;
      const it = qs[i], sec = exam.sections.find((s) => s.sectionId === it.sectionId) || {};
      const opts = split(it.options), pics = split(it.optionPics);
      $(el.ask).innerHTML = '<span class="qbook">' + esc(sec.title || "") + "</span>" + esc(it.question);
      $(el.stage).className = "stagewide";
      $(el.stage).innerHTML = it.pic ? '<div class="askpic" aria-hidden="true">' + esc(it.pic) + "</div>" : "";
      $(el.ch).className = "choices rc-opts";
      $(el.ch).innerHTML = shuffle(opts.map((t, k) => ({ t, p: pics[k] }))).map((x) =>
        '<button type="button" class="rc-opt" data-t="' + esc(x.t) + '">' + (x.p ? '<span class="rc-pic" aria-hidden="true">' + picOf(x.p) + "</span>" : "") +
        '<span class="rc-t">' + esc(x.t) + "</span></button>").join("");
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      $(el.score).textContent = "Question " + (i + 1) + " of " + qs.length;
      sayHere(o.finish, it.question);
    }
    $(el.ch).addEventListener("click", (e) => {
      const b = e.target.closest(".rc-opt"); if (!b || lock) return;
      lock = true;
      const it = qs[i], ok = b.dataset.t === it.correctAnswer;
      answers[it.questionId] = b.dataset.t;
      $(el.ch).querySelectorAll(".rc-opt").forEach((c) => { c.disabled = true; if (c.dataset.t === it.correctAnswer) c.classList.add("right"); });
      if (!ok) b.classList.add("wrong");
      SOUND.play(ok ? "tada" : "pop", ok ? 0.35 : 0.2);
      $(el.fb).className = "fb " + (ok ? "good" : "bad");
    ...
    function report() {
      const r = results();
      const info = exam.banding[r.band] || {};
      const minSec = (exam.banding.ready || {}).minSectionPercent ?? 60;
      const weak = r.sectionScores.filter((s) => s.percent < minSec);
      const review = weak.length ? weak : (r.band === "ready" ? [] : r.sectionScores.filter((s) => s.percent < ((exam.banding.ready || {}).minOverallPercent ?? 80)));
      const first = lessons[1];
      try { localStorage.setItem(KEY, JSON.stringify({ band: r.band, percent: r.percent, score: r.score, total: r.total, sections: r.sectionScores.map((s) => ({ id: s.id, percent: s.percent })), at: new Date().toISOString() })); } catch (_) { /* a private window: the report still shows */ }
      $(el.ask).innerHTML = "Here is <b>where you start</b>.";
      $(el.ch).innerHTML = ""; $(el.ch).className = "choices";
      $(el.score).textContent = "";
      $(el.stage).innerHTML = '<div class="rc-report">' +
        '<p class="rc-band ' + esc(r.band) + '">' + esc(info.label || "") + "</p>" +
        '<p class="lead">' + esc(info.message || "") + "</p>" +
        '<p class="sub">You got ' + r.score + " of " + r.total + ".</p>" +
        '<div class="rc-secs">' + r.sectionScores.map((s) => '<div class="rc-sec"><span class="rc-sname">' + esc(s.label) + "</span>" +
          '<span class="rc-bar" aria-hidden="true"><i style="width:' + s.percent + '%"></i></span><span class="rc-pct">' + s.score + " of " + s.total + "</span></div>").join("") + "</div>" +
        (review.length ? '<h3 class="rc-h">Do these with a grown-up close by</h3><ul class="rc-rev">' +
          review.flatMap((s) => s.remediation).filter((m, k, a) => a.findIndex((x) => x.unit === m.unit) === k)
            .map((m) => { const l = lessons["g" + m.grade + "u" + m.unit] || lessons[m.unit]; return l ? '<li><a href="' + esc(link(l.file)) + '">' + (l.grade ? "Grade " + l.grade + ", " : "") + "Lesson " + m.unit + ": " + esc(l.title) + "</a></li>" : ""; }).join("") + "</ul>" : "") +
        '<div class="bigbtns">' + (first ? '<a class="big" href="' + esc(link(first.file)) + '">Start Lesson 1 &#9654;</a>' : "") +
        '<button type="button" class="big small ghost" id="' + el.stage + 'again">Try again</button></div></div>';
      $(el.stage + "again").addEventListener("click", intro);
      $(el.fb).className = "fb good"; $(el.fb).textContent = "";
      say((info.label || "") + ". " + (info.message || ""));
      finish(o.finish, "");
    }
    intro();
  }
