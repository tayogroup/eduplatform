<?php
/**
 * Repoint Grade 1 English from the shared subject entry to the standalone
 * grade-1-v2 build.
 *
 * Changes exactly one Moodle config value:
 *   local_prequran / ehel_app_url_overrides
 * which pqpg_ehel_app_base() reads to send a single course somewhere other
 * than its subject entry (app/english/index.html, which serves all eight
 * English grades). Nothing is deployed and no file is edited - grade-1-v2
 * is already live on Bunny storage before this script is ever run.
 *
 * UNLIKE the Mathematics grade-1-app version of this script, English's
 * override does not exist yet: Grade 1 English has always used the shared
 * entry, so the "before" state is (not set), not an earlier override this
 * one supersedes. The rollback at the end is therefore "clear the entry",
 * not "point it at a different URL".
 *
 * REPORTS BY DEFAULT. Pass --apply to write. Run it once with no argument
 * first and read what it says the before and after will be.
 *
 ...
 * The require below is __DIR__-relative, so a home-directory run fails
...
const RP_COURSE   = 'ehel-eng-g01';
const RP_EXPECTDB = 'ehelacad_quraantest';
const RP_TO       = 'https://ehelacademy.b-cdn.net/Ehel%20Primary/app/english/grade-1-v2/index.html';
 ...
 * uniso.site and quraantest.academy. So it excludes the other eight Moodles
 * on the box, which is its job, but it does not narrow within this one -
 * hence fence 2. One installation means one config_plugins table, so there
 * is exactly one setting to write either way. */
 ...
 * (ehel-primary -> ehel-k12). If neither is found the script REFUSES but
 * prints the slugs that do exist, so a false negative comes back as a
...
if ($before !== '(not set)') {
    rp_say("");
    rp_say("NOTE: " . RP_COURSE . " already has an override, and it is not the shared");
    rp_say("      entry's absence this script expects. Read the line above before");
    rp_say("      applying - someone has pointed this course somewhere else already.");
}
...
unset($map[RP_COURSE]);
rp_say("  " . (empty($map) ? "(clear the box entirely)" : json_encode($map, JSON_UNESCAPED_SLASHES)));
rp_say("to send Grade 1 English back to the shared subject entry (app/english/index.html),");
rp_say("which is what it used before this script ran.");
rp_say("");
