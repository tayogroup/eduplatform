...
const unitDir = path.join(root, "src", "prototypes", "ehel-academy", "english", "grade-2", "unit-1");
const course = JSON.parse(fs.readFileSync(path.join(unitDir, "data", "grade2-unit1.json"), "utf8"));
const dictionary = JSON.parse(fs.readFileSync(path.join(unitDir, "data", "master-dictionary.unit1.json"), "utf8"));
...
if (/speechSynthesis|SpeechSynthesisUtterance/.test(source)) throw new Error("Browser-generated voice remains in the Unit 1 runtime.");
...
const requiredAnswerIds = [
  ...course.grammar.filter((item) => /grammar0[456]$/.test(item.grammarId)).map((item) => item.grammarId),
  ...course.speaking.slice(3).map((item) => item.speakingId),
  ...course.writing.slice(3).map((item) => item.writingId),
  ...course.activities.slice(3).map((item) => item.activityId),
...
const workbook = path.join(root, "outputs", "019f5d39-7fcd-7f23-a425-201fe8206eef", "Ehel-English-Content-Template-v1.1-Grade-2-Unit-1-Reference.xlsx");
if (!fs.existsSync(workbook) || fs.statSync(workbook).size < 10000) throw new Error("Template v1.1 workbook is missing.");
