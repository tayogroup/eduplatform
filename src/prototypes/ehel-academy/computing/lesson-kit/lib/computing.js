  ...
  /* white words on a dark pill, for a scene that says what went wrong. LABEL on
     its own is white on whatever the scene drew underneath, which on a kitchen
     floor is beige. The Stage 3 scenes use this; the older ones are unchanged. */
  const NOTE = (y, t) => '<rect x="14" y="' + (y - 17) + '" width="292" height="24" rx="12" fill="#0B1D2C" opacity="0.82"/>' + LABEL(160, y, t);
  const SCENES = {
    ...
    /* ---- Stage 3's own tasks. Until the Grade 3 validation (2026-09-11)
       Grade 3 followed, debugged, trimmed and changed the same routines Grades
       1 and 2 had taught - handwashing, a seed, tea, getting dressed, the
       sandwich, bedtime. These four are its own, drawn the same way: from the
       ids done so far, in order, so a wrong order draws its own consequence. */

    /* a smoothie: fruit, milk, the lid, blend, pour. Blending before the lid
       is on sprays it round the kitchen; pouring before blending puts lumps of
       fruit in the glass. "ice" and "mango" are the what-if extras. */
    smoothie: (ids) => {
      ids = ids || [];
      const has = (id) => ids.includes(id), at = (id) => ids.indexOf(id);
      const blended = has("blend"), splat = blended && !(has("lid") && at("lid") < at("blend"));
      const poured = has("pour"), lumpy = poured && !(blended && at("blend") < at("pour"));
      const colour = has("berries") ? (has("mango") ? "#F59A7A" : "#F2A5B5") : has("mango") ? "#F7B35C" : has("banana") ? "#F5E6A8" : "#F4F8FB";
      const d = (y) => (8 * (y - 58)) / 102;
      const fillTo = (top, fill, op) => '<path d="M' + (94 + d(top)).toFixed(1) + " " + top + " H" + (166 - d(top)).toFixed(1) + ' L158 156 H102 Z" fill="' + fill + '"' + (op ? ' opacity="' + op + '"' : "") + "/>";
      let g = '<rect width="320" height="240" fill="#F4EAD4"/><rect x="0" y="186" width="320" height="54" fill="#C9B79C"/>';
      if (splat) g += '<g fill="' + colour + '" stroke="#C2667A" stroke-width="1.5"><circle cx="52" cy="46" r="12"/><circle cx="108" cy="22" r="8"/><circle cx="200" cy="30" r="14"/><circle cx="262" cy="62" r="9"/><circle cx="30" cy="112" r="7"/><circle cx="190" cy="96" r="6"/><ellipse cx="232" cy="204" rx="26" ry="7"/><ellipse cx="62" cy="206" rx="18" ry="5"/></g>';
      g += '<rect x="86" y="160" width="88" height="28" rx="8" fill="#4A5A6A"/><circle cx="130" cy="174" r="6" fill="' + (blended && !poured ? "#4FD1A0" : "#93AABE") + '"/>';
      g += '<path d="M90 58 h80 l-8 102 h-64z" fill="#EAF4FA" stroke="#93AABE" stroke-width="4"/>';
      if (!poured) {
        if (blended) {
          g += fillTo(splat ? 126 : has("milk") ? 88 : 122, colour);
          if (has("ice")) g += '<g fill="#fff" opacity="0.9"><circle cx="116" cy="112" r="2.5"/><circle cx="140" cy="104" r="2.5"/><circle cx="130" cy="130" r="2.5"/><circle cx="150" cy="120" r="2.5"/></g>' + T(188, 60, 22, "✨");
          if (!splat) g += '<g stroke="#93AABE" stroke-width="3" fill="none" stroke-linecap="round"><path d="M76 96 q-8 12 0 24"/><path d="M184 96 q8 12 0 24"/></g>';
        } else {
          if (has("milk")) g += fillTo(90, "#FFFDF6", 0.95) + '<line x1="' + (94 + d(90)).toFixed(1) + '" y1="90" x2="' + (166 - d(90)).toFixed(1) + '" y2="90" stroke="#D6E3DE" stroke-width="2"/>';
          if (has("banana")) g += '<g fill="#F7E9A0" stroke="#E0C95A" stroke-width="2"><ellipse cx="112" cy="146" rx="10" ry="6"/><ellipse cx="132" cy="148" rx="10" ry="6"/><ellipse cx="150" cy="145" rx="9" ry="6"/></g>';
          if (has("mango")) g += '<g fill="#F4A23B" stroke="#D9822B" stroke-width="2"><rect x="104" y="138" width="14" height="12" rx="3"/><rect x="124" y="140" width="14" height="12" rx="3"/><rect x="143" y="136" width="13" height="12" rx="3"/></g>';
          if (has("berries")) g += '<g fill="#D33A4C"><circle cx="118" cy="128" r="8"/><circle cx="138" cy="126" r="8"/><circle cx="154" cy="131" r="7"/></g><g fill="#3E8E4A"><rect x="116" y="118" width="4" height="4"/><rect x="136" y="116" width="4" height="4"/><rect x="152" y="122" width="4" height="4"/></g>';
          if (has("ice")) g += '<g fill="#E6F7FF" stroke="#9CC8E0" stroke-width="2"><rect x="108" y="106" width="14" height="14" rx="3"/><rect x="136" y="100" width="14" height="14" rx="3"/></g>';
        }
        if (has("lid")) g += '<rect x="84" y="44" width="92" height="16" rx="6" fill="#4A5A6A"/><rect x="122" y="36" width="16" height="10" rx="3" fill="#4A5A6A"/>';
      }
      g += '<path d="M212 104 h48 l-6 80 h-36z" fill="#FFFFFF" stroke="#93AABE" stroke-width="4" opacity="0.95"/>';
      if (poured && !lumpy) g += '<path d="M216 116 h40 l-5 64 h-30z" fill="' + colour + '"/><line x1="248" y1="84" x2="240" y2="150" stroke="#E9744F" stroke-width="5" stroke-linecap="round"/>';
      if (lumpy) g += '<path d="M216 128 h40 l-4 52 h-32z" fill="#FFFFFF" opacity="0.7"/><g fill="#F7E9A0" stroke="#E0C95A" stroke-width="1.5"><circle cx="228" cy="164" r="6"/><circle cx="244" cy="170" r="6"/></g><g fill="#D33A4C"><circle cx="236" cy="150" r="5"/><circle cx="246" cy="140" r="5"/></g>';
      const name = { banana: "banana", berries: "strawberries", milk: "milk", lid: "lid on", blend: "blend", pour: "pour", ice: "ice", mango: "mango" };
      return '<svg viewBox="0 0 320 240" role="img" aria-label="Making a smoothie: ' + (ids.map((i) => name[i] || i).join(", ") || "nothing yet") + (splat ? ", blended with no lid on" : "") + (lumpy ? ", poured before it was blended" : "") + '">' + g +
        (splat ? NOTE(228, "no lid on: smoothie everywhere!") : lumpy ? NOTE(228, "lumps of fruit in the glass!") : "") + "</svg>";
    },
    /* a kite: cross the sticks, tie them, paper, a tail, the string, then run
       into the wind. Running before the string is tied lets the wind take it.
       "face" is the what-if extra. */
    kite: (ids) => {
      ids = ids || [];
      const has = (id) => ids.includes(id), at = (id) => ids.indexOf(id);
      const flying = has("fly") && has("string") && at("string") < at("fly");
      const gone = has("fly") && !flying;
      let g = '<rect width="320" height="240" fill="#BFE3F5"/><rect x="0" y="200" width="320" height="40" fill="#3E8E4A"/>';
      if (has("fly")) g += '<g fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round" opacity="0.8"><path d="M12 40 q30 -10 60 0"/><path d="M96 24 q30 -10 60 0"/><path d="M20 96 q24 -8 48 0"/></g>';
      g += '<circle cx="46" cy="148" r="14" fill="#C68642"/><rect x="34" y="162" width="24" height="38" rx="8" fill="#35BFB2"/><line x1="54" y1="172" x2="62" y2="166" stroke="#C68642" stroke-width="6" stroke-linecap="round"/>';
      const cx = flying ? 214 : gone ? 290 : 170, cy = flying ? 64 : gone ? 30 : 128, sc = gone ? 0.45 : 1;
      let k = "";
      if (has("paper")) k += '<path d="M0 -46 L34 -8 L0 46 L-34 -8Z" fill="#E9744F" stroke="#C2482C" stroke-width="3"/>';
      if (has("face") && has("paper")) k += '<circle cx="-10" cy="-14" r="4" fill="#0B1D2C"/><circle cx="10" cy="-14" r="4" fill="#0B1D2C"/><path d="M-12 4 q12 12 24 0" fill="none" stroke="#0B1D2C" stroke-width="3" stroke-linecap="round"/>';
      if (has("sticks")) k += '<g stroke="#8B5A2B" stroke-width="' + (has("paper") ? 3 : 5) + '" stroke-linecap="round" opacity="' + (has("paper") ? 0.55 : 1) + '"><line x1="0" y1="-46" x2="0" y2="46"/><line x1="-34" y1="-8" x2="34" y2="-8"/></g>';
      if (has("tie")) k += '<circle cx="0" cy="-8" r="5" fill="#F4C95D" stroke="#8B5A2B" stroke-width="2"/>';
      if (has("tail")) k += '<path d="M0 46 q14 16 0 30 q-14 16 0 30" fill="none" stroke="#6E9DE8" stroke-width="3"/><g fill="#F4C95D"><path d="M-8 64 l8 -4 l8 4 l-8 4z"/><path d="M-8 92 l8 -4 l8 4 l-8 4z"/></g>';
      if (k) g += '<g transform="translate(' + cx + " " + cy + ") scale(" + sc + ')">' + k + "</g>";
      if (has("string") && !gone) g += '<path d="M62 166 Q' + (flying ? "150 160 " : "115 196 ") + cx + " " + (cy - 8) + '" fill="none" stroke="#fff" stroke-width="2"/>';
      return '<svg viewBox="0 0 320 240" role="img" aria-label="Making a kite: ' + (ids.join(", ") || "nothing yet") + (gone ? ", and the wind took it" : flying ? ", flying" : "") + '">' + g +
        (gone ? NOTE(226, "no string yet: the wind took the kite!") : "") + "</svg>";
    },
    /* a cake: bowl, flour and sugar, eggs, mix, the tin, the oven, then cool */
    cake: (ids) => {
      ids = ids || [];
      const has = (id) => ids.includes(id);
      const inTin = has("tin"), baked = inTin && has("oven"), cooled = baked && has("cool");
      let g = '<rect width="320" height="240" fill="#F4EAD4"/><rect x="0" y="180" width="320" height="60" fill="#C9B79C"/>';
      g += '<rect x="214" y="70" width="92" height="110" rx="8" fill="#4A5A6A"/><rect x="226" y="96" width="68" height="50" rx="6" fill="' + (baked && !cooled ? "#F9B35C" : "#1B2A36") + '"/><g fill="#93AABE"><circle cx="234" cy="82" r="4"/><circle cx="250" cy="82" r="4"/><circle cx="266" cy="82" r="4"/></g>';
      if (baked && !cooled) g += '<rect x="238" y="126" width="44" height="14" rx="3" fill="#93AABE"/><path d="M240 126 q20 -18 40 0z" fill="#D9A15A"/>';
      if (has("bowl")) {
        g += '<path d="M24 132 h100 q0 46 -50 46 q-50 0 -50 -46z" fill="#6E9DE8" stroke="#3B6FB8" stroke-width="3"/>';
        if (!inTin && has("mix")) g += '<ellipse cx="74" cy="136" rx="46" ry="8" fill="#F3D9A4"/><line x1="98" y1="94" x2="78" y2="136" stroke="#93AABE" stroke-width="6" stroke-linecap="round"/>';
        else if (!inTin) {
          if (has("flour")) g += '<ellipse cx="74" cy="134" rx="40" ry="9" fill="#FFFDF6"/><path d="M50 134 q24 -26 48 0z" fill="#FFFDF6"/>';
          if (has("eggs")) g += '<ellipse cx="64" cy="126" rx="9" ry="6" fill="#fff"/><circle cx="64" cy="126" r="4" fill="#F4C95D"/><ellipse cx="86" cy="128" rx="9" ry="6" fill="#fff"/><circle cx="86" cy="128" r="4" fill="#F4C95D"/>';
        }
      } else if (has("flour") || has("eggs")) g += T(74, 170, 34, has("eggs") ? "\u{1F95A}" : "\u{1F33E}");
      if (inTin && !baked) g += '<rect x="136" y="158" width="56" height="22" rx="4" fill="#93AABE"/><rect x="139" y="158" width="50" height="8" fill="#F3D9A4"/>';
      if (cooled) g += '<rect x="130" y="178" width="80" height="6" rx="3" fill="#93AABE"/><path d="M138 154 v20 a32 7 0 0 0 64 0 v-20z" fill="#C98B45" stroke="#A9552B" stroke-width="3"/><ellipse cx="170" cy="154" rx="32" ry="7" fill="#E0B070" stroke="#A9552B" stroke-width="3"/>' + T(170, 138, 20, "✨");
      return '<svg viewBox="0 0 320 240" role="img" aria-label="Baking a cake: ' + (ids.join(", ") || "nothing yet") + '">' + g + "</svg>";
    },
    /* a present: the teddy in the box, the lid, paper, tape, ribbon, a tag */
    present: (ids) => {
      ids = ids || [];
      const has = (id) => ids.includes(id);
      const wrapped = has("paper");
      const fill = wrapped ? "#6E9DE8" : "#C9A06A", edge = wrapped ? "#3B6FB8" : "#A67C44";
      let g = '<rect width="320" height="240" fill="#EDE3F5"/><rect x="0" y="182" width="320" height="58" fill="#C9B79C"/>';
      if (!has("box")) return '<svg viewBox="0 0 320 240" role="img" aria-label="Wrapping a present: nothing yet, the teddy is waiting">' + g + T(160, 176, 54, "\u{1F9F8}") + "</svg>";
      if (!has("lid")) g += T(160, 126, 46, "\u{1F9F8}");
      g += '<rect x="100" y="106" width="120" height="76" rx="6" fill="' + fill + '" stroke="' + edge + '" stroke-width="3"/>';
      if (has("lid")) g += '<rect x="94" y="96" width="132" height="18" rx="4" fill="' + fill + '" stroke="' + edge + '" stroke-width="3"/>';
      if (wrapped) g += '<g fill="#fff" opacity="0.85"><circle cx="118" cy="126" r="5"/><circle cx="146" cy="160" r="5"/><circle cx="184" cy="124" r="5"/><circle cx="204" cy="164" r="5"/><circle cx="126" cy="104" r="4"/><circle cx="196" cy="104" r="4"/></g>';
      if (has("tape")) g += '<g fill="#FFF8D0" opacity="0.9" stroke="#E0D6A0" stroke-width="1"><rect x="104" y="140" width="18" height="10" rx="2"/><rect x="198" y="150" width="18" height="10" rx="2"/><rect x="140" y="98" width="18" height="10" rx="2"/></g>';
      if (has("ribbon")) g += '<rect x="154" y="96" width="12" height="86" fill="#E9744F"/><rect x="94" y="136" width="132" height="12" fill="#E9744F"/><path d="M160 94 q-30 -26 -34 -4 q4 16 34 4z M160 94 q30 -26 34 -4 q-4 16 -34 4z" fill="#F0806F" stroke="#C2482C" stroke-width="2"/><circle cx="160" cy="94" r="6" fill="#C2482C"/>';
      if (has("tag")) g += '<line x1="226" y1="140" x2="236" y2="150" stroke="#93AABE" stroke-width="2"/><path d="M232 146 l30 8 l-6 22 l-30 -8z" fill="#FFFDF6" stroke="#93AABE" stroke-width="2"/><text x="244" y="168" text-anchor="middle" font-size="11" font-family="Inter, sans-serif" font-weight="700" fill="#0B1D2C" transform="rotate(15 244 164)">Nora</text>';
      return '<svg viewBox="0 0 320 240" role="img" aria-label="Wrapping a present: ' + ids.join(", ") + '">' + g + "</svg>";
    },
  ...
  function blockBtn(id, attr, extra) {
    const b = BLOCKS[id] || { label: id, icon: "?", cat: "control" };
    return '<button type="button" class="block ' + b.cat + (extra ? " " + extra : "") + '" ' + (attr || "") + '><span class="bicon" aria-hidden="true">' + b.icon + "</span>" + esc(b.label) + "</button>";
  }
  function spriteStage(box, sprites, opts) {
    ...
    /* Stage 3: a marker on the ground at square x (the flower the cat must reach) */
    function mark(x, glyph) {
      const m = document.createElement("div");
      m.className = "mark"; m.setAttribute("aria-hidden", "true"); m.textContent = glyph;
      m.style.left = "calc(50% - 32px + " + (x * 64) + "px)";
      box.appendChild(m);
    }
    ...
    /* reset returns every sprite to its START state - the origin unless the
       round says the sprite was left somewhere else (3P.02: the point of a
       go-home block at the top of a program) */
    function reset() { st.forEach((s, k) => { s.x = start.x || 0; s.scale = start.scale || 1; s.spin = start.spin || 0; s.hidden = false; bb(k).hidden = true; paint(k, false); }); }
    ...
    reset();
    return { run, reset, active, mark, state: st };
  }
  ...
  function robotGrid(o) {
    ...
    /* Stage 4: a looped program is shown as its loop, with the turn counter */
    function loopChips(k) {
      const lv = level(), nb = (lv.before || []).length, nbody = lv.loop.body.length, nt = Number(lv.loop.times);
      const chip = (cc, on) => '<span class="chip' + (on ? " now" : "") + '">' + CMD[cc].icon + " " + CMD[cc].label + "</span>";
      const inBody = k >= nb && k < nb + nbody * nt, turn = inBody ? Math.floor((k - nb) / nbody) + 1 : 0;
      return '<div class="prog" id="' + el.stage + 'prog">' + (lv.before || []).map((cc, j) => chip(cc, j === k)).join("") +
        '<span class="loopchip' + (inBody ? " now" : "") + '"><b>repeat ' + nt + " times" + (inBody ? " · " + turn + " of " + nt : "") + ":</b>" + lv.loop.body.map((cc, j) => chip(cc, inBody && (k - nb) % nbody === j)).join("") + "</span>" +
        (lv.after || []).map((cc, j) => chip(cc, k === nb + nbody * nt + j)).join("") + "</div>";
    }
    function chips(k) {
    ...
    function draw(k) {
      ...
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">' + esc(lv.title) + "</p>" +
        '<div class="sim robobox" id="' + el.stage + 'g">' + gridSvg(st.pos, st.facing, predicted) + "</div>" + (level().predict && level().loop ? loopChips(k) : chips(k)) + controls() + "</div>";
    ...
    function startLevel() {
      const lv = level();
      reset(); program = lv.predict ? (lv.loop ? expandLoop(lv.before, lv.loop.body, lv.loop.times, lv.after) : lv.program.slice()) : []; firstGo = true; predicted = null; running = false;
  ...
  const scriptHtml = (ids, now, tappable) => '<div class="script">' + (ids.length ? ids.map((id, k) =>
    tappable ? blockBtn(id, 'data-k="' + k + '"', k === now ? "now" : "") : blockHtml(id, k === now ? " now" : "")).join("")
  ...
  function blockProgram(o) {
    ...
    function draw(now) {
      ...
      sprite = spriteStage($(el.stage + "sp"), sprites, { start: rd.start });
    ...
    function start() {
      ...
      if (rd.given) {
        ...
        $(el.ask).innerHTML = lead + "<b>" + esc(rd.algorithm.join(", ")) + "</b>." + (rd.mustRepeat ? " Use a <b>repeat</b> block." : "") + (rd.mustReset ? " The " + esc(who()) + " is not at the start: put a <b>go home</b> block first." : "") + " Then press Run.";
        sayHere(o.finish, lead + rd.algorithm.join(". ") + "." + (rd.mustRepeat ? " Use a repeat block." : "") + (rd.mustReset ? " The " + who() + " is not at the start. Put a go home block first." : "") + " Then press Run.");
      }
    }
    function runIt() {
  ...
  /* ---- FIGURES: hardware with parts to tap (2CS.01). Every part is a
     ...
     EVERY RING IS AT LEAST 24 px AT PHONE WIDTH (WCAG 2.5.8), measured: the
     laptop's speaker and port and the tablet's volume, port and speaker
     measured 17 to 23 px on a 375 px screen (Grade 2 validation,
     2026-09-11). Their rings grew into empty space - the viewBoxes are a
     little taller to make room below - and no ring now reaches another. */
  const FIGURES = {
    laptop: () => (
      '<svg viewBox="0 0 360 268" role="img" aria-label="A laptop, open">' +
      ...
      '<g data-part="camera" tabindex="0" role="button" aria-label="camera"><circle cx="180" cy="26" r="4" fill="#0B1D2C" stroke="#93AABE" stroke-width="2"/><circle class="outline" cx="180" cy="26" r="20"/></g>' +
      ...
      '<g data-part="speaker" tabindex="0" role="button" aria-label="speaker"><g fill="#0B1D2C">' + Array.from({ length: 6 }, (_, i) => '<circle cx="' + (300 + (i % 3) * 6) + '" cy="' + (246 + Math.floor(i / 3) * 6) + '" r="2"/>').join("") + '</g><rect class="outline" x="286" y="230" width="42" height="34" rx="8"/></g>' +
      '<g data-part="port" tabindex="0" role="button" aria-label="charging port"><rect x="30" y="240" width="22" height="10" rx="3" fill="#0B1D2C"/><rect class="outline" x="18" y="230" width="46" height="34" rx="8"/></g>' +
      "</svg>"),
    tablet: () => (
      '<svg viewBox="0 0 260 378" role="img" aria-label="A tablet">' +
      ...
      '<g data-part="volume" tabindex="0" role="button" aria-label="volume buttons"><rect x="12" y="90" width="8" height="26" rx="3" fill="#93AABE"/><rect x="12" y="124" width="8" height="26" rx="3" fill="#93AABE"/><rect class="outline" x="0" y="84" width="28" height="72" rx="6"/></g>' +
      '<g data-part="port" tabindex="0" role="button" aria-label="charging port"><rect x="118" y="352" width="24" height="6" rx="2" fill="#0B1D2C"/><rect class="outline" x="108" y="350" width="44" height="26" rx="7"/></g>' +
      '<g data-part="speaker" tabindex="0" role="button" aria-label="speaker"><g fill="#0B1D2C">' + Array.from({ length: 5 }, (_, i) => '<circle cx="' + (62 + i * 8) + '" cy="336" r="2"/>').join("") + '</g><rect class="outline" x="50" y="320" width="56" height="30" rx="7"/></g>' +
      "</svg>"),
  };
  ...
  /* ==================================================================
     STAGE 3 - concise algorithms, the steps that repeat, what a change
     does, machines with an input and an output, tidy programs, objects
     that run at the same time, numbers inside blocks, a physical device,
     three views of one table, a spreadsheet, a filter, a cipher.

     The rule is unchanged: the child drives a machine that only does what
     it is told, and every key the kit can compute it computes. The
     functions in the first block are mirrored, name for name, in
     _rules.py, so the builder and the gate check the same arithmetic the
     page runs.
     ================================================================== */

  /* rules mirrored in _rules.py */
  function ruleOutput(rule, x) {
    const parts = String(rule).split(":"), name = parts[0], k = Number(parts[1] || 0);
    if (name === "letters") return String(String(x).length);
    const n = Number(x);
    if (!Number.isFinite(n)) return null;
    if (name === "double") return String(n * 2);
    if (name === "half") return n % 2 === 0 ? String(n / 2) : null;
    if (name === "add") return String(n + k);
    if (name === "minus") return String(n - k);
    if (name === "times") return String(n * k);
    return null;
  }
  function walkEnd(program) {
    let x = 0;
    program.forEach((b) => { if (b.id === "right") x += Number(b.n); else if (b.id === "left") x -= Number(b.n); });
    return x;
  }
  const codeWord = (word) => String(word).toLowerCase().split("").filter((c) => c >= "a" && c <= "z").map((c) => c.charCodeAt(0) - 96);
  const decodeCode = (nums) => nums.map((n) => String.fromCharCode(96 + Number(n))).join("");
  function rowMatches(row, spec) {
    const v = row[spec.field], op = spec.op || "eq";
    if (op === "gt") return Number(v) > Number(spec.value);
    if (op === "lt") return Number(v) < Number(spec.value);
    if (op === "ne") return String(v) !== String(spec.value);
    return String(v) === String(spec.value);
  }
  const sameEffect = (a, b) => {
    const ea = expandProgram(a).filter((x) => x !== "wait"), eb = expandProgram(b).filter((x) => x !== "wait");
    return ea.length === eb.length && ea.every((x, i) => x === eb[i]);
  };
  const ALPHA = "abcdefghijklmnopqrstuvwxyz".split("");

  /* paint a scene one step at a time, on a timer, into a box */
  function paintScene(box, scene, ids, ms, then) {
    let k = 0;
    box.hidden = false;
    const tick = () => {
      box.innerHTML = SCENES[scene](ids.slice(0, k));
      if (k < ids.length) { k++; SOUND.play("pop", 0.25); setTimeout(tick, ms || 520); }
      else if (then) then();
    };
    tick();
  }

  /* ---- an efficient algorithm is concise (3CT.02; 3CT.01 as editing) --
     The algorithm does its task, but with steps it does not need: a step
     done twice, a step that does nothing for this task. The child taps
     each wasteful step out. When only the needed steps are left the scene
     runs them, so the shorter algorithm is SEEN to do the same job. */
  function trimSteps(o) {
    const el = o.el, steps = o.steps, cut = new Set();
    const wasteful = steps.map((s, k) => (s.waste ? k : -1)).filter((k) => k >= 0);
    let wrong = 0, lock = false;
    function draw() {
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(o.task) + '</b>. ' + steps.length + ' steps.</p><ol class="algo tappable">' + steps.map((s, k) =>
        '<li><button type="button" class="algostep' + (cut.has(k) ? " cut" : "") + '" data-k="' + k + '"' + (cut.has(k) || lock ? " disabled" : "") + '><span class="apic" aria-hidden="true">' + small(s.pic) + "</span>" + esc(s.label) + "</button></li>").join("") +
        '</ol><div class="sim scenebox" id="' + el.stage + 'sc" hidden></div></div>';
      $(el.score).textContent = (steps.length - cut.size) + " steps left" + (cut.size ? " · " + cut.size + " cut" : "") + " · " + (wasteful.length - cut.size) + " more to find";
    }
    $(el.stage).addEventListener("click", (e) => {
      const b = e.target.closest(".algostep"); if (!b || lock) return;
      const k = Number(b.dataset.k), s = steps[k];
      if (s.waste) {
        cut.add(k); SOUND.play("click", 0.4);
        $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + s.waste; say(cheer() + " " + s.waste);
        draw();
        if (cut.size >= wasteful.length) {
          ...
          setTimeout(() => {
            ...
            paintScene($(el.stage + "sc"), o.scene, kept, 560, () => {
              SOUND.play("tada", 0.5);
              const line = steps.length + " steps became " + kept.length + ", and the task still got done. " + o.done;
              reportScore(o.finish, Math.max(0, wasteful.length - wrong), wasteful.length);
              endStep(o, line);
            });
          }, 2200);
        }
        ...
        wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
        const line = "That step is needed. " + (s.need || "Without it the task is not done.");
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        setTimeout(() => b.classList.remove("wrong"), 700);
      }
    });
  ...
  /* ---- the steps that repeat inside an everyday task (3CT.03) ----------
     A task written out long-hand: some run of steps happens again and
     again, back to back. The child taps every step of the FIRST time it
     happens; the page folds all the repeats into one "repeat N times"
     block, and then asks how many times. */
  function loopSpot(o) {
    const el = o.el, steps = o.steps, run = o.run, got = new Set();
    const first = new Set(Array.from({ length: run.length }, (_, j) => run.start + j));
    let wrong = 0, folded = false, lock = false;
    function draw() {
      let html;
      if (!folded) html = '<ol class="algo tappable">' + steps.map((s, k) =>
        '<li><button type="button" class="algostep' + (got.has(k) ? " lit" : "") + '" data-k="' + k + '"' + (got.has(k) || lock ? " disabled" : "") + '><span class="apic" aria-hidden="true">' + small(s.pic) + "</span>" + esc(s.label) + "</button></li>").join("") + "</ol>";
      else {
        const before = steps.slice(0, run.start), seg = steps.slice(run.start, run.start + run.length), after = steps.slice(run.start + run.length * run.times);
        html = '<ol class="algo">' + before.map((s) => '<li><span class="apic" aria-hidden="true">' + small(s.pic) + "</span>" + esc(s.label) + "</li>").join("") +
          '<li class="fold"><b>repeat ' + run.times + ' times:</b><ol class="algo inner">' + seg.map((s) => '<li><span class="apic" aria-hidden="true">' + small(s.pic) + "</span>" + esc(s.label) + "</li>").join("") + "</ol></li>" +
          after.map((s) => '<li><span class="apic" aria-hidden="true">' + small(s.pic) + "</span>" + esc(s.label) + "</li>").join("") + "</ol>";
      }
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(o.task) + '</b>. ' + (folded ? (before_len() + 1 + (steps.length - run.start - run.length * run.times)) + " steps written with a repeat." : steps.length + " steps written out long.") + "</p>" + html + "</div>";
      $(el.score).textContent = folded ? "" : got.size + " of " + run.length + " repeating steps tapped";
    }
    const before_len = () => run.start;
    $(el.stage).addEventListener("click", (e) => {
      const b = e.target.closest(".algostep"); if (!b || lock || folded) return;
      const k = Number(b.dataset.k);
      if (first.has(k)) {
        got.add(k); SOUND.play("click", 0.4); draw();
        ...
        if (got.size >= first.size) {
          ...
          setTimeout(() => {
            ...
            $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
            setTimeout(() => { $(el.fb).textContent = ""; askOnce(o, o.then, (ok) => { reportScore(o.finish, (ok ? 1 : 0) + Math.max(0, run.length - wrong), run.length + 1); endStep(o); }); }, 3600);
          }, 1800);
        }
      } else {
        wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
        const later = k >= run.start + run.length && k < run.start + run.length * run.times;
        const line = later ? "That is " + steps[k].label + " happening AGAIN. Tap the first time it happens." : steps[k].label + " happens once. Which steps happen again and again?";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        setTimeout(() => b.classList.remove("wrong"), 700);
      }
    });
  ...
  /* ---- predict what a change to an algorithm will do (3CT.05) ---------
     Each round proposes ONE change to the same algorithm - swap two
     steps, take one out, put one in, replace one - and asks what will
     happen. The child answers, THEN the change is applied and the scene
     paints the changed algorithm, so the prediction meets the outcome. */
  function applyChange(steps, ch) {
    const out = steps.slice();
    if (ch.kind === "swap") { const t = out[ch.a]; out[ch.a] = out[ch.b]; out[ch.b] = t; }
    else if (ch.kind === "remove") out.splice(ch.at, 1);
    else if (ch.kind === "insert") out.splice(ch.at, 0, ch.step);
    else if (ch.kind === "replace") out[ch.at] = ch.step;
    return out;
  }
  function changeWords(steps, ch) {
    const L = (k) => '"' + steps[k].label + '"';
    if (ch.kind === "swap") return "swap step " + (ch.a + 1) + " (" + L(ch.a) + ") with step " + (ch.b + 1) + " (" + L(ch.b) + ")";
    if (ch.kind === "remove") return "take out step " + (ch.at + 1) + " (" + L(ch.at) + ")";
    if (ch.kind === "insert") return 'put "' + ch.step.label + '" in as step ' + (ch.at + 1);
    return "change step " + (ch.at + 1) + " (" + L(ch.at) + ') to "' + ch.step.label + '"';
  }
  function whatIf(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, score = 0;
    function draw(steps, changed) {
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">' + (changed ? "The changed algorithm:" : "The algorithm:") + "</p><div class=\"algo2\">" + algoList(steps, -1) +
        '<div class="sim scenebox" id="' + el.stage + 'sc"' + (changed ? "" : " hidden") + "></div></div></div>";
      $(el.score).textContent = "Change " + (r + 1) + " of " + rounds.length;
    }
    function start() {
      const rd = rounds[r];
      draw(o.steps, false);
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      const spec = { ask: "What if we <b>" + esc(changeWords(o.steps, rd.change)) + "</b>? " + rd.ask, opts: rd.opts, why: rd.why };
      askOnce(o, spec, (ok) => {
        if (ok) score++;
        const changed = applyChange(o.steps, rd.change);
        draw(changed, true);
        $(el.ask).innerHTML = "Now watch the change happen.";
        say("Now watch the change happen.");
        paintScene($(el.stage + "sc"), o.scene, changed.map((s) => s.id), 520, () => {
          r++;
          setTimeout(() => {
            if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You predicted " + score + " of " + rounds.length + " changes before they happened. " + o.done); }
            else start();
          }, 1800);
        });
      });
  ...
  /* ---- a machine with an input and an output (3CT.07, 3CT.08) ---------
     First the child puts the machine's steps in order (an algorithm that
     produces an output FROM an input). Then feeds it inputs and watches
     the steps light up and the output come out. Then one question about
     an input it has not tried; the key is computed by ruleOutput. */
  function inOut(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, score = 0;
    function machineHtml(rd, order, lit, input, output) {
      return '<div class="machine2"><div class="slot in"><span class="slotlab">INPUT</span><span class="slotval">' + (input == null ? "?" : esc(input)) + "</span></div>" +
        '<ol class="algo small">' + order.map((k, j) => '<li class="' + (lit === j ? "now" : lit > j ? "done" : "") + '">' + esc(rd.steps[k]) + "</li>").join("") + "</ol>" +
        '<div class="slot out"><span class="slotlab">OUTPUT</span><span class="slotval">' + (output == null ? "?" : esc(output)) + "</span></div></div>";
    }
    function build() {
      const rd = rounds[r], placed = [];
      const pool = shuffle(rd.steps.map((_, k) => k));
      let wrong = 0;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = ""; $(el.ch).className = "choices";
      function draw() {
        $(el.stage).innerHTML = '<div class="stagewide"><p class="goal"><b>' + esc(rd.name) + "</b></p>" + machineHtml(rd, placed, -1, null, null) +
          '<p class="sub">Tap the steps in order to build the machine.</p><div class="cardsgrid">' + pool.filter((k) => !placed.includes(k)).map((k) => '<button type="button" class="tapcard" data-k="' + k + '">' + esc(rd.steps[k]) + "</button>").join("") + "</div></div>";
        $(el.score).textContent = "Machine " + (r + 1) + " of " + rounds.length + " · " + placed.length + " of " + rd.steps.length + " steps";
      }
      $(el.ask).innerHTML = "Build <b>" + esc(rd.name) + "</b>: tap its steps in order. Then feed it inputs.";
      sayHere(o.finish, "Build " + rd.name + ". Tap its steps in order. Then feed it inputs.");
      const handler = (e) => {
        const b = e.target.closest(".tapcard[data-k]"); if (!b) return;
        const k = Number(b.dataset.k);
        if (k === placed.length) {
          placed.push(k); SOUND.play("pop", 0.35); draw();
          if (placed.length === rd.steps.length) {
            $(el.stage).removeEventListener("click", handler);
            if (!wrong) score++;
            const line = cheer() + " The machine is built. Now give it an input.";
            $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
            setTimeout(feed, 2000);
          }
        } else {
          wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
          const line = k < placed.length ? "That step is already in." : "Not yet. Something comes before that.";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          setTimeout(() => b.classList.remove("wrong"), 700);
        }
      };
      $(el.stage).addEventListener("click", handler);
      draw();
    }
    function feed() {
      const rd = rounds[r], tried = new Set(), order = rd.steps.map((_, k) => k);
      let lock = false;
      function draw(lit, input, output) {
        $(el.stage).innerHTML = '<div class="stagewide"><p class="goal"><b>' + esc(rd.name) + "</b></p>" + machineHtml(rd, order, lit, input, output) +
          '<div class="chips">' + rd.inputs.map((x) => '<button type="button" class="chip' + (tried.has(String(x)) ? " tried" : "") + '" data-in="' + esc(x) + '"' + (lock ? " disabled" : "") + ">" + esc(x) + "</button>").join("") + "</div></div>";
        $(el.score).textContent = "Machine " + (r + 1) + " of " + rounds.length + " · " + tried.size + " of " + rd.inputs.length + " inputs tried";
      }
      $(el.ask).innerHTML = "Tap an input and watch what comes out.";
      sayHere(o.finish, "Tap an input and watch what comes out.");
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      const handler = (e) => {
        const b = e.target.closest(".chip[data-in]"); if (!b || lock) return;
        lock = true;
        const x = b.dataset.in, out = ruleOutput(rd.rule, x);
        let j = 0;
        const step = () => {
          ...
          if (j < rd.steps.length) { SOUND.play("click", 0.3); j++; setTimeout(step, 520); }
            ...
            $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
            reportAttempt(o.finish, tried.size, rd.inputs.length, "inputs");
            if (tried.size >= rd.inputs.length) {
              $(el.stage).removeEventListener("click", handler);
              setTimeout(() => {
                $(el.fb).textContent = "";
                askOnce(o, { ask: rd.then.ask, opts: rd.then.opts, why: rd.then.why }, (ok) => {
                  if (ok) score++;
                  r++;
                  if (r >= rounds.length) { reportScore(o.finish, score, rounds.length * 2); endStep(o, "You built " + rounds.length + " machines and worked out their outputs. " + o.done); }
                  else build();
                });
              }, 2200);
            }
  ...
  /* ---- tidy a program: take out what does nothing, fold what repeats
     (3P.01). The program works, but it is long: a wait block that does
     nothing, the same block three times in a row. The child taps a block
     and chooses to delete it or fold the run into a repeat block, then
     presses Run: the cat must do exactly what it did before, in fewer
     blocks. sameEffect() decides, mirrored in _rules.py. */
  function tidyProgram(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, script = [], picked = -1, running = false, score = 0, firstRun = true, sprite = null;
    const round = () => rounds[r];
    const runOf = (k) => { let n = 1; while (script[k + n] === script[k]) n++; return n; };
    function draw(now) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">It should: <b>' + esc(rd.goal) + "</b>. It does, in " + script.length + " blocks. Make it shorter.</p>" +
        '<div class="spritestage" id="' + el.stage + 'sp"></div>' +
        '<div class="script">' + script.map((id, k) => blockBtn(id, 'data-k="' + k + '"' + (running ? " disabled" : ""), k === picked || k === now ? "now" : "")).join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small ghost" id="' + el.stage + 'undo"' + (running ? " disabled" : "") + '>Start again</button><button type="button" class="big small teal" id="' + el.stage + 'run"' + (running ? " disabled" : "") + ">&#9654; Run</button></div></div>";
      sprite = spriteStage($(el.stage + "sp"), o.sprite || "\u{1F431}");
      $(el.score).textContent = "Program " + (r + 1) + " of " + rounds.length + " · " + script.length + " blocks (was " + rd.program.length + ")";
      $(el.stage + "run").addEventListener("click", runIt);
      $(el.stage + "undo").addEventListener("click", () => { script = rd.program.slice(); picked = -1; menu(); draw(-1); });
    }
    function menu() {
      $(el.ch).className = "choices";
      if (picked < 0) { $(el.ch).innerHTML = ""; return; }
      const n = runOf(picked), id = script[picked];
      $(el.ch).innerHTML = '<button type="button" class="choice text" data-do="delete">Delete this ' + esc((BLOCKS[id] || {}).label || id) + " block</button>" +
        (n >= 2 && n <= 4 && !REPEAT[id] ? '<button type="button" class="choice text" data-do="fold">Fold these ' + n + " into a repeat block</button>" : "") +
        '<button type="button" class="choice text ghost" data-do="cancel">Leave it</button>';
    }
    function start() {
      script = round().program.slice(); picked = -1; running = false; firstRun = true;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; menu(); draw(-1);
      $(el.ask).innerHTML = "Tap a block to delete it or fold a run of the same block into a <b>repeat</b>. Then press <b>Run</b>.";
      sayHere(o.finish, "It should " + round().goal + ". It does, but it is long. Tap a block to delete it, or fold a run of the same block into a repeat. Then press Run.");
    }
    function runIt() {
      if (running || !script.length) return;
      running = true; picked = -1; menu(); sprite.reset(); draw(-1);
      const moves = expandProgram(script);
      const owner = []; for (let i = 0, m = 0; i < script.length; i++) { const n = REPEAT[script[i]]; if (n) { const next = script[i + 1]; if (next && !REPEAT[next]) { for (let j = 0; j < n; j++) owner[m++] = i; i++; } } else owner[m++] = i; }
      let i = 0;
      const step = () => {
        if (i >= moves.length) return finished();
        draw(owner[i]);
        sprite.run(moves[i]).then(() => { i++; step(); });
      };
      setTimeout(step, 300);
    }
    function finished() {
      const rd = round();
      running = false; draw(-1);
      const same = sameEffect(script, rd.program), short = script.length <= rd.expect.length;
      if (same && short) {
        if (firstRun) score++;
        SOUND.play("tada", 0.5);
        ...
        r++;
        setTimeout(() => {
          if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You made " + rounds.length + " programs shorter without changing what they do. " + o.done); }
          else start();
        }, 3400);
      } else if (same) {
        firstRun = false; SOUND.play("click", 0.3);
    ...
    $(el.stage).addEventListener("click", (e) => {
      if (running) return;
      const b = e.target.closest(".script [data-k]"); if (!b) return;
      picked = Number(b.dataset.k); SOUND.play("click", 0.3); menu(); draw(-1);
      $(el.fb).className = "fb"; $(el.fb).textContent = "Block " + (picked + 1) + ": " + ((BLOCKS[script[picked]] || {}).label || script[picked]) + ". What shall we do with it?";
    });
    $(el.ch).addEventListener("click", (e) => {
      const b = e.target.closest("[data-do]"); if (!b || running || picked < 0) return;
      const what = b.dataset.do;
      if (what === "delete") { const id = script[picked]; script.splice(picked, 1); SOUND.play("pop", 0.3); $(el.fb).className = "fb"; $(el.fb).textContent = "Deleted " + ((BLOCKS[id] || {}).label || id) + ". " + script.length + " blocks now. Run it to test."; say($(el.fb).textContent); }
      else if (what === "fold") { const n = runOf(picked), id = script[picked]; script.splice(picked, n, "repeat" + n, id); SOUND.play("whoosh", 0.4); $(el.fb).className = "fb"; $(el.fb).textContent = n + " " + ((BLOCKS[id] || {}).label || id) + " blocks folded into repeat " + n + " times. Run it to test."; say($(el.fb).textContent); }
      picked = -1; menu(); draw(-1);
    });
    start();
  }

  /* ---- more than one algorithm running at the same time (3P.03), with
     more than one object, one of them static (3P.04). Every object has
     its own script; the child builds each one (tabs), then presses Run
     and all of them run AT ONCE - the cat's first block and the dog's
     first block in the same beat. A static object (a tree, a sun) gets
     look blocks only: it can say and grow, it cannot move. */
  function parallelProgram(o) {
    const el = o.el, rounds = o.rounds, sprites = o.sprites, names = o.spriteNames, statics = new Set(o.static || []);
    let r = 0, scripts = [], active = 0, running = false, firstRun = true, score = 0, sprite = null;
    const round = () => rounds[r];
    const palette = (k) => statics.has(k) ? (o.staticBlocks || ["say", "grow", "shrink", "hide", "wait"]) : (o.blocks || Object.keys(BLOCKS));
    function draw(nows) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide">' +
        '<div class="spritestage" id="' + el.stage + 'sp"></div>' +
        '<div class="objtabs">' + sprites.map((s, k) => '<button type="button" class="objtab' + (k === active ? " on" : "") + (statics.has(k) ? " static" : "") + '" data-obj="' + k + '"' + (running ? " disabled" : "") + '><span aria-hidden="true">' + esc(s) + "</span> " + esc(names[k]) + (statics.has(k) ? " (static)" : "") + "</button>").join("") + "</div>" +
        '<p class="goal">The <b>' + esc(names[active]) + "</b>'s algorithm: <b>" + esc(rd.scripts[active].algorithm.join(", ")) + "</b></p>" +
        scriptHtml(scripts[active], nows ? nows[active] : -1, !running) +
        '<div class="palette">' + palette(active).map((id) => blockBtn(id, 'data-add="' + id + '"' + (running ? " disabled" : ""))).join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small ghost" id="' + el.stage + 'clear"' + (running || !scripts[active].length ? " disabled" : "") + '>Clear</button>' +
        '<button type="button" class="big small teal" id="' + el.stage + 'run"' + (running || scripts.every((s) => !s.length) ? " disabled" : "") + ">&#9654; Run all at once</button></div></div>";
      sprite = spriteStage($(el.stage + "sp"), sprites);
      sprites.forEach((_, k) => { if (statics.has(k)) $(el.stage + "spsp" + k).classList.add("static"); });
      $(el.score).textContent = "Program " + (r + 1) + " of " + rounds.length + " · " + sprites.map((s, k) => names[k] + " " + scripts[k].length).join(" · ");
      $(el.stage + "clear").addEventListener("click", () => { scripts[active] = []; draw(); });
      $(el.stage + "run").addEventListener("click", runIt);
    }
    function start() {
      scripts = sprites.map(() => []); active = 0; running = false; firstRun = true;
      $(el.ch).innerHTML = ""; $(el.ch).className = "choices"; $(el.fb).textContent = ""; $(el.fb).className = "fb";
      draw();
      $(el.ask).innerHTML = "Build a program for <b>each</b> object (tap its tab), then press <b>Run all at once</b>.";
      sayHere(o.finish, "Build a program for each object. Tap an object's tab to see its algorithm. Then press Run all at once.");
    }
    function runIt() {
      if (running) return;
      running = true; sprite.reset();
      const moves = scripts.map((s) => expandProgram(s)), longest = Math.max.apply(null, moves.map((m) => m.length));
      const owners = scripts.map((s) => { const own = []; for (let i = 0, m = 0; i < s.length; i++) { const n = REPEAT[s[i]]; if (n) { const next = s[i + 1]; if (next && !REPEAT[next]) { for (let j = 0; j < n; j++) own[m++] = i; i++; } } else own[m++] = i; } return own; });
      let i = 0;
      const beat = () => {
        if (i >= longest) return finished();
        draw(owners.map((own, k) => (i < moves[k].length ? own[i] : -1)));
        Promise.all(moves.map((m, k) => (i < m.length ? sprite.run(m[i], k) : wait(720)))).then(() => { i++; beat(); });
      };
      draw(); setTimeout(beat, 300);
    }
    function finished() {
      const rd = round();
      running = false; draw();
      const ok = sprites.map((_, k) => { const did = expandProgram(scripts[k]), want = expandProgram(rd.scripts[k].expect); return did.length === want.length && did.every((x, j) => x === want[j]); });
      if (ok.every(Boolean)) {
        if (firstRun) score++;
        SOUND.play("tada", 0.5);
        ...
        r++;
        setTimeout(() => {
          if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You ran " + sprites.length + " programs at once, " + rounds.length + " times. " + o.done); }
          else start();
        }, 3600);
      } else {
        firstRun = false; SOUND.play("error", 0.3);
    ...
    $(el.stage).addEventListener("click", (e) => {
      if (running) return;
      const tab = e.target.closest("[data-obj]");
      if (tab) { active = Number(tab.dataset.obj); SOUND.play("click", 0.3); draw(); sprite.active(active); return; }
      const add = e.target.closest("[data-add]");
      if (add) { scripts[active].push(add.dataset.add); SOUND.play("click", 0.3); draw(); return; }
      const placed = e.target.closest(".script [data-k]");
      if (placed) { scripts[active].splice(Number(placed.dataset.k), 1); SOUND.play("pop", 0.3); draw(); }
    });
    start();
  }

  /* ---- change a number inside a block (3P.05) --------------------------
     The program is given and cannot be re-ordered; each move block has a
     NUMBER in it. The cat must stop exactly on the flower. Tap a number
     to change it, run, read where the cat stopped, change again. The
     end square is computed by walkEnd(), mirrored in _rules.py. */
  function tweakProgram(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, program = [], running = false, firstRun = true, score = 0, sprite = null;
    const round = () => rounds[r];
    function draw(now) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">We want: <b>' + esc(rd.goal) + "</b></p>" +
        '<div class="spritestage" id="' + el.stage + 'sp"></div>' +
        '<div class="script">' + program.map((b, k) => '<span class="block ' + (BLOCKS[b.id] || {}).cat + (k === now ? " now" : "") + '"><span class="bicon" aria-hidden="true">' + (BLOCKS[b.id] || {}).icon + "</span>" + esc((BLOCKS[b.id] || {}).label) +
          (b.id === "jump" ? "" : ' <button type="button" class="numchip" data-k="' + k + '" aria-label="' + esc((BLOCKS[b.id] || {}).label) + " " + b.n + ' squares; tap to change"' + (running ? " disabled" : "") + ">&times; " + b.n + "</button>") + "</span>").join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small teal" id="' + el.stage + 'run"' + (running ? " disabled" : "") + ">&#9654; Run</button></div></div>";
      sprite = spriteStage($(el.stage + "sp"), o.sprite || "\u{1F431}");
      sprite.mark(rd.target, rd.targetPic || "\u{1F33C}");
      $(el.score).textContent = "Program " + (r + 1) + " of " + rounds.length + " · the flower is " + rd.target + " squares along";
      $(el.stage + "run").addEventListener("click", runIt);
    }
    function start() {
      program = round().program.map((b) => Object.assign({}, b)); running = false; firstRun = true;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      draw(-1);
      $(el.ask).innerHTML = "Tap a <b>number</b> to change it. Make the cat stop on the flower, then press <b>Run</b>.";
      sayHere(o.finish, "We want the cat to " + round().goal + ". Tap a number in a block to change it, then press Run.");
    }
    function runIt() {
      if (running) return;
      running = true; sprite.reset(); draw(-1);
      const moves = [], owner = [];
      program.forEach((b, k) => { const n = b.id === "jump" ? 1 : Number(b.n); for (let j = 0; j < n; j++) { moves.push(b.id); owner.push(k); } });
      let i = 0;
      const step = () => {
        if (i >= moves.length) return finished();
        draw(owner[i]);
    ...
    function finished() {
      const rd = round(), end = walkEnd(program);
      running = false; draw(-1);
      if (end === rd.target) {
        if (firstRun) score++;
        SOUND.play("tada", 0.5);
        ...
        r++;
        setTimeout(() => {
          if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You changed the numbers in " + rounds.length + " programs to hit the target. " + o.done); }
          else start();
        }, 3400);
      } else {
        firstRun = false; SOUND.play("error", 0.3);
    ...
    $(el.stage).addEventListener("click", (e) => {
      const b = e.target.closest(".numchip"); if (!b || running) return;
      const k = Number(b.dataset.k);
      program[k].n = (Number(program[k].n) % 4) + 1; SOUND.play("click", 0.3); draw(-1);
      $(el.fb).className = "fb"; $(el.fb).textContent = (BLOCKS[program[k].id] || {}).label + " is now " + program[k].n + ". Run it to see.";
    });
    start();
  }

  /* ---- a physical computing device (3P.06, 3P.10, 3CS.05; 4P.08, 4P.09,
     4CS.01) ---------------------------------------------------------------
     Bitsy: a small board with a 5-by-5 light grid, a speaker, a motor and
     a bell, and five INPUTS on it - button A, a shake, a clap, a heat
     sensor, a light sensor. A program starts with a "when ..." hat block
     naming its input, then output blocks. Stage 4 adds the loop blocks: a
     repeat N times block repeats the block after it, and a forever block
     repeats everything after it until the child presses Stop - because
  ...
  const DEVICE_BLOCKS = {
    ...
    heart: { label: "show a heart", icon: "❤️", cat: "out" },
    smile: { label: "show a smile", icon: "\u{1F642}", cat: "out" },
    light: { label: "all lights on", icon: "\u{1F4A1}", cat: "out" },
    dark: { label: "all lights off", icon: "⬛", cat: "out" },
    beep: { label: "beep", icon: "\u{1F50A}", cat: "out" },
    motor: { label: "turn the motor", icon: "⚙️", cat: "out" },
    bell: { label: "ring the bell", icon: "\u{1F514}", cat: "out" },
    wait: { label: "wait a moment", icon: "⏳", cat: "out" },
    repeat2: { label: "repeat 2 times", icon: "\u{1F501}", cat: "loop", repeat: 2 },
    repeat3: { label: "repeat 3 times", icon: "\u{1F501}", cat: "loop", repeat: 3 },
    repeat4: { label: "repeat 4 times", icon: "\u{1F501}", cat: "loop", repeat: 4 },
    forever: { label: "forever", icon: "♾️", cat: "loop", forever: true },
  };
  const LED = {
  ...
  function devicePlan(script) {
    const plan = []; let foreverAt = -1;
    for (let i = 1; i < script.length; i++) {
      const b = DEVICE_BLOCKS[script[i]];
      if (b.forever) { foreverAt = plan.length; continue; }
      if (b.repeat) { const nx = script[i + 1]; if (nx && DEVICE_BLOCKS[nx].cat === "out") { for (let j = 0; j < b.repeat; j++) plan.push([i + 1, nx]); i++; } continue; }
      plan.push([i, script[i]]);
    }
    return { plan, foreverAt };
  }
  function deviceProgram(o) {
    const el = o.el, rounds = o.rounds, inputs = DEVICE_INPUTS.filter(([id]) => !o.inputs || o.inputs.includes(id));
    let r = 0, script = [], running = false, firstRun = true, score = 0, leds = LED.dark, motorOn = false, bellOn = false, stopReq = false, looping = false;
    const round = () => rounds[r];
    const hatOf = (s) => (s.length && DEVICE_BLOCKS[s[0]] && DEVICE_BLOCKS[s[0]].cat === "hat" ? DEVICE_BLOCKS[s[0]].input : null);
    function board() {
      return '<div class="board"><div class="ledgrid" role="img" aria-label="Bitsy\'s lights">' + leds.map((row) => row.split("").map((c) => '<i class="led' + (c === "1" ? " on" : "") + '"></i>').join("")).join("") + "</div>" +
        '<div class="boardparts"><span class="bpart' + (motorOn ? " on" : "") + '" aria-hidden="true">⚙️</span><span class="bpart' + (bellOn ? " on" : "") + '" aria-hidden="true">\u{1F514}</span></div>' +
        '<div class="inputs">' + inputs.map(([id, lab]) => '<button type="button" class="big small" data-in="' + id + '"' + (running ? " disabled" : "") + ">" + lab + "</button>").join("") +
        (looping ? '<button type="button" class="big small stopbtn" data-stop="1">⏹ Stop</button>' : "") + "</div></div>";
    }
    function draw(now) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The algorithm: <b>' + esc(rd.algorithm.join(", ")) + "</b></p>" + board() +
        '<div class="script">' + (script.length ? script.map((id, k) => devBlock(id, 'data-k="' + k + '"' + (running ? " disabled" : ""))).join("") : '<span class="hint">Start with a "when" block, then the outputs</span>') + "</div>" +
        '<div class="palette">' + (o.blocks || Object.keys(DEVICE_BLOCKS)).map((id) => devBlock(id, 'data-add="' + id + '"' + (running ? " disabled" : ""))).join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small ghost" id="' + el.stage + 'clear"' + (running || !script.length ? " disabled" : "") + ">Clear</button></div></div>";
      if (now >= 0) { const b = $(el.stage).querySelectorAll(".script .block")[now]; if (b) b.classList.add("now"); }
      $(el.score).textContent = "Program " + (r + 1) + " of " + rounds.length;
      $(el.stage + "clear").addEventListener("click", () => { script = []; draw(-1); });
    }
    function start() {
      script = []; running = false; firstRun = true; leds = LED.dark; motorOn = false; bellOn = false; stopReq = false; looping = false;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      draw(-1);
      $(el.ask).innerHTML = "Build the program: a <b>when</b> block first, then the outputs. Then press that input on the board.";
      sayHere(o.finish, "Build the program. A when block first, then the outputs. Then press that input on the board to run it.");
    }
    function trigger(input) {
      if (running) return;
      const hat = hatOf(script);
      if (!script.length) { const line = "There is no program yet. Build it first."; $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); return; }
      if (!hat) { const line = "The program has no when block, so nothing can start it. Put a when block first."; $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); SOUND.play("error", 0.3); firstRun = false; return; }
      if (hat !== input) { const line = "Nothing happened. The program is waiting for " + DEVICE_BLOCKS[script[0]].label.replace("when ", "") + ", not that."; $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); SOUND.play("click", 0.3); return; }
      const { plan, foreverAt } = devicePlan(script);
      running = true; leds = LED.dark; motorOn = false; bellOn = false; stopReq = false; looping = foreverAt >= 0; draw(0);
      let k = 0, cycles = 0;
      const doOne = (id) => { if (LED[id]) leds = LED[id]; if (id === "beep") SOUND.play("beep", 0.5); if (id === "motor") { motorOn = true; SOUND.play("robot", 0.4); } if (id === "bell") { bellOn = true; SOUND.play("bell", 0.5); } };
      const step = () => {
        if (stopReq) { looping = false; return finished(cycles); }
        if (k >= plan.length) {
          if (foreverAt >= 0 && cycles < 5) {
            cycles++; k = foreverAt;
            if (cycles === 1) { const line = "It is a forever loop: it goes round and round until you press Stop."; $(el.fb).className = "fb"; $(el.fb).textContent = line; say(line); }
          } else { looping = false; return finished(cycles); }
        }
    ...
    function finished(cycles) {
      const rd = round();
      running = false; looping = false; draw(-1);
      const same = script.length === rd.expect.length && script.every((id, k) => id === rd.expect[k]);
      const loopNote = cycles > 0 ? " The forever loop went round " + cycles + " time" + (cycles === 1 ? "" : "s") + (stopReq ? " before you stopped it." : " and would have gone on.") : "";
      if (same) {
        if (firstRun) score++;
        SOUND.play("tada", 0.5);
        const line = cheer() + " " + DEVICE_BLOCKS[script[0]].label.replace(/^when /, "When ") + ", Bitsy did: " + script.slice(1).map((id) => DEVICE_BLOCKS[id].label).join(", ") + "." + loopNote + " An input made an output.";
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        $(el.stage).querySelectorAll("button").forEach((b) => { b.disabled = true; });
        r++;
        setTimeout(() => {
          if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You programmed Bitsy " + rounds.length + " times: an input in, outputs out. " + o.done); }
          else start();
        }, 3400);
      } else {
        firstRun = false; SOUND.play("error", 0.3);
        const line = "Bitsy did " + script.slice(1).map((id) => DEVICE_BLOCKS[id].label).join(", ") + "." + loopNote + " The algorithm says " + rd.algorithm.join(", ").toLowerCase() + ". Change the blocks and try the input again.";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    }
    $(el.stage).addEventListener("click", (e) => {
      const stop = e.target.closest("[data-stop]"); if (stop) { stopReq = true; SOUND.play("click", 0.3); return; }
      if (running) return;
      const inp = e.target.closest("[data-in]"); if (inp) { trigger(inp.dataset.in); return; }
      const add = e.target.closest("[data-add]");
      if (add) {
        const id = add.dataset.add;
        if (DEVICE_BLOCKS[id].cat === "hat" && script.length) { const line = "A when block goes first, before the outputs. Clear and start with it."; $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); SOUND.play("error", 0.3); return; }
        script.push(id); SOUND.play("click", 0.3); draw(-1); return;
      }
      const placed = e.target.closest(".script [data-k]");
      if (placed) { script.splice(Number(placed.dataset.k), 1); SOUND.play("pop", 0.3); draw(-1); }
    });
    start();
  }

  /* ---- three views of the same data (3MD.02) -----------------------------
     A table, a bar chart and a pictogram of ONE set of counts. The child
     opens each view, then answers questions the data can answer (keys
     computed by table_answer in the builder and the gate), and one
     question about which view showed it best. */
  function dataViews(o) {
    const el = o.el, cols = o.columns, seen = new Set(), maxV = Math.max.apply(null, cols.map((c) => c.value));
    let view = "table", qi = 0, right = 0, phase = "look";
    function viewHtml() {
      if (view === "table") return '<table class="rec small tally"><caption>' + esc(o.title) + "</caption><thead><tr><th>" + esc(o.columns_label || "") + "</th><th>" + esc(o.value_label || "how many") + "</th></tr></thead><tbody>" +
        cols.map((c) => '<tr><td><span class="rowlab">' + small(c.pic) + " " + esc(c.label) + '</span></td><td><span class="cell filled">' + c.value + "</span></td></tr>").join("") + "</tbody></table>";
      if (view === "bar") return '<div class="graph" role="img" aria-label="A bar chart: ' + esc(cols.map((c) => c.label + " " + c.value).join(", ")) + '">' + cols.map((c) =>
        '<div class="gcol"><span class="gval">' + c.value + '</span><div class="gstack" style="height:' + (maxV * 22 + 6) + 'px"><i class="gbar" style="height:' + (c.value * 22) + 'px"></i></div><span class="glab">' + small(c.pic) + "<br>" + esc(c.label) + "</span></div>").join("") + "</div>";
      return '<table class="rec small tally picto2"><caption>' + esc(o.title) + " (one picture = one)</caption><tbody>" +
        cols.map((c) => '<tr><td><span class="rowlab">' + esc(c.label) + '</span></td><td><span class="cell filled"><span class="picto" aria-hidden="true">' + small(c.pic).repeat(c.value) + "</span></span></td></tr>").join("") + "</tbody></table>";
    }
    function draw() {
      $(el.stage).innerHTML = '<div class="stagewide"><div class="tabs">' + [["table", "Table"], ["bar", "Bar chart"], ["picto", "Pictogram"]].map(([id, lab]) =>
        '<button type="button" class="tab' + (view === id ? " on" : "") + (seen.has(id) ? " seen" : "") + '" data-view="' + id + '">' + lab + "</button>").join("") + "</div>" + viewHtml() + "</div>";
      $(el.score).textContent = phase === "look" ? seen.size + " of 3 views seen" : "Question " + (qi + 1) + " of " + o.questions.length;
    }
    function ask() {
      phase = "ask"; draw();
      const q = o.questions[qi];
      askOnce(o, { ask: q.ask + " <span class=\"dim\">(use any view)</span>", opts: q.opts, why: q.why }, (ok) => {
        if (ok) right++;
        qi++;
        if (qi < o.questions.length) ask();
        else {
          reportScore(o.finish, right, o.questions.length);
          endStep(o, "You read the same data three ways and answered " + right + " of " + o.questions.length + ". " + o.done);
        }
      });
    }
    $(el.stage).addEventListener("click", (e) => {
      const t = e.target.closest("[data-view]"); if (!t) return;
      view = t.dataset.view; seen.add(view); SOUND.play("click", 0.3); draw();
      if (phase === "look") {
        const line = { table: "The table: every count as a number.", bar: "The bar chart: the tallest bar is the biggest count, at a glance.", picto: "The pictogram: one picture for each one, so you can count them." }[view];
        $(el.fb).className = "fb"; $(el.fb).textContent = line; say(line);
        reportAttempt(o.finish, seen.size, 3, "views");
        if (seen.size >= 3) setTimeout(() => { $(el.fb).textContent = ""; ask(); }, 2400);
      }
    });
    draw();
    ONSHOW[o.finish] = (function () { let said = false; return () => { if (said) return; said = true; afterVoice(() => sayHere(o.finish, "Open all three views: the table, the bar chart and the pictogram.")); }; })();
  }

  /* ---- a spreadsheet: rows, columns, cells (3MD.04), formats (3MD.05) --
     A small grid with lettered columns and numbered rows. Tasks: tap the
     cell that holds a thing; put a value into a named cell; format a
     column or a cell as text, number, date or currency, and see the
     display change. Nothing here is typed: values come from chips. */
  const FORMATS = { text: "Text", number: "Number", date: "Date", currency: "Currency" };
  function fmtValue(v, f) {
    if (v == null || v === "") return "";
    if (f === "currency" && Number.isFinite(Number(v))) return "£" + Number(v).toFixed(2);
    if (f === "date") return "\u{1F4C5} " + esc(v);
    return esc(v);
  }
  function spreadsheet(o) {
    const el = o.el, cols = o.cols, nrows = o.rows, tasks = o.tasks;
    const cells = Object.assign({}, o.cells), formats = Object.assign({}, o.formats || {});
    let t = 0, right = 0, picked = null, lock = false, wrongHere = false;
    const fmtOf = (name) => formats[name] || formats[name.replace(/\d+$/, "")] || "text";
    function draw() {
      let html = '<div class="sheetwrap"><table class="sheet" role="grid" aria-label="A spreadsheet"><thead><tr><th></th>' + cols.map((c) => '<th><button type="button" class="colhead' + (picked === c ? " on" : "") + '" data-col="' + c + '">' + c + "</button></th>").join("") + "</tr></thead><tbody>";
      for (let rI = 1; rI <= nrows; rI++) {
        html += "<tr><th>" + rI + "</th>" + cols.map((c) => { const name = c + rI, f = fmtOf(name); return '<td><button type="button" class="cellbtn f-' + f + (picked === name ? " on" : "") + '" data-cell="' + name + '" aria-label="cell ' + name + '">' + fmtValue(cells[name], f) + "</button></td>"; }).join("") + "</tr>";
      }
      $(el.stage).innerHTML = '<div class="stagewide">' + html + "</tbody></table></div></div>";
      $(el.score).textContent = t < tasks.length ? "Task " + (t + 1) + " of " + tasks.length : "";
    }
    function chips() {
      const task = tasks[t];
      $(el.ch).className = "choices";
      if (task.kind === "enter" && picked === task.cell) $(el.ch).innerHTML = '<span class="dim">Put in:</span>' + shuffle(task.values).map((v) => '<button type="button" class="chip" data-val="' + esc(v) + '">' + esc(v) + "</button>").join("");
      else if (task.kind === "format" && picked === task.target) $(el.ch).innerHTML = '<span class="dim">Format as:</span>' + Object.keys(FORMATS).map((f) => '<button type="button" class="chip" data-fmt="' + f + '">' + FORMATS[f] + "</button>").join("");
      else $(el.ch).innerHTML = "";
    }
    function ask() {
      const task = tasks[t];
      picked = null; lock = false; wrongHere = false; draw(); chips();
      $(el.ask).innerHTML = task.ask;
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      sayHere(o.finish, plain(task.ask));
    }
    function good(line) {
      lock = true; if (!wrongHere) right++;
      SOUND.play("ding", 0.4); draw(); chips();
      $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + line; say(cheer() + " " + line);
      t++;
      setTimeout(() => {
        if (t >= tasks.length) { reportScore(o.finish, right, tasks.length); endStep(o, "You found, filled and formatted cells in a spreadsheet. " + o.done); }
        else ask();
      }, 2800);
    }
    function bad(line) { wrongHere = true; SOUND.play("error", 0.3); $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); }
    $(el.stage).addEventListener("click", (e) => {
      if (lock || t >= tasks.length) return;
      const task = tasks[t];
      const cb = e.target.closest("[data-cell]"), ch = e.target.closest("[data-col]");
      const name = cb ? cb.dataset.cell : ch ? ch.dataset.col : null; if (!name) return;
      SOUND.play("click", 0.3);
      if (task.kind === "find") {
        if (cb && String(cells[name]) === String(task.value)) { picked = name; good("Cell " + name + " holds " + task.value + ": column " + name[0] + ", row " + name.slice(1) + "."); }
        else if (cb) bad("Cell " + name + " holds " + (cells[name] == null || cells[name] === "" ? "nothing" : cells[name]) + ". Find " + task.value + ".");
        else bad("That is a whole column. Tap one cell.");
        return;
      }
      if (task.kind === "enter") {
        if (cb && name === task.cell) { picked = name; draw(); chips(); $(el.fb).className = "fb"; $(el.fb).textContent = "Cell " + name + " picked. Now choose what to put in it."; say($(el.fb).textContent); }
        else bad((cb ? "That is cell " + name : "That is a whole column") + ". We want cell " + task.cell + ": column " + task.cell[0] + ", row " + task.cell.slice(1) + ".");
        return;
      }
      if (task.kind === "format") {
        if (name === task.target) { picked = name; draw(); chips(); $(el.fb).className = "fb"; $(el.fb).textContent = (ch ? "Column " : "Cell ") + name + " picked. Now choose a format."; say($(el.fb).textContent); }
        else bad("Not that one. " + (task.target.length === 1 ? "Tap the letter at the top of column " + task.target + "." : "Tap cell " + task.target + "."));
      }
    });
    $(el.ch).addEventListener("click", (e) => {
      if (lock || t >= tasks.length) return;
      const task = tasks[t], v = e.target.closest("[data-val]"), f = e.target.closest("[data-fmt]");
      if (v && task.kind === "enter") {
        if (v.dataset.val === String(task.value)) { cells[task.cell] = task.value; good(task.value + " is in cell " + task.cell + "."); }
        else bad("We want " + task.value + " in " + task.cell + ", not " + v.dataset.val + ".");
      } else if (f && task.kind === "format") {
        if (f.dataset.fmt === task.format) { formats[task.target] = task.format; good((task.target.length === 1 ? "Column " : "Cell ") + task.target + " is now " + FORMATS[task.format].toLowerCase() + ". " + (task.why || "")); }
        else bad(FORMATS[f.dataset.fmt] + " is not the right format for " + (task.reason || "this") + ". " + (task.hint || ""));
      }
    });
    ask();
  }

  /* ---- select data by its characteristics (3MD.06) ---------------------
     A table of children with a few facts each. The child builds a filter
     from chips - which fact, is / is more than / is less than, which value
     - presses Filter and sees the matching rows light up, then answers
     how many. The count is computed by rowMatches, mirrored in _rules.py. */
  function dataFilter(o) {
    const el = o.el, rows = o.rows, fields = o.fields, tasks = o.tasks;
    let t = 0, right = 0, sel = { field: null, op: "eq", value: null }, matched = null, lock = false, wrongHere = false;
    const fieldOf = (id) => fields.find((f) => f.id === id);
    const OPS = { eq: "is", ne: "is not", gt: "is more than", lt: "is less than" };
    function draw() {
      const f = sel.field && fieldOf(sel.field);
      $(el.stage).innerHTML = '<div class="stagewide"><table class="rec small tally"><thead><tr><th>who</th>' + fields.map((x) => "<th>" + esc(x.label) + "</th>").join("") + "</tr></thead><tbody>" +
        rows.map((rw, k) => '<tr class="' + (matched ? (matched.has(k) ? "hit" : "miss") : "") + '"><td><span class="rowlab">' + small(rw.pic) + " " + esc(rw.name) + "</span></td>" + fields.map((x) => '<td><span class="cell filled">' + esc(rw[x.id]) + "</span></td>").join("") + "</tr>").join("") + "</tbody></table>" +
        '<div class="filterrow"><div class="chips">' + fields.map((x) => '<button type="button" class="chip' + (sel.field === x.id ? " on" : "") + '" data-field="' + x.id + '"' + (lock ? " disabled" : "") + ">" + esc(x.label) + "</button>").join("") + "</div>" +
        (f ? '<div class="chips">' + (f.numeric ? ["eq", "gt", "lt"] : ["eq", "ne"]).map((op) => '<button type="button" class="chip' + (sel.op === op ? " on" : "") + '" data-op="' + op + '"' + (lock ? " disabled" : "") + ">" + OPS[op] + "</button>").join("") + "</div>" +
          '<div class="chips">' + f.values.map((v) => '<button type="button" class="chip' + (String(sel.value) === String(v) ? " on" : "") + '" data-value="' + esc(v) + '"' + (lock ? " disabled" : "") + ">" + esc(v) + "</button>").join("") + "</div>" : "") +
        '<div class="bigbtns"><button type="button" class="big small teal" id="' + el.stage + 'go"' + (lock || !sel.field || sel.value == null ? " disabled" : "") + ">Filter</button></div></div></div>";
      $(el.score).textContent = t < tasks.length ? "Question " + (t + 1) + " of " + tasks.length + (matched ? " · " + matched.size + " of " + rows.length + " rows match" : "") : "";
      $(el.stage + "go").addEventListener("click", run);
    }
    function ask() {
      const task = tasks[t];
      sel = { field: null, op: "eq", value: null }; matched = null; lock = false; wrongHere = false; draw();
      $(el.ask).innerHTML = task.ask + " Build the filter, then press <b>Filter</b>.";
      $(el.ch).innerHTML = ""; $(el.fb).textContent = ""; $(el.fb).className = "fb";
      sayHere(o.finish, plain(task.ask) + " Build the filter, then press Filter.");
    }
    function run() {
      if (lock || !sel.field || sel.value == null) return;
      const task = tasks[t];
      const mine = new Set(rows.map((rw, k) => (rowMatches(rw, sel) ? k : -1)).filter((k) => k >= 0));
      const want = new Set(rows.map((rw, k) => (rowMatches(rw, task.spec) ? k : -1)).filter((k) => k >= 0));
      matched = mine; SOUND.play("whoosh", 0.4); draw();
      const same = mine.size === want.size && [...mine].every((k) => want.has(k));
      if (same) {
        lock = true; if (!wrongHere) right++; draw();
        const names = [...mine].map((k) => rows[k].name);
        const line = cheer() + " " + fieldOf(sel.field).label + " " + OPS[sel.op] + " " + sel.value + " picks out " + (names.length ? names.join(", ") : "nobody") + ".";
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        setTimeout(() => {
          $(el.fb).textContent = "";
          askOnce(o, task.then, (ok) => {
            if (ok) right++;
            t++;
            if (t >= tasks.length) { reportScore(o.finish, right, tasks.length * 2); endStep(o, "You filtered the data " + tasks.length + " times and read off the answers. " + o.done); }
            else ask();
          });
        }, 2600);
      } else {
        wrongHere = true; SOUND.play("error", 0.3);
        const line = "That filter picks out " + mine.size + " row" + (mine.size === 1 ? "" : "s") + ". Read the question again: which fact, and which value?";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    }
    $(el.stage).addEventListener("click", (e) => {
      if (lock) return;
      const f = e.target.closest("[data-field]"), op = e.target.closest("[data-op]"), v = e.target.closest("[data-value]");
      if (f) { sel = { field: f.dataset.field, op: "eq", value: null }; matched = null; SOUND.play("click", 0.3); draw(); }
      else if (op) { sel.op = op.dataset.op; matched = null; SOUND.play("click", 0.3); draw(); }
      else if (v) { sel.value = v.dataset.value; matched = null; SOUND.play("click", 0.3); draw(); }
    });
    ask();
  }

  /* ---- ciphers: 1 = a (3DC.05), Caesar and Pigpen (4DC.06) --------------
     The key stays on screen: the numbered strip, or the Caesar strip with
     the shifted row under it, or the Pigpen grid. A decode round shows
     the coded message and the child taps the letters; an encode round
     shows a word and the child taps the code - numbers, shifted letters or
     Pigpen glyphs. codeWord, caesarShift and pigpenIndex decide, mirrored
     in _rules.py. */
  function cipher(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, got = [], right = 0, wrongHere = false, lock = false;
    const modeOf = (rd) => rd.mode || "number";
    const targetOf = (rd) => {
      const m = modeOf(rd), decode = rd.kind === "decode";
      if (m === "caesar") return (decode ? rd.answer : caesarShift(rd.word, rd.shift)).split("");
      if (m === "pigpen") return (decode ? rd.answer : rd.word).split("");
      return decode ? decodeCode(rd.code).split("") : codeWord(rd.word).map(String);
    };
    function strip(rd) {
      const m = modeOf(rd);
      if (m === "caesar") return '<div class="alpha" aria-label="The Caesar key: each letter moves ' + rd.shift + ' places along">' + ALPHA.map((c, k) => '<span class="al"><b>' + c + "</b><small>" + ALPHA[(k + rd.shift + 26) % 26] + "</small></span>").join("") + '</div><p class="sub">top row: the real letter · under it: the letter it becomes (shift ' + rd.shift + ")</p>";
      if (m === "pigpen") return '<div class="alpha" aria-label="The Pigpen key">' + ALPHA.map((c, k) => '<span class="al pig"><b>' + c + "</b>" + pigpenSvg(k) + "</span>").join("") + "</div>";
      return '<div class="alpha" aria-label="The code: 1 is a, 2 is b, and so on">' + ALPHA.map((c, k) => '<span class="al"><b>' + c + "</b><small>" + (k + 1) + "</small></span>").join("") + "</div>";
    }
    function shown(rd, target) {
      const m = modeOf(rd), decode = rd.kind === "decode";
      const cellOf = (top, k) => '<span class="codecell' + (k < got.length ? " done" : k === got.length ? " now" : "") + '">' + top + "<small>" + (k < got.length ? (m === "pigpen" && !decode ? pigpenSvg(pigpenIndex(got[k])) : esc(got[k])) : "?") + "</small></span>";
      if (m === "pigpen" && decode) return rd.code.split("").map((c, k) => cellOf(pigpenSvg(pigpenIndex(c)), k)).join("");
      const tops = decode ? (m === "number" ? rd.code.map(String) : rd.code.split("")) : rd.word.split("");
      return tops.map((t, k) => cellOf(esc(t), k)).join("");
    }
    function keys(rd) {
      const m = modeOf(rd), decode = rd.kind === "decode";
      if (m === "number" && !decode) return Array.from({ length: 26 }, (_, k) => '<button type="button" class="key num" data-key="' + (k + 1) + '"' + (lock ? " disabled" : "") + ">" + (k + 1) + "</button>").join("");
      if (m === "pigpen" && !decode) return ALPHA.map((c, k) => '<button type="button" class="key pigk" data-key="' + c + '" aria-label="pigpen ' + c + '"' + (lock ? " disabled" : "") + ">" + pigpenSvg(k) + "</button>").join("");
      return ALPHA.map((c) => '<button type="button" class="key" data-key="' + c + '"' + (lock ? " disabled" : "") + ">" + c + "</button>").join("");
    }
    function draw() {
      const rd = rounds[r], target = targetOf(rd), decode = rd.kind === "decode";
      $(el.stage).innerHTML = '<div class="stagewide">' + strip(rd) + '<p class="goal">' + (decode ? "Decode this message:" : "Write <b>" + esc(rd.word) + "</b> in code:") + '</p><div class="coderow">' + shown(rd, target) + "</div>" +
        '<div class="keys">' + keys(rd) + "</div></div>";
      $(el.score).textContent = "Message " + (r + 1) + " of " + rounds.length + " · " + got.length + " of " + target.length;
    }
    function askText(rd) {
      const m = modeOf(rd), decode = rd.kind === "decode";
      if (m === "caesar") return decode ? "A Caesar cipher, shift " + rd.shift + ". Find each coded letter in the BOTTOM row and tap the real letter above it to decode <b>" + esc(rd.code) + "</b>." : "A Caesar cipher, shift " + rd.shift + ". For each letter of <b>" + esc(rd.word) + "</b>, tap the letter it becomes (the one under it on the strip).";
      if (m === "pigpen") return decode ? "A Pigpen cipher. Each symbol is a letter: find it on the grid and tap the letter." : "A Pigpen cipher. For each letter of <b>" + esc(rd.word) + "</b>, tap its symbol.";
      return decode ? "Each number is a letter: 1 is a, 2 is b. Tap the letters to decode <b>" + esc(rd.code.join(" ")) + "</b>." : "Each letter is a number: a is 1, b is 2. Tap the numbers to write <b>" + esc(rd.word) + "</b> in code.";
    }
    function start() {
      got = []; wrongHere = false; lock = false; draw();
      const rd = rounds[r];
      $(el.ask).innerHTML = askText(rd);
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      sayHere(o.finish, plain(askText(rd)));
    }
    $(el.stage).addEventListener("click", (e) => {
      const k = e.target.closest(".key"); if (!k || lock) return;
      const rd = rounds[r], m = modeOf(rd), decode = rd.kind === "decode", target = targetOf(rd), v = k.dataset.key;
      if (v === target[got.length]) {
        got.push(v); SOUND.play("click", 0.35); draw();
        if (got.length >= target.length) {
          lock = true; if (!wrongHere) right++; SOUND.play("tada", 0.5); draw();
          const result = m === "number" ? (decode ? rd.code.join(" ") + " says " + decodeCode(rd.code).toUpperCase() : rd.word + " in code is " + codeWord(rd.word).join(" ")) :
            m === "caesar" ? (decode ? rd.code.toUpperCase() + " says " + rd.answer.toUpperCase() : rd.word + " with shift " + rd.shift + " is " + caesarShift(rd.word, rd.shift).toUpperCase()) :
              (decode ? "The symbols say " + rd.answer.toUpperCase() : rd.word + " in Pigpen is written");
          const line = cheer() + " " + result + "." + (rd.why ? " " + rd.why : "");
          $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
          r++;
          setTimeout(() => {
            if (r >= rounds.length) { reportScore(o.finish, right, rounds.length); endStep(o, "You wrote and decoded " + rounds.length + " secret messages. " + o.done); }
            else start();
          }, 3200);
        }
      } else {
        wrongHere = true; k.classList.add("wrong"); SOUND.play("error", 0.3);
        const want = target[got.length];
        const line = m === "number" ? (decode ? "Look at the strip: " + rd.code[got.length] + " is the letter " + want + "." : "Look at the strip: " + rd.word[got.length] + " is number " + want + ".") :
          m === "caesar" ? (decode ? "Look at the strip: " + rd.code[got.length] + " in the bottom row sits under " + want + "." : "Look at the strip: under " + rd.word[got.length] + " is " + want + ".") :
            (decode ? "Look at the grid: that symbol is " + want + "." : "Look at the grid: the symbol for " + want + " is in its own cell.");
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        setTimeout(() => k.classList.remove("wrong"), 700);
      }
    });
    start();
  }
  /* ==================================================================
     STAGE 4 - algorithms with repeat and forever loops, algorithms
     compared for a purpose, sub-routines, if-branches, a loop the child
     builds, comments on code, one program per input, plans for objects,
     testing part by part, sorting data, the parts of a table, and the
     Caesar and Pigpen ciphers. The functions in the first block are
     mirrored, name for name, in _rules.py.
     ================================================================== */

  /* rules mirrored in _rules.py */
  function expandLoop(before, body, times, after) {
    const out = (before || []).slice();
    for (let i = 0; i < Number(times); i++) out.push.apply(out, body || []);
    return out.concat(after || []);
  }
  const FOREVER_CYCLES = 2;
  /* the flat list a structured algorithm makes; each item remembers where it
     came from (block b, body index j, turn t of n) so the page can light it */
  function flattenAlgo(blocks) {
    const out = [];
    blocks.forEach((bl, b) => {
      if (bl.kind === "repeat") { for (let t = 1; t <= Number(bl.times); t++) bl.body.forEach((st, j) => out.push(Object.assign({}, st, { b, j, t, n: Number(bl.times) }))); }
      else if (bl.kind === "forever") { for (let t = 1; t <= FOREVER_CYCLES; t++) bl.body.forEach((st, j) => out.push(Object.assign({}, st, { b, j, t, n: Infinity }))); out.push({ id: "stop", label: "Stop", pic: "\u{1F6D1}", b, j: -1, t: FOREVER_CYCLES + 1, n: Infinity }); }
      else out.push(Object.assign({}, bl, { b, j: -1, t: 0, n: 0 }));
    });
    return out;
  }
  function subExpand(main, subs) {
    const out = [];
    main.forEach((bl, b) => {
      if (bl.kind === "call") { out.push({ id: "call:" + bl.sub, label: "do " + bl.sub, pic: "\u{1F4E6}", b, j: -1, sub: bl.sub }); subs[bl.sub].forEach((st, j) => out.push(Object.assign({}, st, { b, j, sub: bl.sub }))); }
      else out.push(Object.assign({}, bl, { b, j: -1 }));
    });
    return out;
  }
  function branchRun(rd, inputId) {
    const branch = inputId === rd.inputs[0].id ? rd.yes : rd.no;
    return (rd.before || []).concat(branch, rd.after || []);
  }
  function bestAlgo(algos, check) {
    const kind = check && check.kind;
    const key = kind === "fewest_steps" ? (a) => a.steps.length : kind === "fastest" ? (a) => Number(a.facts.minutes) : null;
    if (!key) return null;
    const best = Math.min.apply(null, algos.map(key));
    const winners = algos.filter((a) => key(a) === best);
    return winners.length === 1 ? winners[0].id : null;
  }
  function sortRows(rows, field, direction) {
    const numeric = rows.every((r) => typeof r[field] === "number");
    const key = numeric ? (r) => Number(r[field]) : (r) => String(r[field]).toLowerCase();
    const ordered = rows.slice().sort((a, b) => (key(a) < key(b) ? -1 : key(a) > key(b) ? 1 : 0));
    if (direction === "desc") ordered.reverse();
    return ordered;
  }
  const caesarShift = (text, shift) => String(text).toLowerCase().split("").filter((c) => c >= "a" && c <= "z").map((c) => String.fromCharCode(((c.charCodeAt(0) - 97 + Number(shift)) % 26 + 26) % 26 + 97)).join("");
  const pigpenIndex = (c) => (c >= "a" && c <= "z" ? c.charCodeAt(0) - 97 : null);
  /* the Pigpen glyph for a letter, as a small SVG: 0-8 the grid, 9-17 the
     dotted grid, 18-21 the cross, 22-25 the dotted cross */
  function pigpenSvg(i) {
    let d = "", dot = false;
    if (i < 18) {
      const k = i % 9, col = k % 3, row = Math.floor(k / 3); dot = i >= 9;
      if (col > 0) d += "M4 4 V20 ";       // a left wall when not in the left column
      if (col < 2) d += "M20 4 V20 ";      // a right wall when not in the right column
      if (row > 0) d += "M4 4 H20 ";       // a top wall when not in the top row
      if (row < 2) d += "M4 20 H20 ";      // a bottom wall when not in the bottom row
    } else {
      const k = (i - 18) % 4; dot = i >= 22;
      d = ["M4 4 L12 12 L20 4", "M4 4 L12 12 L4 20", "M4 20 L12 12 L20 20", "M20 4 L12 12 L20 20"][k];   // v pointing down, right, up, left
    }
    return '<svg viewBox="0 0 24 24" width="26" height="26" aria-hidden="true"><path d="' + d + '" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/>' + (dot ? '<circle cx="12" cy="12" r="2.2" fill="currentColor"/>' : "") + "</svg>";
  }

  /* an algorithm list with repeat and forever boxes; `lit` is a flat item */
  function structHtml(blocks, lit, tappable, foldTurn) {
    const step = (st, b, j, cls) => {
      const on = lit && lit.b === b && lit.j === j;
      const inner = '<span class="apic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label);
      return "<li" + (cls ? ' class="' + cls + '"' : "") + ">" + (tappable ? '<button type="button" class="algostep' + (on ? " lit" : "") + '" data-b="' + b + '" data-j="' + j + '">' + inner + "</button>" : '<span class="' + (on ? "now" : "") + '">' + inner + "</span>") + "</li>";
    };
    return '<ol class="algo' + (tappable ? " tappable" : "") + '">' + blocks.map((bl, b) => {
      if (bl.kind === "repeat" || bl.kind === "forever") {
        const here = lit && lit.b === b;
        const head = bl.kind === "repeat" ? "repeat " + bl.times + " times" + (here && lit.t ? " · time " + lit.t + " of " + bl.times : "") : "forever" + (here && lit.t && lit.j >= 0 ? " · round " + lit.t : "");
        return '<li class="loopbox' + (here ? " now" : "") + '"><b>' + esc(head) + ':</b><ol class="algo inner">' + bl.body.map((st, j) => step(st, b, j, "")).join("") + "</ol></li>";
      }
      return step(bl, b, -1, "");
    }).join("") + "</ol>";
  }
  const distinctSteps = (items) => { const seen = new Map(); items.forEach((st) => { if (st.id !== "stop" && !seen.has(st.id)) seen.set(st.id, st); }); return Array.from(seen.values()); };

  /* ---- follow, fix and understand an algorithm with loops (4CT.01, 4CT.02)
     A structured algorithm: plain steps, repeat N times boxes, forever
     boxes. In `follow` mode the child taps the step that comes next,
     round and round the loop, with the turn counter on the box; a forever
     loop is followed twice and then the child must press Stop, because
     nothing else ends it. In `fix` mode one step inside a loop is wrong:
     tap it, choose the fix, watch the corrected algorithm run. */
  function loopAlgo(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, score = 0;
    function follow(rd) {
      const flat = flattenAlgo(rd.blocks), done = [];
      const btns = shuffle(distinctSteps(flat)), hasForever = rd.blocks.some((b) => b.kind === "forever");
      let i = 0, wrong = 0, lock = false;
      function draw() {
        const lit = flat[i];
        $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(rd.task) + "</b></p><div class=\"algo2\">" + structHtml(rd.blocks, lit, false) +
          (rd.scene ? '<div class="sim scenebox">' + SCENES[rd.scene](done) + "</div>" : "") + "</div>" +
          '<div class="actions">' + btns.map((st) => '<button type="button" class="tapcard act" data-id="' + esc(st.id) + '"' + (lock ? " disabled" : "") + '><span class="cpic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label) + "</button>").join("") +
          (hasForever ? '<button type="button" class="tapcard act stop" data-id="stop"' + (lock ? " disabled" : "") + '><span class="cpic" aria-hidden="true">\u{1F6D1}</span>Stop</button>' : "") + "</div></div>";
        $(el.score).textContent = "Round " + (r + 1) + " of " + rounds.length + " · step " + Math.min(i + 1, flat.length) + " of " + flat.length + (lit && lit.n === Infinity && lit.j >= 0 ? " · forever, round " + lit.t : lit && lit.n ? " · repeat, time " + lit.t + " of " + lit.n : "");
      }
      const handler = (e) => {
        const b = e.target.closest(".act"); if (!b || lock) return;
        const want = flat[i], id = b.dataset.id;
        if (id === want.id) {
          if (id !== "stop") done.push(id);
          SOUND.play(id === "stop" ? "click" : "pop", 0.4); i++; draw();
          const line = id === "stop" ? "Stop. A forever loop only ends when something stops it." : want.label + "." + (want.n && want.n !== Infinity ? " Time " + want.t + " of " + want.n + "." : want.n === Infinity ? " Round " + want.t + " of the forever loop." : "");
          $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
          if (i >= flat.length) {
            lock = true; draw(); $(el.stage).removeEventListener("click", handler);
            reportScore(o.finish, Math.max(0, flat.length - wrong), flat.length);
            setTimeout(next, 2400);
          }
        } else {
          wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
          const line = want.id === "stop" ? "The loop has gone round " + FOREVER_CYCLES + " times and would go on forever. Press Stop." :
            "Not yet. Next comes " + want.label + (want.n && want.n !== Infinity ? " - it is inside the repeat, time " + want.t + " of " + want.n : want.n === Infinity ? " - the forever loop goes round again" : "") + ".";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          setTimeout(() => b.classList.remove("wrong"), 700);
        }
      };
      $(el.stage).addEventListener("click", handler);
      $(el.ask).innerHTML = "Follow the algorithm: tap the step that comes <b>next</b>, round and round the loop." + (hasForever ? " Press <b>Stop</b> when a forever loop has gone round twice." : "");
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      sayHere(o.finish, "Follow the algorithm. Tap the step that comes next, round and round the loop." + (hasForever ? " Press Stop when the forever loop has gone round twice." : ""));
      draw();
    }
    function fix(rd) {
      let phase = "find", missed = false, lock = false, blocks = rd.blocks.map((bl) => (bl.body ? Object.assign({}, bl, { body: bl.body.map((st) => Object.assign({}, st)) }) : Object.assign({}, bl)));
      const stepAt = (b, j) => (j >= 0 ? blocks[b].body[j] : blocks[b]);
      function draw(lit) {
        $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(rd.task) + "</b></p><div class=\"algo2\">" + structHtml(blocks, lit, phase === "find") +
          (rd.scene ? '<div class="sim scenebox" id="' + el.stage + 'sc">' + SCENES[rd.scene]([]) + "</div>" : "") + "</div></div>";
        $(el.score).textContent = "Round " + (r + 1) + " of " + rounds.length + " · find the wrong step";
      }
      const handler = (e) => {
        const b = e.target.closest(".algostep"); if (!b || lock || phase !== "find") return;
        const bi = Number(b.dataset.b), j = Number(b.dataset.j);
        if (bi === rd.wrong[0] && j === rd.wrong[1]) {
          lock = true; if (!missed) score++; SOUND.play("ding", 0.4);
          $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + rd.why; say(cheer() + " " + rd.why + " What should it say instead?");
          setTimeout(() => {
            phase = "fix"; lock = false; draw({ b: bi, j });
            $(el.ask).innerHTML = "What should that step say instead?";
            $(el.ch).className = "choices stack";
            $(el.ch).innerHTML = shuffle(rd.fix.opts).map((c) => '<button type="button" class="choice text" data-id="' + esc(c.id) + '" data-ok="' + (c.ok ? 1 : 0) + '">' + (c.pic ? '<span class="cpic" aria-hidden="true">' + small(c.pic) + "</span> " : "") + esc(c.t) + "</button>").join("");
          }, 2400);
        } else {
          missed = true; b.classList.add("wrong"); SOUND.play("error", 0.3);
          const line = "That step is fine" + (j >= 0 ? " - and it is inside the loop, so it happens every time" : "") + ". Which one makes the task go wrong?";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          setTimeout(() => b.classList.remove("wrong"), 700);
        }
      };
      const chooser = (e) => {
        ...
        const ok = b.dataset.ok === "1";
        if (ok) {
          lock = true; score++; $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
          const opt = rd.fix.opts.find((c) => c.id === b.dataset.id), target = stepAt(rd.wrong[0], rd.wrong[1]);
          target.id = opt.id; target.label = opt.t; if (opt.pic) target.pic = opt.pic;
          $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + rd.fix.why; say(cheer() + " " + rd.fix.why + " Now watch it run.");
          $(el.stage).removeEventListener("click", handler); $(el.ch).removeEventListener("click", chooser);
          setTimeout(() => {
            phase = "run"; $(el.ch).innerHTML = ""; $(el.ask).innerHTML = "Watch the corrected algorithm run, loop and all.";
            const flat = flattenAlgo(blocks).filter((st) => st.id !== "stop"), done = [];
            let k = 0;
            const tick = () => {
              if (k >= flat.length) { SOUND.play("tada", 0.5); reportScore(o.finish, score, rounds.length * 2); return setTimeout(next, 1600); }
              done.push(flat[k].id); draw(flat[k]); SOUND.play("pop", 0.25);
              if (rd.scene) $(el.stage + "sc").innerHTML = SCENES[rd.scene](done);
              k++; setTimeout(tick, 560);
            };
            tick();
          }, 2600);
        } else {
          missed = true; b.classList.add("wrong"); b.disabled = true; SOUND.play("error", 0.3);
          const line = "That would not make " + rd.task + " work. Try another.";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        }
      };
      $(el.stage).addEventListener("click", handler); $(el.ch).addEventListener("click", chooser);
      $(el.ask).innerHTML = "One step of this algorithm is wrong. Tap it.";
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      sayHere(o.finish, "One step of this algorithm is wrong. Tap it.");
      draw(null);
    }
    function next() {
      r++;
      if (r >= rounds.length) endStep(o, "You followed and fixed " + rounds.length + " algorithms with loops in them. " + o.done);
      else start();
    }
    function start() { const rd = rounds[r]; if (rd.mode === "fix") fix(rd); else follow(rd); }
    start();
  }

  /* ---- compare algorithms for the same task (4CT.04) ----------------
     Two or three algorithms that all do the job, side by side, with the
     facts about each. Each round names a PURPOSE - fewest steps, quickest,
     safest, best for a child - and the child picks the algorithm that fits
     it best. Fewest steps and quickest are computed; the rest are judged. */
  function compareAlgos(o) {
    const el = o.el, algos = o.algos, rounds = o.rounds;
    let r = 0, right = 0, lock = false;
    function draw() {
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(o.task) + "</b></p><div class=\"algocards\">" + algos.map((a) =>
        '<button type="button" class="algocard" data-algo="' + esc(a.id) + '"' + (lock ? " disabled" : "") + '><span class="acname"><span aria-hidden="true">' + small(a.pic) + "</span> " + esc(a.name) + '</span><ol class="algo small">' + a.steps.map((s) => "<li>" + esc(s) + "</li>").join("") + "</ol>" +
        '<span class="acfacts">' + a.steps.length + " steps · " + esc(a.facts.minutes) + " minutes" + (a.facts.note ? " · " + esc(a.facts.note) : "") + "</span></button>").join("") + "</div></div>";
      $(el.score).textContent = "Purpose " + (r + 1) + " of " + rounds.length;
    }
    function ask() {
      const rd = rounds[r];
      lock = false; draw();
      $(el.ask).innerHTML = "All of these do the job. Which is best when the purpose is: <b>" + esc(rd.purpose) + "</b>?";
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      sayHere(o.finish, "All of these do the job. Which is best when the purpose is " + rd.purpose + "?");
    }
    $(el.stage).addEventListener("click", (e) => {
      const b = e.target.closest("[data-algo]"); if (!b || lock) return;
      lock = true;
      const rd = rounds[r], ok = b.dataset.algo === rd.answer;
      $(el.stage).querySelectorAll(".algocard").forEach((c) => { c.disabled = true; if (c.dataset.algo === rd.answer) c.classList.add("right"); });
      if (ok) right++; else b.classList.add("wrong");
      SOUND.play(ok ? "ding" : "error", 0.35);
      const line = (ok ? cheer() + " " : "Not for that purpose. ") + rd.why;
      $(el.fb).className = "fb " + (ok ? "good" : "bad"); $(el.fb).textContent = line; say(line);
      r++;
      setTimeout(() => {
        if (r >= rounds.length) { reportScore(o.finish, right, rounds.length); endStep(o, "The best algorithm depends on the purpose, and you chose it " + right + " times out of " + rounds.length + ". " + o.done); }
        else ask();
      }, 3200);
    });
    ask();
  ...
  /* ---- follow an algorithm that uses a sub-routine (4CT.08) -----------
     The main algorithm calls named sub-routines ("do WASH"); each is its
     own short algorithm in a box. Following means: tap the call, then the
     sub-routine's steps, then carry on in the main algorithm where you
     left off. */
  function subRoutine(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, score = 0;
    function start() {
      const rd = rounds[r], flat = subExpand(rd.main, rd.subs), done = [];
      const btns = shuffle(distinctSteps(flat.filter((st) => !st.id.startsWith("call:"))).concat(Object.keys(rd.subs).map((name) => ({ id: "call:" + name, label: "do " + name, pic: "\u{1F4E6}" }))));
      let i = 0, wrong = 0, lock = false;
      function draw() {
        const lit = flat[i];
        $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(rd.task) + '</b></p><div class="subs"><div class="mainalgo"><b>MAIN</b><ol class="algo">' + rd.main.map((bl, b) => {
          const on = lit && lit.b === b && (lit.j === -1 ? true : lit.j >= 0);
          return bl.kind === "call" ? '<li class="call' + (on ? " now" : "") + '"><span class="apic" aria-hidden="true">\u{1F4E6}</span>do ' + esc(bl.sub) + "</li>" : '<li class="' + (on ? "now" : "") + '"><span class="apic" aria-hidden="true">' + small(bl.pic) + "</span>" + esc(bl.label) + "</li>";
        }).join("") + "</ol></div>" + Object.keys(rd.subs).map((name) => '<div class="subbox' + (lit && lit.sub === name ? " now" : "") + '"><b>' + esc(name) + '</b><ol class="algo small">' + rd.subs[name].map((st, j) => '<li class="' + (lit && lit.sub === name && lit.j === j ? "now" : "") + '"><span class="apic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label) + "</li>").join("") + "</ol></div>").join("") + "</div>" +
          (rd.scene ? '<div class="sim scenebox">' + SCENES[rd.scene](done) + "</div>" : "") +
          '<div class="actions">' + btns.map((st) => '<button type="button" class="tapcard act' + (st.id.startsWith("call:") ? " call" : "") + '" data-id="' + esc(st.id) + '"' + (lock ? " disabled" : "") + '><span class="cpic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label) + "</button>").join("") + "</div></div>";
        $(el.score).textContent = "Round " + (r + 1) + " of " + rounds.length + " · step " + Math.min(i + 1, flat.length) + " of " + flat.length + (lit && lit.sub ? " · inside " + lit.sub : "");
      }
      const handler = (e) => {
        const b = e.target.closest(".act"); if (!b || lock) return;
        const want = flat[i], id = b.dataset.id;
        if (id === want.id) {
          if (!id.startsWith("call:")) done.push(id);
          SOUND.play(id.startsWith("call:") ? "whoosh" : "pop", 0.4); i++; draw();
          const line = id.startsWith("call:") ? "Do " + want.sub + ": jump into the " + want.sub + " sub-routine." : want.label + "." + (want.sub && i < flat.length && !flat[i].sub ? " That is the end of " + want.sub + ". Back to the main algorithm." : "");
          $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
          if (i >= flat.length) { lock = true; draw(); $(el.stage).removeEventListener("click", handler); reportScore(o.finish, Math.max(0, flat.length - wrong), flat.length); setTimeout(next, 2400); }
        } else {
          wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
          const line = want.id.startsWith("call:") ? "The main algorithm says do " + want.sub + " next: tap the call." : "Not yet. Next comes " + want.label + (want.sub ? ", inside " + want.sub : ", back in the main algorithm") + ".";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          setTimeout(() => b.classList.remove("wrong"), 700);
        }
      };
      $(el.stage).addEventListener("click", handler);
      $(el.ask).innerHTML = "Follow the main algorithm. When it says <b>do</b> a sub-routine, tap the call, then that sub-routine's steps, then carry on.";
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      sayHere(o.finish, "Follow the main algorithm. When it says do a sub-routine, tap the call, then that sub-routine's steps, then carry on where you left off.");
      draw();
    }
    function next() { r++; if (r >= rounds.length) endStep(o, "You followed " + rounds.length + " algorithms with sub-routines. " + o.done); else start(); }
    start();
  }

  /* ---- different outputs from different inputs (4CT.09) ---------------
     An if-algorithm: the same steps before and after, and a branch in the
     middle that depends on the input. The child chooses the input, then
     follows the branch that input takes; then tries the other input and
     sees a different output from the same algorithm. */
  function branchAlgo(o) {
    /* `word` labels the fork: IF at Stage 5, and at Stage 4 whatever the grade calls
       asking a question about the input (the owner's decision, 2026-09-12) */
    const el = o.el, rounds = o.rounds, word = o.word || "IF";
    let r = 0, score = 0;
    function start() {
      const rd = rounds[r], tried = new Set();
      let input = null, flat = [], i = 0, done = [], wrong = 0, lock = false;
      const all = distinctSteps((rd.before || []).concat(rd.yes, rd.no, rd.after || []));
      const btns = shuffle(all);
      function draw() {
        const lit = flat[i];
        const li = (st, on) => '<li class="' + (on ? "now" : "") + '"><span class="apic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label) + "</li>";
        const isLit = (st) => lit && lit.id === st.id && lit.label === st.label;
        $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(rd.task) + '</b></p><div class="branch">' +
          '<ol class="algo">' + (rd.before || []).map((st) => li(st, isLit(st))).join("") + "</ol>" +
          '<div class="ifbox"><b>' + esc(word) + " " + esc(rd.question) + '</b><div class="arms"><div class="arm' + (input === rd.inputs[0].id ? " on" : "") + '"><i>' + esc(rd.inputs[0].label) + ':</i><ol class="algo small">' + rd.yes.map((st) => li(st, input === rd.inputs[0].id && isLit(st))).join("") + '</ol></div><div class="arm' + (input === rd.inputs[1].id ? " on" : "") + '"><i>' + esc(rd.inputs[1].label) + ':</i><ol class="algo small">' + rd.no.map((st) => li(st, input === rd.inputs[1].id && isLit(st))).join("") + "</ol></div></div></div>" +
          '<ol class="algo">' + (rd.after || []).map((st) => li(st, isLit(st))).join("") + "</ol></div>" +
          (input ? '<div class="actions">' + btns.map((st) => '<button type="button" class="tapcard act" data-id="' + esc(st.id) + '"' + (lock ? " disabled" : "") + '><span class="cpic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label) + "</button>").join("") + "</div>"
            : '<div class="chips">' + rd.inputs.map((x) => '<button type="button" class="chip' + (tried.has(x.id) ? " tried" : "") + '" data-input="' + esc(x.id) + '"' + (tried.has(x.id) ? " disabled" : "") + '>' + small(x.pic) + " " + esc(x.label) + "</button>").join("") + "</div>") + "</div>";
        $(el.score).textContent = "Round " + (r + 1) + " of " + rounds.length + " · " + tried.size + " of 2 inputs tried" + (input ? " · step " + Math.min(i + 1, flat.length) + " of " + flat.length : "");
      }
      const handler = (e) => {
        const inp = e.target.closest("[data-input]");
        if (inp && !input) {
          input = inp.dataset.input; flat = branchRun(rd, input); i = 0; done = []; SOUND.play("click", 0.3); draw();
          const chosen = rd.inputs.find((x) => x.id === input);
          $(el.ask).innerHTML = "The input is <b>" + esc(chosen.label) + "</b>. Follow the algorithm: tap what comes next.";
          $(el.fb).textContent = ""; $(el.fb).className = "fb"; say("The input is " + chosen.label + ". Follow the algorithm. Tap what comes next.");
          return;
        }
        const b = e.target.closest(".act"); if (!b || lock || !input) return;
        const want = flat[i];
        if (b.dataset.id === want.id) {
          done.push(want.id); SOUND.play("pop", 0.4); i++; draw();
          $(el.fb).className = "fb good"; $(el.fb).textContent = want.label + "."; say(want.label);
          if (i >= flat.length) {
            tried.add(input); lock = true; draw();
            const chosen = rd.inputs.find((x) => x.id === input);
            const line = "With the input " + chosen.label + ", the output was: " + flat.map((st) => st.label).join(", ") + ".";
            $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
            setTimeout(() => {
              if (tried.size >= 2) { $(el.stage).removeEventListener("click", handler); reportScore(o.finish, Math.max(0, 2 * flat.length - wrong), 2 * flat.length); next(); }
              else { input = null; lock = false; draw(); $(el.ask).innerHTML = "Now try the <b>other</b> input. Same algorithm, different output."; sayHere(o.finish, "Now try the other input. Same algorithm, different output."); }
            }, 3400);
          }
        } else {
          wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
          const chosen = rd.inputs.find((x) => x.id === input);
          const line = "Not with this input. The input is " + chosen.label + ", so next comes " + want.label + ".";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          setTimeout(() => b.classList.remove("wrong"), 700);
        }
      };
      $(el.stage).addEventListener("click", handler);
      $(el.ask).innerHTML = "Choose an <b>input</b>, then follow the algorithm to see what it outputs.";
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      sayHere(o.finish, "Choose an input, then follow the algorithm to see what it outputs.");
      draw();
    }
    function next() { r++; if (r >= rounds.length) endStep(o, "One algorithm, two inputs, two different outputs - " + rounds.length + " times over. " + o.done); else start(); }
    start();
  }

  /* ---- build an algorithm with a repeat in it (4CT.10) ------------------
     A task that needs the same steps done N times, then something else.
     The child puts steps into the repeat box, sets how many times, and
     runs it: the box unrolls into the flat list and the page checks that
     it matches the task. expandLoop decides, mirrored in _rules.py. */
  function loopBuild(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, score = 0, body = [], times = 1, firstRun = true, lock = false;
    const round = () => rounds[r];
    const stepOf = (id) => round().pool.find((st) => st.id === id);
    function draw() {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">The task: <b>' + esc(rd.task) + '</b></p><div class="loopbuild"><div class="loopbox2"><b>repeat <button type="button" class="numchip" id="' + el.stage + 'n" aria-label="times; tap to change"' + (lock ? " disabled" : "") + ">" + times + ' times</button>:</b>' +
        '<ol class="algo inner" id="' + el.stage + 'body">' + (body.length ? body.map((id, k) => '<li><button type="button" class="algostep" data-rm="' + k + '"' + (lock ? " disabled" : "") + '><span class="apic" aria-hidden="true">' + small(stepOf(id).pic) + "</span>" + esc(stepOf(id).label) + ' <small>(tap to remove)</small></button></li>').join("") : '<li class="dim">tap steps below to put them in the loop</li>') + "</ol></div>" +
        (rd.after && rd.after.length ? '<p class="sub">then: ' + rd.after.map((st) => esc(st.label)).join(", ") + "</p>" : "") + "</div>" +
        '<div class="cardsgrid">' + rd.pool.map((st) => '<button type="button" class="tapcard" data-add="' + esc(st.id) + '"' + (lock ? " disabled" : "") + '><span class="cpic" aria-hidden="true">' + small(st.pic) + "</span>" + esc(st.label) + "</button>").join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small teal" id="' + el.stage + 'run"' + (lock || !body.length ? " disabled" : "") + ">&#9654; Run</button></div></div>";
      $(el.score).textContent = "Algorithm " + (r + 1) + " of " + rounds.length + " · " + body.length + " step" + (body.length === 1 ? "" : "s") + " in the loop, " + times + " time" + (times === 1 ? "" : "s");
      $(el.stage + "n").addEventListener("click", () => { times = (times % 5) + 1; SOUND.play("click", 0.3); draw(); });
      $(el.stage + "run").addEventListener("click", run);
    }
    function start() {
      body = []; times = 1; firstRun = true; lock = false;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      draw();
      $(el.ask).innerHTML = "Put the steps that repeat into the <b>repeat</b> box, set how many times, then press <b>Run</b>.";
      sayHere(o.finish, "The task: " + round().task + ". Put the steps that repeat into the repeat box, set how many times, then press Run.");
    }
    function run() {
      if (lock || !body.length) return;
      const rd = round(), after = (rd.after || []).map((st) => st.id);
      const did = expandLoop([], body, times, after), want = expandLoop([], rd.expect.body, rd.expect.times, after);
      const same = did.length === want.length && did.every((id, k) => id === want[k]);
      lock = true; draw();
      if (same) {
        if (firstRun) score++; SOUND.play("tada", 0.5);
        const line = cheer() + " Repeat " + times + " times, " + body.map((id) => stepOf(id).label).join(", ") + (after.length ? ", then " + rd.after.map((st) => st.label).join(", ") : "") + ". That is " + did.length + " steps written as " + (body.length + after.length + 1) + ".";
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        r++;
        setTimeout(() => { if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You built " + rounds.length + " algorithms with a repeat in them. " + o.done); } else start(); }, 3400);
      } else {
        firstRun = false; lock = false; draw(); SOUND.play("error", 0.3);
        const line = "Unrolled, that is " + did.length + " steps: " + did.map((id) => (stepOf(id) || { label: id }).label).join(", ") + ". The task needs " + want.length + ". " + (rd.hint || "Check what goes in the loop and how many times.");
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    }
    $(el.stage).addEventListener("click", (e) => {
      if (lock) return;
      const add = e.target.closest("[data-add]"); if (add) { body.push(add.dataset.add); SOUND.play("click", 0.3); draw(); return; }
      const rm = e.target.closest("[data-rm]"); if (rm) { body.splice(Number(rm.dataset.rm), 1); SOUND.play("pop", 0.3); draw(); }
    });
    start();
  }

  /* ---- write an algorithm with a branch in it (4CT.09) ------------------
     The follow machine above reads an algorithm that forks on its input;
     this one has the child DEVELOP it. First the question that decides the
     middle (when the round offers a choice of questions), then every step
     put where it belongs: first, in one answer's branch or the other's, or
     after. Test it runs the child's own algorithm with both inputs, so a
     misplaced step shows up as the wrong output for one input; the page
     names it, says what the test showed, and hands it back to be placed
     again. branch_home in _rules.py is the key the gate checks. */
  function branchBuild(o) {
    const el = o.el, rounds = o.rounds, word = o.word || "IF", ZONES = ["before", "yes", "no", "after"];
    let r = 0, score = 0;
    function start() {
      const rd = rounds[r], all = [], home = {};
      ZONES.forEach((z) => (rd[z] || []).forEach((st) => { all.push(st); home[st.id] = z; }));
      const stepOf = (id) => all.find((st) => st.id === id);
      const inputOf = (z) => rd.inputs[z === "no" ? 1 : 0];
      const pool = shuffle(all.map((st) => st.id));
      const placed = { before: [], yes: [], no: [], after: [] };
      const qs = rd.questions ? shuffle(rd.questions.map((t, k) => ({ t: t, k: k }))) : null;
      let asked = !qs, picked = null, lock = false, firstTest = true;
      const where = (id) => ZONES.find((z) => placed[z].indexOf(id) >= 0);
      const free = () => pool.filter((id) => !where(id));
      function zone(z, cls) {
        const rows = placed[z].map((id) => '<li><button type="button" class="algostep" data-rm="' + esc(id) + '"' + (lock ? " disabled" : "") + '><span class="apic" aria-hidden="true">' + small(stepOf(id).pic) + "</span>" + esc(stepOf(id).label) + " <small>(tap to take out)</small></button></li>").join("");
        /* four buttons all named "Put it here" tell a screen-reader user nothing;
           each says where here is */
        const where = z === "before" ? "first, every time" : z === "after" ? "after the branch, every time" : "in the branch for " + inputOf(z).label;
        return '<ol class="algo' + (cls || "") + '">' + (rows || '<li class="dim">nothing here yet</li>') + "</ol>" +
          '<button type="button" class="zonebtn" data-zone="' + z + '" aria-label="Put it ' + esc(where) + '"' + (picked && !lock ? "" : " disabled") + ">Put it here</button>";
      }
      function draw() {
        const head = '<p class="goal">The task: <b>' + esc(rd.task) + "</b></p>";
        if (!asked) {
          $(el.stage).innerHTML = '<div class="stagewide">' + head + '<div class="chips">' + qs.map((x) => '<button type="button" class="chip" data-q="' + x.k + '">' + esc(x.t) + "</button>").join("") + "</div></div>";
          $(el.score).textContent = "Algorithm " + (r + 1) + " of " + rounds.length + " · choose the question";
          return;
        }
        const left = free();
        $(el.stage).innerHTML = '<div class="stagewide">' + head + '<div class="branch build">' +
          '<div class="zonebox"><b>First, every time:</b>' + zone("before") + "</div>" +
          '<div class="ifbox"><b>' + esc(word) + " " + esc(rd.question) + '</b><div class="arms">' +
          ["yes", "no"].map((z) => '<div class="arm on"><i>' + esc(inputOf(z).label) + ":</i>" + zone(z, " small") + "</div>").join("") + "</div></div>" +
          '<div class="zonebox"><b>Then, every time:</b>' + zone("after") + "</div></div>" +
          (left.length ? '<div class="cardsgrid">' + left.map((id) => '<button type="button" class="tapcard' + (picked === id ? " now" : "") + '" data-pick="' + esc(id) + '" aria-pressed="' + (picked === id) + '"' + (lock ? " disabled" : "") + '><span class="cpic" aria-hidden="true">' + small(stepOf(id).pic) + "</span>" + esc(stepOf(id).label) + "</button>").join("") + "</div>" : "") +
          '<div class="bigbtns"><button type="button" class="big small teal" id="' + el.stage + 'test"' + (lock || left.length ? " disabled" : "") + ">&#9654; Test it</button></div></div>";
        $(el.score).textContent = "Algorithm " + (r + 1) + " of " + rounds.length + " · " + (all.length - left.length) + " of " + all.length + " steps placed";
      }
      function refocus(e, sel) {
        /* a keyboard press (detail 0) keeps the child's place after the redraw */
        if (e && e.detail === 0) { const f = $(el.stage).querySelector(sel); if (f) f.focus(); }
      }
      function advice(id) {
        const h = home[id], z = where(id), label = stepOf(id).label;
        if (h === "before" || h === "after") {
          if (z === "yes" || z === "no") return "With the input " + inputOf(z === "yes" ? "no" : "yes").label + ", your algorithm never did " + label + ". It happens every time, so it goes outside the branches, " + (h === "before" ? "first." : "after them.");
          return h === "before" ? label + " ran after the branch, but it has to happen first, before the question." : label + " ran first, but it belongs after the branch.";
        }
        const other = h === "yes" ? "no" : "yes";
        if (z === other) return "With the input " + inputOf(h).label + ", your algorithm never did " + label + ", and with " + inputOf(other).label + " it did. It belongs in the branch for " + inputOf(h).label + ".";
        return "With the input " + inputOf(other).label + ", your algorithm did " + label + " too. It happens only for " + inputOf(h).label + ", so it goes in that branch.";
      }
      function next() {
        $(el.stage).removeEventListener("click", handler);
        r++;
        if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You wrote " + rounds.length + " algorithms that give different outputs for different inputs. " + o.done); }
        else start();
      }
      function test() {
        if (lock || free().length) return;
        const wrong = all.map((st) => st.id).filter((id) => where(id) !== home[id]);
        const out = ["yes", "no"].map((z) => "With the input " + inputOf(z).label + ", the output is: " + placed.before.concat(placed[z], placed.after).map((id) => stepOf(id).label).join(", ") + ".");
        if (!wrong.length) {
          lock = true; draw(); if (firstTest) score++; SOUND.play("tada", 0.5);
          const line = cheer() + " " + out.join(" ") + " One algorithm, and the input decides the output.";
          $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
          setTimeout(next, 4200);
          return;
        }
        firstTest = false; SOUND.play("error", 0.3);
        const line = "Not yet. " + advice(wrong[0]) + (wrong.length > 1 ? " " + (wrong.length - 1) + " more step" + (wrong.length > 2 ? "s were" : " was") + " in the wrong place too." : "") + " They are back in the pile: put them where they belong and test again.";
        wrong.forEach((id) => { const z = where(id); placed[z].splice(placed[z].indexOf(id), 1); });
        draw(); $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
      const handler = (e) => {
        if (lock) return;
        const qb = e.target.closest("[data-q]");
        if (qb && !asked) {
          const t = rd.questions[Number(qb.dataset.q)];
          if (t === rd.question) {
            asked = true; SOUND.play("ding", 0.4); draw(); refocus(e, "[data-pick]");
            $(el.ask).innerHTML = "Tap a step, then <b>Put it here</b> where it belongs. When every step is placed, press <b>Test it</b>.";
            const line = cheer() + " The answer to " + rd.question + " decides the middle. Now write the algorithm: tap a step, then Put it here where it belongs, and Test it.";
            $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
          } else {
            qb.classList.add("wrong"); qb.disabled = true; SOUND.play("error", 0.3);
            const line = "The answer to " + t + " would not change any step of " + rd.task + ". Which question would?";
            $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
          }
          return;
        }
        const pk = e.target.closest("[data-pick]");
        if (pk) { picked = picked === pk.dataset.pick ? null : pk.dataset.pick; SOUND.play("click", 0.3); draw(); refocus(e, picked ? ".zonebtn" : "[data-pick]"); return; }
        const zb = e.target.closest("[data-zone]");
        if (zb && picked) { placed[zb.dataset.zone].push(picked); picked = null; SOUND.play("pop", 0.4); $(el.fb).className = "fb"; $(el.fb).textContent = ""; draw(); refocus(e, free().length ? "[data-pick]" : "#" + el.stage + "test"); return; }
        const rm = e.target.closest("[data-rm]");
        if (rm) { const id = rm.dataset.rm, z = where(id); placed[z].splice(placed[z].indexOf(id), 1); SOUND.play("pop", 0.3); draw(); refocus(e, "[data-pick]"); return; }
        if (e.target.closest("#" + el.stage + "test")) test();
      };
      $(el.stage).addEventListener("click", handler);
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      $(el.ask).innerHTML = asked ? "Tap a step, then <b>Put it here</b> where it belongs. When every step is placed, press <b>Test it</b>." : "First choose the <b>question</b> this algorithm needs to ask.";
      sayHere(o.finish, "The task: " + rd.task + ". " + (asked ? "Tap a step, then Put it here where it belongs. When every step is placed, press Test it." : "First choose the question this algorithm needs to ask."));
      draw();
    }
    start();
  }

  /* ---- comments on blocks of code (4P.01) --------------------------------
     A program and a set of comments. The child puts each comment on the
     block it explains; the page shows them beside the code, then asks why
     programmers write them. */
  function commentBlocks(o) {
    const el = o.el, program = o.program, comments = o.comments, placed = {};
    let picked = -1, wrong = 0, lock = false;
    function draw() {
      $(el.stage).innerHTML = '<div class="stagewide"><div class="commented">' + program.map((id, k) => '<div class="crow">' + (placed[k] != null ? '<span class="ccomment">' + esc(comments[placed[k]].text) + "</span>" : '<span class="ccomment empty">no comment yet</span>') + blockBtn(id, 'data-k="' + k + '"' + (lock || placed[k] != null ? " disabled" : ""), placed[k] != null ? "right" : "") + "</div>").join("") + "</div>" +
        '<div class="chips">' + comments.map((c, i) => '<button type="button" class="chip comment' + (picked === i ? " on" : "") + '" data-c="' + i + '"' + (lock || Object.values(placed).includes(i) ? " disabled" : "") + ">" + esc(c.text) + "</button>").join("") + "</div></div>";
      $(el.score).textContent = Object.keys(placed).length + " of " + comments.length + " comments placed";
    }
    $(el.stage).addEventListener("click", (e) => {
      if (lock) return;
      const c = e.target.closest("[data-c]");
      if (c) { picked = Number(c.dataset.c); SOUND.play("click", 0.3); draw(); $(el.fb).className = "fb"; $(el.fb).textContent = "Comment picked. Now tap the block it explains."; return; }
      const b = e.target.closest(".block[data-k]"); if (!b || picked < 0) return;
      const k = Number(b.dataset.k);
      if (comments[picked].block === k) {
        placed[k] = picked; picked = -1; SOUND.play("ding", 0.4); draw();
        $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " That comment explains that block."; say(cheer() + " That comment explains that block.");
        if (Object.keys(placed).length >= comments.length) {
          lock = true; draw();
          setTimeout(() => { $(el.fb).textContent = ""; askOnce(o, o.then, (ok) => { reportScore(o.finish, (ok ? 1 : 0) + Math.max(0, comments.length - wrong), comments.length + 1); endStep(o); }); }, 2200);
        }
      } else {
        wrong++; b.classList.add("wrong"); SOUND.play("error", 0.3);
        const line = "That comment does not explain " + ((BLOCKS[program[k]] || {}).label || program[k]) + ". Read the comment again: which block does it describe?";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        setTimeout(() => b.classList.remove("wrong"), 700);
      }
    });
  ...
  /* ---- different outputs from different inputs, in a program (4P.05) ----
     One sprite, two inputs on the stage (press A, press B). The child
     builds a script for each input, then presses the inputs to test:
     each input runs only its own script. */
  function inputProgram(o) {
    const el = o.el, rounds = o.rounds, inputs = o.inputs;
    let r = 0, scripts = {}, active = inputs[0].id, running = false, tested = {}, firstRun = true, score = 0, sprite = null;
    const round = () => rounds[r];
    function draw(now) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><div class="spritestage" id="' + el.stage + 'sp"></div>' +
        '<div class="inputs">' + inputs.map((x) => '<button type="button" class="big small" data-in="' + esc(x.id) + '"' + (running ? " disabled" : "") + ">" + small(x.pic) + " " + esc(x.label) + (tested[x.id] === true ? " ✓" : "") + "</button>").join("") + "</div>" +
        '<div class="objtabs">' + inputs.map((x) => '<button type="button" class="objtab' + (active === x.id ? " on" : "") + '" data-tab="' + esc(x.id) + '"' + (running ? " disabled" : "") + '>when ' + esc(x.label) + "</button>").join("") + "</div>" +
        '<p class="goal">When <b>' + esc(inputs.find((x) => x.id === active).label) + "</b>, the algorithm says: <b>" + esc(rd.scripts[active].algorithm.join(", ")) + "</b></p>" +
        scriptHtml(scripts[active], now, !running) +
        '<div class="palette">' + (o.blocks || Object.keys(BLOCKS)).map((id) => blockBtn(id, 'data-add="' + id + '"' + (running ? " disabled" : ""))).join("") + "</div>" +
        '<div class="bigbtns"><button type="button" class="big small ghost" id="' + el.stage + 'clear"' + (running || !scripts[active].length ? " disabled" : "") + ">Clear</button></div></div>";
      sprite = spriteStage($(el.stage + "sp"), o.sprite || "\u{1F431}");
      $(el.score).textContent = "Program " + (r + 1) + " of " + rounds.length + " · " + inputs.map((x) => x.label + " " + scripts[x.id].length).join(" · ");
      $(el.stage + "clear").addEventListener("click", () => { scripts[active] = []; tested[active] = undefined; draw(-1); });
    }
    function start() {
      scripts = {}; tested = {}; inputs.forEach((x) => { scripts[x.id] = []; }); active = inputs[0].id; running = false; firstRun = true;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      draw(-1);
      $(el.ask).innerHTML = "Build a script for <b>each</b> input (tap its tab). Then press the inputs to test them.";
      sayHere(o.finish, "Build a script for each input. Tap an input's tab to see its algorithm. Then press the inputs on the stage to test them.");
    }
    function trigger(id) {
      if (running) return;
      const s = scripts[id];
      if (!s.length) { const line = "Nothing happened: there is no script for " + inputs.find((x) => x.id === id).label + " yet."; $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); firstRun = false; return; }
      running = true; sprite.reset(); draw(-1);
      const moves = expandProgram(s), rd = round();
      let i = 0;
      const step = () => {
        if (i >= moves.length) {
          running = false;
          const want = expandProgram(rd.scripts[id].expect);
          ...
          if (inputs.every((x) => tested[x.id] === true)) {
            ...
            $(el.stage).querySelectorAll("button").forEach((b) => { b.disabled = true; });
            r++;
            setTimeout(() => { if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "Two inputs, two scripts, two different outputs, " + rounds.length + " times. " + o.done); } else start(); }, 3200);
          }
          return;
        }
        draw(i < s.length ? i : -1);
      ...
      SOUND.play("click", 0.3); setTimeout(step, 300);
    }
    $(el.stage).addEventListener("click", (e) => {
      if (running) return;
      const inp = e.target.closest("[data-in]"); if (inp) { trigger(inp.dataset.in); return; }
      const tab = e.target.closest("[data-tab]"); if (tab) { active = tab.dataset.tab; SOUND.play("click", 0.3); draw(-1); return; }
      const add = e.target.closest("[data-add]"); if (add) { scripts[active].push(add.dataset.add); tested[active] = undefined; SOUND.play("click", 0.3); draw(-1); return; }
      const placed = e.target.closest(".script [data-k]"); if (placed) { scripts[active].splice(Number(placed.dataset.k), 1); tested[active] = undefined; SOUND.play("pop", 0.3); draw(-1); }
    });
    start();
  }

  /* ---- plan the instructions for objects: inputs and outputs (4P.06) ----
     Before a program is built, each object gets a plan: what starts it (its
     input) and what it does (its output). Two questions per object. */
  function planObjects(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, right = 0;
    function draw(stage) {
      const rd = rounds[r];
      $(el.stage).innerHTML = '<div class="stagewide"><div class="plancard"><span class="ppic" aria-hidden="true">' + small(rd.pic) + '</span><div><b>' + esc(rd.object) + "</b><p>Goal: " + esc(rd.goal) + '</p><p class="planline">Input: <b>' + (stage > 0 ? esc(rd.input.opts.find((x) => x.ok).t) : "?") + '</b></p><p class="planline">Output: <b>' + (stage > 1 ? esc(rd.output.opts.find((x) => x.ok).t) : "?") + "</b></p></div></div></div>";
      $(el.score).textContent = "Object " + (r + 1) + " of " + rounds.length;
    }
    function start() {
      const rd = rounds[r];
      draw(0); $(el.fb).textContent = ""; $(el.fb).className = "fb";
      askOnce(o, rd.input, (ok1) => {
        if (ok1) right++;
        draw(1);
        askOnce(o, rd.output, (ok2) => {
          if (ok2) right++;
          draw(2);
          r++;
          if (r >= rounds.length) { reportScore(o.finish, right, rounds.length * 2); endStep(o, "Every object planned: an input and an output each. " + o.done); }
          else start();
        });
      });
    }
  ...
  /* ---- test a program part by part (4P.07) -------------------------------
     A program in named parts. Each part runs on its own so the child can
     see WHICH part goes wrong, tap the wrong block in that part, fix it,
     and run just that part again. When every part passes, the program does. */
  function partTest(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, parts = [], status = [], running = false, phase = "test", bugPart = -1, score = 0, sprite = null;
    const round = () => rounds[r];
    function draw(now) {
      const rd = round();
      $(el.stage).innerHTML = '<div class="stagewide"><p class="goal">We want: <b>' + esc(rd.goal) + '</b></p><div class="spritestage" id="' + el.stage + 'sp"></div>' +
        '<div class="parts">' + parts.map((p, pi) => '<div class="part' + (status[pi] === true ? " ok" : status[pi] === false ? " bad" : "") + '"><div class="parthead"><b>' + esc(p.name) + "</b><span>" + esc(p.wants) + '</span><button type="button" class="big small' + (status[pi] === true ? " ghost" : " teal") + '" data-run="' + pi + '"' + (running || phase === "fix" ? " disabled" : "") + '>&#9654; Run part</button></div>' +
          '<div class="script">' + p.program.map((id, k) => blockBtn(id, 'data-p="' + pi + '" data-k="' + k + '"' + (!(phase === "find" && bugPart === pi) ? " disabled" : ""), now && now[0] === pi && now[1] === k ? "now" : "")).join("") + "</div>" +
          '<span class="partstat">' + (status[pi] === true ? "✓ this part works" : status[pi] === false ? "✗ this part goes wrong" : "not tested yet") + "</span></div>").join("") + "</div></div>";
      sprite = spriteStage($(el.stage + "sp"), o.sprite || "\u{1F431}");
      $(el.score).textContent = "Program " + (r + 1) + " of " + rounds.length + " · " + status.filter((s) => s === true).length + " of " + parts.length + " parts pass";
    }
    function start() {
      parts = round().parts.map((p) => Object.assign({}, p, { program: p.program.slice() })); status = parts.map(() => undefined); running = false; phase = "test"; bugPart = -1;
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      draw(null);
      $(el.ask).innerHTML = "Test each part on its own: press <b>Run part</b>. A part that goes wrong is where the bug is.";
      sayHere(o.finish, "Test each part on its own. Press Run part. A part that goes wrong is where the bug is.");
    }
    function runPart(pi) {
      if (running || phase === "fix") return;
      running = true; sprite.reset(); draw(null);
      const p = parts[pi], moves = expandProgram(p.program);
        ...
        if (i >= moves.length) {
          ...
          if (ok) {
            ...
            if (status.every((s) => s === true)) {
              ...
              $(el.stage).querySelectorAll("button").forEach((b) => { b.disabled = true; });
              r++;
              setTimeout(() => { if (r >= rounds.length) { reportScore(o.finish, score, rounds.length); endStep(o, "You tested " + rounds.length + " programs part by part and found every bug where it lived. " + o.done); } else start(); }, 3200);
            }
          } else {
    ...
    $(el.stage).addEventListener("click", (e) => {
      const run = e.target.closest("[data-run]"); if (run) { runPart(Number(run.dataset.run)); return; }
      const b = e.target.closest(".block[data-p]"); if (!b || phase !== "find") return;
      const pi = Number(b.dataset.p), k = Number(b.dataset.k), p = parts[pi];
      if (pi === bugPart && k === p.bug) {
        phase = "fix"; b.classList.add("bug"); SOUND.play("ding", 0.4);
        $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + p.why; say(cheer() + " " + p.why + " What should it be instead?");
        $(el.ask).innerHTML = "What should that block be instead?";
        $(el.ch).className = "choices blocks";
        $(el.ch).innerHTML = shuffle(p.fix.opts).map((c) => blockBtn(c.id, 'data-id="' + esc(c.id) + '" data-ok="' + (c.ok ? 1 : 0) + '"')).join("");
      } else {
        b.classList.add("wrong"); SOUND.play("error", 0.3);
        const line = "That block is fine. Which block in " + parts[bugPart].name + " made it go wrong?";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        setTimeout(() => b.classList.remove("wrong"), 700);
      }
    });
    $(el.ch).addEventListener("click", (e) => {
      const b = e.target.closest("[data-id]"); if (!b || phase !== "fix") return;
      const p = parts[bugPart];
      if (b.dataset.ok === "1") {
        p.program[p.bug] = b.dataset.id; phase = "test"; status[bugPart] = undefined; SOUND.play("pop", 0.4);
        $(el.ch).innerHTML = ""; draw(null);
        $(el.ask).innerHTML = "Fixed. Now <b>run that part again</b> to test the fix.";
        $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + p.fix.why; say(cheer() + " " + p.fix.why + " Now run that part again to test it.");
      } else {
        b.classList.add("wrong"); b.disabled = true; SOUND.play("error", 0.3);
        const line = "That block would not make " + p.wants.toLowerCase() + ". Try another.";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    });
    start();
  ...
  /* ---- sort data into a required order (4MD.04) --------------------------
     A table; the child chooses a field and a direction, presses Sort, and
     the rows reorder - numbers by size, words alphabetically. Then a
     question the sorted table answers; sortRows keys it, mirrored in
     _rules.py. */
  function dataSort(o) {
    const el = o.el, rows = o.rows, fields = o.fields, tasks = o.tasks;
    let t = 0, right = 0, field = null, dir = "asc", ordered = rows.slice(), sorted = false, lock = false, wrongHere = false;
    const fieldOf = (id) => fields.find((f) => f.id === id);
    function draw() {
      const f = field && fieldOf(field);
      $(el.stage).innerHTML = '<div class="stagewide"><table class="rec small tally"><thead><tr><th>who</th>' + fields.map((x) => "<th>" + esc(x.label) + "</th>").join("") + "</tr></thead><tbody>" +
        ordered.map((rw) => '<tr><td><span class="rowlab">' + small(rw.pic) + " " + esc(rw.name) + "</span></td>" + fields.map((x) => '<td><span class="cell filled' + (field === x.id && sorted ? " sorted" : "") + '">' + esc(rw[x.id]) + "</span></td>").join("") + "</tr>").join("") + "</tbody></table>" +
        '<div class="filterrow"><div class="chips">' + fields.map((x) => '<button type="button" class="chip' + (field === x.id ? " on" : "") + '" data-field="' + x.id + '"' + (lock ? " disabled" : "") + ">by " + esc(x.label) + "</button>").join("") + "</div>" +
        (f ? '<div class="chips"><button type="button" class="chip' + (dir === "asc" ? " on" : "") + '" data-dir="asc"' + (lock ? " disabled" : "") + ">" + (f.numeric ? "1 → 9 (ascending)" : "A → Z") + '</button><button type="button" class="chip' + (dir === "desc" ? " on" : "") + '" data-dir="desc"' + (lock ? " disabled" : "") + ">" + (f.numeric ? "9 → 1 (descending)" : "Z → A") + "</button></div>" : "") +
        '<div class="bigbtns"><button type="button" class="big small teal" id="' + el.stage + 'go"' + (lock || !field ? " disabled" : "") + ">Sort</button></div></div></div>";
      $(el.score).textContent = t < tasks.length ? "Sort " + (t + 1) + " of " + tasks.length : "";
      $(el.stage + "go").addEventListener("click", doSort);
    }
    function ask() {
      const task = tasks[t];
      field = null; dir = "asc"; ordered = shuffle(rows); sorted = false; lock = false; wrongHere = false; draw();
      $(el.ask).innerHTML = task.ask + " Choose the field and the order, then press <b>Sort</b>.";
      $(el.fb).textContent = ""; $(el.fb).className = "fb"; $(el.ch).innerHTML = "";
      sayHere(o.finish, plain(task.ask) + " Choose the field and the order, then press Sort.");
    }
    function doSort() {
      if (lock || !field) return;
      const task = tasks[t];
      ordered = sortRows(rows, field, dir); sorted = true; SOUND.play("whoosh", 0.4); draw();
      if (field === task.field && dir === task.dir) {
        lock = true; if (!wrongHere) right++; draw();
        const line = cheer() + " Sorted by " + fieldOf(field).label + ", " + (dir === "asc" ? (fieldOf(field).numeric ? "smallest first" : "A to Z") : (fieldOf(field).numeric ? "biggest first" : "Z to A")) + ": " + ordered.map((rw) => rw.name).join(", ") + ".";
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        setTimeout(() => {
          $(el.fb).textContent = "";
          askOnce(o, task.then, (ok) => { if (ok) right++; t++; if (t >= tasks.length) { reportScore(o.finish, right, tasks.length * 2); endStep(o, "You sorted the data " + tasks.length + " ways and read off the answers. " + o.done); } else ask(); });
        }, 3000);
      } else {
        wrongHere = true; SOUND.play("error", 0.3);
        const line = "That sorted by " + fieldOf(field).label + ", " + (dir === "asc" ? "ascending" : "descending") + ". Read the task again: which field, and which way round?";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    }
    $(el.stage).addEventListener("click", (e) => {
      if (lock) return;
      const f = e.target.closest("[data-field]"), d = e.target.closest("[data-dir]");
      if (f) { field = f.dataset.field; sorted = false; SOUND.play("click", 0.3); draw(); }
      else if (d) { dir = d.dataset.dir; sorted = false; SOUND.play("click", 0.3); draw(); }
    });
    ask();
  }

  /* ---- data, records and fields in a table (4MD.07) ----------------------
     A table with the three parts a child can point at: a record is a row,
     a field is a column, a piece of data is one cell. Tasks name one and
     the child taps it. */
  function tableParts(o) {
    const el = o.el, rows = o.rows, fields = o.fields, tasks = o.tasks;
    let t = 0, right = 0, lock = false, wrongHere = false;
    function draw(hl) {
      $(el.stage).innerHTML = '<div class="stagewide"><div class="sheetwrap"><table class="dbtable" role="grid"><thead><tr><th></th>' + fields.map((f) => '<th><button type="button" class="colhead' + (hl && hl.field === f.id && !hl.record ? " on" : "") + '" data-field="' + f.id + '">' + esc(f.label) + "</button></th>").join("") + "</tr></thead><tbody>" +
        rows.map((rw) => '<tr class="' + (hl && hl.record === rw.name && !hl.field ? "on" : "") + '"><th><button type="button" class="colhead rec" data-record="' + esc(rw.name) + '">' + small(rw.pic) + " " + esc(rw.name) + "</button></th>" + fields.map((f) => '<td><button type="button" class="cellbtn' + (hl && hl.record === rw.name && hl.field === f.id ? " on" : "") + '" data-cell="' + esc(rw.name) + "|" + f.id + '">' + esc(rw[f.id]) + "</button></td>").join("") + "</tr>").join("") + "</tbody></table></div></div>";
      $(el.score).textContent = t < tasks.length ? "Task " + (t + 1) + " of " + tasks.length : "";
    }
    function ask() {
      const task = tasks[t];
      lock = false; wrongHere = false; draw(null);
      $(el.ask).innerHTML = task.ask; $(el.fb).textContent = ""; $(el.fb).className = "fb";
      sayHere(o.finish, plain(task.ask));
    }
    function good(hl, line) {
      lock = true; if (!wrongHere) right++; SOUND.play("ding", 0.4); draw(hl);
      $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + line; say(cheer() + " " + line);
      t++;
      setTimeout(() => { if (t >= tasks.length) { reportScore(o.finish, right, tasks.length); endStep(o, "Records, fields and data: you can point at all three. " + o.done); } else ask(); }, 2800);
    }
    function bad(line) { wrongHere = true; SOUND.play("error", 0.3); $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line); }
    $(el.stage).addEventListener("click", (e) => {
      if (lock || t >= tasks.length) return;
      const task = tasks[t];
      const rec = e.target.closest("[data-record]"), fld = e.target.closest("[data-field]"), cell = e.target.closest("[data-cell]");
      SOUND.play("click", 0.3);
      if (task.kind === "record") {
        if (rec && rec.dataset.record === task.target) good({ record: task.target }, "That row is " + task.target + "'s record: everything the table knows about " + task.target + ".");
        else bad(rec ? "That is " + rec.dataset.record + "'s record. Find " + task.target + "'s." : fld ? "That is a whole field, a column. A record is a row." : "That is one piece of data. A record is the whole row.");
      } else if (task.kind === "field") {
        const f = fields.find((x) => x.id === task.target);
        if (fld && fld.dataset.field === task.target) good({ field: task.target }, "That column is the " + f.label + " field: the same kind of fact for every record.");
        else bad(fld ? "That is the " + fields.find((x) => x.id === fld.dataset.field).label + " field. Find the " + f.label + " field, at the top of its column." : rec ? "That is a record, a row. A field is a column." : "That is one piece of data. A field is the whole column.");
      } else {
        const [name, fid] = task.target, f = fields.find((x) => x.id === fid), rw = rows.find((x) => x.name === name);
        if (cell && cell.dataset.cell === name + "|" + fid) good({ record: name, field: fid }, "That cell holds one piece of data: " + name + "'s " + f.label + " is " + rw[fid] + ".");
        else bad(cell ? "That is " + cell.dataset.cell.split("|")[0] + "'s " + fields.find((x) => x.id === cell.dataset.cell.split("|")[1]).label + ". Find " + name + "'s " + f.label + ": their row, that column." : "A piece of data is one cell: a row and a column meeting.");
      }
    });
    ask();
  }
