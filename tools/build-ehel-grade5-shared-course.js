...
const GRADE_DIR = path.join(ROOT, "src", "prototypes", "ehel-academy", "english", "grade-5");
const DATA_DIR = path.join(GRADE_DIR, "data");
const UNIT_DIR = path.join(DATA_DIR, "units");
const SOURCE_FILE = path.join(ROOT, "inputs", "ehel-grade5-source", "grade5-source-extracted.json");
...
const gradeProfile = templateConfig.gradeProfiles.g05;
if (!gradeProfile || gradeProfile.writingExpectation !== "2-3 organised paragraphs across genres") throw new Error("Grade 5 neutral profile is missing or invalid.");
...
const visualMap = {
  1: ["unit-8-home.png", "A Grade 5 learner reading a fable and reflecting on its moral lesson"],
  2: ["unit-4-big-sky.png", "Learners investigating space, discovery and teamwork"],
  3: ["unit-5-measure.png", "A learner comparing poems and recording thoughtful reflections"],
  4: ["unit-9-city.png", "Learners retelling myths and legends for an audience"],
  5: ["unit-3-ready-steady-go.png", "Learners following, testing and improving clear instructions"],
  6: ["unit-2-neighbours-jobs.png", "Learners exploring story genres and planning original writing"],
  7: ["unit-8-home.png", "Learners comparing narrators and retelling a story from another viewpoint"],
  8: ["unit-9-city.png", "Learners presenting persuasive views with evidence"],
  9: ["unit-3-ready-steady-go.png", "Learners rehearsing a play script for stage and camera"],
  10: ["capstone-my-english-world.png", "A Grade 5 learner presenting an independent English portfolio"],
};
...
function unitId(unitNo) {
  return `eng-g05-${termFor(unitNo).id}-u${String(unitNo).padStart(2, "0")}`;
}
...
function doc(unitNo, type) {
  ...
  if (!record) throw new Error(`Missing Grade 5 source: Unit ${unitNo} ${type}`);
  return record;
}
...
function sourcePath(unitNo, type) {
  return `inputs/ehel-grade5-source/Year 5/${doc(unitNo, type).path}`;
}
...
function extractVocabulary(unitNo) {
  ...
  return tables.map((table, groupIndex) => ({
    ...
    words: table.slice(1).filter((row) => {
      ...
      if (!key || seen.has(key) || total >= 40) return false;
      seen.add(key);
      total += 1;
      return true;
...
function extractReadings(unitNo) {
  ...
  const cleanLines = (paragraphs) => paragraphs.filter((line) => !/^(Year 5 English|After Reading|Understanding the Story|Comprehension|Reflection Questions|Activity \d+|Answer Key|Ask Your AI Tutor|Self-)/i.test(line) && !line.endsWith("?") && line.split(/\s+/).length > 5);
  ...
  const lessonChunks = chunk(cleanLines(lesson.paragraphs.slice(2)), 320).slice(0, 2);
  ...
  const storyChunks = chunk(cleanLines(story.paragraphs.slice(storyStart, storyEnd < 0 ? story.paragraphs.length : storyEnd)), 280).slice(0, 3);
  const readings = [
...
function grammarBlocks(unitNo) {
  ...
  while (blocks.length < 6) {
    ...
    blocks.push([
      ...
      `Practice ${sequence}`,
      "Edit a two-paragraph response using the unit language focus. Mark four changes and explain how two changes improve clarity.",
    ]);
  }
...
function grammarRecord(unitNo, block, sequence) {
  ...
  return {
    ...
    origin: "Authored Grade 5 source",
...
const extensionTasks = {
  speaking: [
    ...
    ["Three-Minute Unit Talk", "Plan and record a three-minute talk using an opening, three connected ideas with evidence or examples, and a purposeful closing."],
  ],
  writing: [
...
function buildTasks(unitNo, kind) {
  ...
  return tasks.slice(0, 6).map(([title, body], index) => {
    ...
    const common = { unitId: unitId(unitNo), sequence, title: `${kind[0].toUpperCase() + kind.slice(1)} ${sequence}: ${title}`, outcomeId: `${unitId(unitNo)}-lo${String(Math.min(sequence, 6)).padStart(2, "0")}`, origin: index < sourceTasks.length ? "Authored Grade 5 source" : "Ehel English Content Template v1.2 curriculum expansion", reviewStatus: "Approved v1.2", sourceFile: index < sourceTasks.length ? sourcePath(unitNo, "Lesson") : "Ehel English Content Template v1.2" };
    ...
    if (kind === "writing") return { writingId: id, ...common, promptAndInstructions: body, modelText: `A strong Grade 5 response uses organised paragraphs, precise unit words, relevant evidence or examples, and a purposeful conclusion about ${title.toLowerCase()}.`, sentenceStarter: "I would like to explain...", expectedLength: sequence <= 2 ? "Two organised paragraphs" : gradeProfile.writingExpectation, successCriteria: "I answered the prompt; organised ideas into paragraphs; used precise Unit vocabulary; supported ideas with details or evidence; checked cohesion and punctuation; revised my work", support: "Plan topic sentences and supporting details, then use the word bank and a source model.", extension: "Vary sentence structures, compare ideas, improve transitions and strengthen the conclusion.", rubricId: "rub-writing-v1" };
...
function extractOutcomeText(lesson, title) {
  ...
  const extensions = [
    ...
    `Use Grade 5 vocabulary accurately in discussion and writing.`,
    ...
    `Present a clear three-minute response and answer follow-up questions.`,
  ];
...
function buildRegularUnit(unitNo, dictionaryEntries) {
  ...
  groups.forEach((group, groupIndex) => {
    ...
    group.words.forEach((word, wordIndex) => {
      ...
      const practiceSentences = [word.example, `The word ${word.word} belongs to our ${group.title.toLowerCase()} vocabulary.`, `I can explain ${word.word} in my own words.`, `I can connect ${word.word} to this unit's texts.`, `I can use ${word.word} in a clear Grade 5 sentence.`];
      dictionaryLinks.push({ vocabularyId, dictionaryEntryId, senseId: `${dictionaryEntryId}-sense-01`, gradeId: "g05", termId: term.id, unitId: id, groupId, groupTitle: group.title, sequence: wordIndex + 1, childMeaning: word.meaning, exampleSentence: word.example, practiceSentences, sentenceAudio: practiceSentences.map((_, sentenceIndex) => pendingAudio(`./unit-${unitNo}/media/audio/vocabulary/${vocabularyId}-sentence-${sentenceIndex + 1}.mp3`)), sentenceStarter: `The ${word.word}`, spellingPractice: word.word.split("").join(" - "), aiTutorPrompt: `Ask me to define '${word.word}', use it in a sentence, and connect it to ${title}.`, reviewStatus: "Approved v1.2" });
...
function buildCapstone(regularUnits, dictionaryEntries) {
  ...
  const id = unitId(unitNo);
  const term = termFor(unitNo);
  const selectedLinks = regularUnits.flatMap((unit) => unit.dictionaryLinks.slice(0, 4)).slice(0, 30).map((link, index) => ({ ...link, vocabularyId: `u10-g1-${index + 1}-${slug(dictionaryEntries.get(link.dictionaryEntryId).displayWord)}`, unitId: id, termId: term.id, groupId: "u10-g1-grade-5-review-words", groupTitle: "Grade 5 Review Words", sequence: index + 1 }));
  const title = "My English Voice: Grade 5 Capstone";
  const outcomes = ["Select strong evidence from Units 1-9 and explain why it shows progress.", "Use Grade 5 vocabulary accurately across speaking and writing.", "Read a chosen passage fluently and answer questions with text evidence.", "Create a multi-section information or narrative product for a real audience.", "Present ideas clearly for three minutes, respond to questions and use feedback.", "Reflect honestly and set a specific Grade 6 English goal."].map((learningOutcome, index) => ({ outcomeId: `${id}-lo0${index + 1}`, unitId: id, sequence: index + 1, learningOutcome, bloomLevel: index < 2 ? "Apply" : index < 4 ? "Analyse and create" : "Evaluate", evidenceOfLearning: "Capstone portfolio, product, presentation and reflection", origin: "Ehel Grade 5 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  const makeSix = (kind, items) => items.map((item, index) => {
    ...
    const common = { unitId: id, sequence: index + 1, title: item[0], outcomeId: outcomes[index].outcomeId, origin: "Ehel Grade 5 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" };
    ...
    if (kind === "writing") return { writingId: itemId, ...common, promptAndInstructions: item[1], modelText: item[2], sentenceStarter: item[3], expectedLength: "Two to three organised paragraphs", successCriteria: "I addressed my audience; organised connected paragraphs; used precise vocabulary; supported ideas with evidence or examples; checked grammar and punctuation; revised from feedback", support: "Use a planning frame, word bank and one model from an earlier unit.", extension: "Vary sentence structures, strengthen transitions and write a purposeful conclusion.", rubricId: "rub-writing-v1" };
  ...
  const speaking = makeSix("speaking", [["Portfolio choice conference", "Explain three portfolio choices and support each choice with a specific example."], ["Fluent reading rehearsal", "Read a selected passage with clear phrasing, pace and expression."], ["Vocabulary in context", "Use ten review words naturally while explaining your project."], ["Three-minute presentation", "Present a clear opening, organised main points with evidence, and a strong closing."], ["Question-and-answer practice", "Answer six audience questions in complete, relevant sentences."], ["Final capstone presentation", "Deliver your polished presentation and record your reflection."]]);
  const writing = makeSix("writing", [["Portfolio captions", "Write a precise paragraph about each of three selected pieces and explain what each shows.", "This revised report shows that I can organise facts and support an explanation with evidence.", "This piece shows..."], ["Project proposal", "Explain your topic, audience, multi-section product and plan in two organised paragraphs.", "My project will teach younger learners about caring for nature. I will create an illustrated guide using facts and examples.", "My project is about..."], ["First draft", "Write the complete first draft of your capstone product with clear sections.", "Use a clear opening, organised sections and paragraphs, evidence or examples, and a conclusion suited to your audience.", "I want my audience to understand..."], ["Revision from feedback", "Record two feedback points and rewrite the sections they improve.", "Feedback asked for stronger evidence, so I added a detail from Unit 7 and explained why it matters.", "One improvement I made was..."], ["Presentation notes", "Prepare concise cue cards for a three-minute presentation.", "Opening; three main ideas; evidence; audience question; closing.", "Today I will share..."], ["Grade 5 reflection", "Explain your strongest progress, one challenge, and a specific Grade 6 goal.", "I became a stronger reader because I learned to compare evidence. Next, I will improve paragraph transitions.", "This year I learned..."]]);
  ...
  while (comprehension.length < 12) comprehension.push({ ...comprehension[comprehension.length % 10], questionId: `${id}-cq${String(comprehension.length + 1).padStart(3, "0")}`, sequence: comprehension.length + 1, question: comprehension.length === 10 ? "Compare two review texts. What important idea do they share?" : "Which Grade 5 text influenced your capstone most, and why?" });
  const quizzes = regularUnits.flatMap((unit) => unit.quizzes.slice(0, 2)).slice(0, 10).map((quiz, index) => ({ ...quiz, quizId: `${id}-quiz${String(index + 1).padStart(2, "0")}`, questionId: `${id}-quiz${String(index + 1).padStart(2, "0")}-q01`, unitId: id, quizTitle: "Grade 5 capstone review checkpoint", sequence: index + 1 }));
  const liveSessions = ["Launch and portfolio curation", "Reading and vocabulary conference", "Project planning and first draft", "Grammar editing and peer feedback", "Presentation rehearsal", "Capstone exhibition and reflection"].map((title, index) => ({ liveSessionId: `${id}-live0${index + 1}`, unitId: id, week: index < 3 ? 1 : 2, sessionWithinWeek: (index % 3) + 1, sessionNo: index + 1, title, durationMin: 45, beforeSession: "Complete the preceding capstone milestone and bring evidence.", agenda: "5 min check-in; 10 min model; 15 min workshop; 10 min conference or rehearsal; 5 min next step", afterSession: index === 5 ? "Complete the final course quiz." : "Apply feedback and upload the improved milestone.", outcomeIds: outcomes[index].outcomeId, origin: "Ehel Grade 5 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  return { schemaVersion: "Ehel English Runtime v1.2", templateVersion: "Ehel English Content Template v1.2", dictionaryVersion: "Ehel Master Dictionary v1.2", generatedAt: GENERATED_AT, grade: { id: "g05", label: "Grade 5" }, subject: "English", term, unit: { gradeId: "g05", subject: "English", termId: term.id, unitId: id, unitNo, unitTitle: title, unitOverview: "Bring together your strongest vocabulary, reading, grammar, speaking and writing from Units 1-9. Create a polished multi-section English product for a real audience, present it clearly, respond to questions and reflect on your Grade 5 growth.", learningPath: "Choose evidence. Review language. Plan and draft sections. Improve from feedback. Present. Reflect. Complete the final course quiz.", origin: "Ehel Grade 5 capstone specification", reviewStatus: "Approved v1.2 capstone", sourceFile: "Ehel English Content Template v1.2" }, visual: { image: "../grade-2/capstone-my-english-world.png", alt: visualMap[10][1], lectureMode: "capstone-launch" }, vocabularyGroups: [{ id: "u10-g1-grade-5-review-words", number: 1, title: "Grade 5 Review Words", vocabularyIds: selectedLinks.map((link) => link.vocabularyId) }], dictionaryLinks: selectedLinks, rubrics: baseRubrics.map((rubric) => ({ ...rubric, origin: "Ehel Grade 5 approved rubric v1.2" })), outcomes, readings, comprehension, grammar, speaking, writing, activities, assignments: [{ assignmentId: `${id}-assignment01`, unitId: id, title: "Grade 5 capstone portfolio and presentation", instructions: "Submit the final multi-section product, three portfolio pieces, presentation recording and reflection.", submissionType: "Portfolio + product + presentation", marks: 64, outcomeIds: outcomes.map((outcome) => outcome.outcomeId).join(" | "), rubricIds: "rub-writing-v1 | rub-speaking-v1", origin: "Ehel Grade 5 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], quizzes, liveSessions, teacherNotes: [{ teacherNoteId: `${id}-note01`, unitId: id, noteType: "Capstone moderation", note: "Conference at each milestone, preserve learner ownership, assess the multi-section product and presentation with shared rubrics, and record actionable Grade 6 transition feedback.", visibility: "Teacher", origin: "Curriculum review", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], answerKey: [...activities.map((activity) => ({ answerId: `${activity.activityId}-answer`, unitId: id, contentId: activity.activityId, contentType: "Capstone milestone", answerOrGuidance: activity.answerSummary, origin: activity.origin, reviewStatus: "Approved v1.2", sourceFile: activity.sourceFile })), ...quizzes.map((quiz) => ({ answerId: `${quiz.questionId}-answer`, unitId: id, contentId: quiz.questionId, contentType: "Quiz", answerOrGuidance: `${quiz.correctAnswer} ${quiz.explanation}`, origin: quiz.origin, reviewStatus: "Approved v1.2", sourceFile: quiz.sourceFile }))], selfAssessment: outcomes.map((outcome, index) => ({ selfAssessmentId: `${id}-self0${index + 1}`, unitId: id, sequence: index + 1, statement: outcome.learningOutcome.replace(/^(Select|Use|Read|Create|Present|Reflect)/, (verb) => `I can ${verb.toLowerCase()}`), scale: "Not yet | With help | By myself", outcomeId: outcome.outcomeId, origin: "Capstone reflection", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" })) };
}

function buildFinalAssessment(units) {
  const selected = units.slice(0, 9).flatMap((unit) => unit.quizzes.slice(0, 4)).slice(0, 30);
  const sections = [
    { sectionId: "grade5-final-sec01", sequence: 1, title: "Words and Meaning", description: "Use Grade 5 vocabulary accurately in familiar and new contexts.", startQuestion: 1, endQuestion: 10, questionCount: 10 },
    { sectionId: "grade5-final-sec02", sequence: 2, title: "Reading and Language", description: "Show reading understanding and control of Grade 5 language patterns.", startQuestion: 11, endQuestion: 20, questionCount: 10 },
    { sectionId: "grade5-final-sec03", sequence: 3, title: "Communication and Application", description: "Apply English to speaking, writing and purposeful communication.", startQuestion: 21, endQuestion: 30, questionCount: 10 },
  ];
  const questions = selected.map((quiz, index) => {
    const sourceUnit = units.find((unit) => unit.unit.unitId === quiz.unitId);
    return { ...quiz, assessmentId: "eng-g05-course-final-quiz-v12", quizId: "eng-g05-course-final-quiz-v12", questionId: `eng-g05-final-q${String(index + 1).padStart(2, "0")}`, sequence: index + 1, sectionId: sections[Math.floor(index / 10)].sectionId, sourceUnitNo: sourceUnit?.unit.unitNo, sourceUnitId: quiz.unitId, sourceUnitTitle: sourceUnit?.unit.unitTitle, curriculumArea: Math.floor(index / 10) === 0 ? "Vocabulary and meaning" : Math.floor(index / 10) === 1 ? "Reading and language" : "Communication and application", reviewRoute: `Review Unit ${sourceUnit?.unit.unitNo || 1}`, audio: pendingAudio(`./media/audio/final-quiz/eng-g05-final-q${String(index + 1).padStart(2, "0")}.mp3`) };
  });
  return { schemaVersion: "Ehel English Course Final Assessment v1.2", assessmentId: "eng-g05-course-final-quiz-v12", gradeId: "g05", subject: "English", title: "Grade 5 English Final Course Quiz", description: "A cumulative assessment of vocabulary, reading, language and communication across Units 1-9.", placement: "After Unit 10 capstone", questionCount: 30, totalMarks: 30, estimatedMinutes: 30, passPercent: 80, attemptsAllowed: "Teacher or school policy", sections, questions, reviewStatus: "Approved v1.2", audioProvider: "ElevenLabs", voiceId: VOICE_ID };
}
...
const dictionary = { schemaVersion: "Ehel Master Dictionary v1.2", language: "en-GB", gradeId: "g05", entryCount: dictionaryEntries.size, entries: [...dictionaryEntries.values()] };
const finalAssessment = buildFinalAssessment(units);
const manifest = { schemaVersion: "Ehel Grade 5 English Course Manifest v1.2", grade: { id: "g05", label: "Grade 5" }, subject: "English", defaultUnit: 1, units: units.map((unit) => ({ number: unit.unit.unitNo, id: unit.unit.unitId, termId: unit.term.id, title: unit.unit.unitTitle, data: `./data/units/unit-${unit.unit.unitNo}.json`, vocabularyCount: unit.dictionaryLinks.length, reviewStatus: unit.unit.reviewStatus })), finalAssessment: { id: finalAssessment.assessmentId, title: finalAssessment.title, data: "./data/course-final-quiz.json", placement: finalAssessment.placement, questionCount: 30, passPercent: 80, reviewStatus: finalAssessment.reviewStatus } };
...
fs.writeFileSync(path.join(DATA_DIR, "master-dictionary.grade5.json"), JSON.stringify(dictionary, null, 2));
fs.writeFileSync(path.join(DATA_DIR, "course-final-quiz.json"), JSON.stringify(finalAssessment, null, 2));
console.log(`Built Grade 5 English: ${units.length} units, ${dictionary.entryCount} dictionary entries, ${finalAssessment.questionCount}-question final quiz.`);
