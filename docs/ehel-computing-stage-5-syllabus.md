# Stage 5 Computing — course syllabus

Narrative copy for the syllabus authoring form at
`local/hubredirect/syllabus.php`, for the Ehel Academy **Stage 5 Computing**
course (Grade 5; this subject labels its years *Stage*). Every figure is read
from `src/prototypes/ehel-academy/computing/grade-5/data/`.
...
3. **The narration is not in the repository.** 735 clips are needed for
   this stage and 735 are present on this machine — but
...
Stage 5 Computing changes shape. The eleven or twelve short job-title units of the earlier years give way to **four large units named for Cambridge's four strands**: computational thinking and programming, managing data, networks and digital communication, and computer systems. It follows **Cambridge Primary Computing 0672, Stage 5**.

Unit 1 alone carries variables, and the course is now built from student lesson books rather than from teacher guides — the learner is being addressed directly by the source material for the first time.

Across the year: **4 units**, **40 learning outcomes**,
**47 concepts**, **15 code examples**, **32 hands-on activities**,
**64 practice questions**, **48 fluency questions**, **22 debugging
cases**, **10 e-safety notes**, **48 games** and **4 unit projects** — one
...
**debugging section** — a symptom, its cause and its fix, 22 of them across the
year — which teaches that finding the fault is part of the work rather than a
sign of failure. And every unit carries **e-safety notes**, 10 across the year,
...
> {{Teacher name}} supports Stage 5 Computing at {{school name}}. {{One or two
...
Before Unit 1 a learner sits the **Stage 5 Computing Placement Exam**: 32 questions, 32 marks, about 30 minutes. Attempts are unlimited and it is never a fail.
...
- **Read the e-safety notes properly.** 10 across the year, inside the
...
**If a learner is struggling**, the placement check can be retaken at any time. Stage 4 Computing remains available for a learner who needs the ground under this one.
...
|---|---|---|---|---|---|---|
| 1 | Computational Thinking and Programming | 10 | 7 | 10 | 8 | 4 |
| 2 | Managing Data | 10 | 14 | 2 | 8 | 1 |
| 3 | Networks and Digital Communication | 10 | 13 | 1 | 8 | 1 |
| 4 | Computer Systems | 10 | 13 | 2 | 8 | 4 |
| | **Total** | **40** | **47** | **15** | **32** | **10** |

Varying between units: concepts (7–14), outcomes (10),
code examples (1–10), activities (8), e-safety notes
(1–4), debugging cases (1–8), practice (16)
...
Framework: Cambridge Primary Computing 0672, Stage 5.
Source audience: student.
Narration: 735/735 clips present locally, 0 tracked in git.

Source: `course-manifest.json` and `units/unit-*.json` under
`src/prototypes/ehel-academy/computing/grade-5/data/`.
