...
every Stage 3 objective reached - so this was a depth and register job, not a
coverage one, measured against Learner's Book 3.
...
(`Go further`, `Challenge yourself!`), and the vocabulary - fourteen of the 37 Stage 3 glossary terms appeared nowhere in the build. The
...
- **47 self-check claims** on the sticker shelf, which is not a step. Each one
...
- **Computing world built in every lesson**: a Did-you-know fact, two to four
  ...
  look at.
- **17 new word cards** plus the lecture text that uses them - plain text and ciphertext, error beside mistake, sub-task, decision, numerical, range, biometric, RFID, e-reader, computer virus, backup, interactive smartboard, multifunction, smart device, computer-controlled, repetitive and volume.
...
35 Stage 3 entries, each naming the lesson that must answer it and the
