...
$PAGE->set_url(new moodle_url('/local/hubredirect/dashboard.php'));
$PAGE->set_pagelayout('standard');
$PAGE->set_title('Quraan Academy Dashboard');
$PAGE->set_heading('Quraan Academy Dashboard');
...
function pqh_live_sessions_link(): moodle_url {
...
$studentsearch = $role === 'teacher' ? optional_param('studentq', '', PARAM_TEXT) : '';
$studentgroupid = $role === 'teacher' ? optional_param('groupid', 0, PARAM_INT) : 0;
$studentgroups = [];
$allchildren = [];
$children = [];
if ($role === 'parent') {
...
.pqh-shell{min-height:100vh;background:linear-gradient(180deg,#f1fff4 0,#fff 46%)}
.pqh-topbar{min-height:70px;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px max(20px,calc((100vw - 1180px)/2));background:#eaffea;border-bottom:1px solid rgba(105,76,45,.14)}
...
.pqh-hero{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:18px;padding:26px;border-radius:16px;background:linear-gradient(135deg,#eaffea 0%,#fff 54%,#fff7e7 100%);border:1px solid rgba(111,78,50,.13);box-shadow:0 16px 38px rgba(105,76,45,.08)}
...
.pqh-quick{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:12px;margin-bottom:18px}
...
@media(max-width:820px){.pqh-hero{display:block}.pqh-filter{grid-template-columns:1fr}.pqh-config__head,.pqh-live-monitor__head{display:block}.pqh-config-filter{grid-template-columns:1fr}.pqh-config-table,.pqh-config-table tbody,.pqh-config-table tr,.pqh-config-table td{display:block;width:100%}.pqh-config-table thead{display:none}.pqh-config-table tr{margin-bottom:10px}.pqh-config-table td{border-left:1px solid rgba(111,78,50,.10);border-right:1px solid rgba(111,78,50,.10);border-radius:0}.pqh-config-table td:first-child{border-radius:10px 10px 0 0}.pqh-config-table td:last-child{border-radius:0 0 10px 10px}.pqh-grid{grid-template-columns:1fr}.pqh-card--wide{grid-column:auto}.pqh-title{font-size:25px}.pqh-live-session__top{display:block}.pqh-live-status{margin-top:8px}.pqh-live-session__stats{grid-template-columns:1fr 1fr}}
...
@media(max-width:560px){.pqh-topbar{height:auto;padding:14px 16px}.pqh-quick{grid-template-columns:1fr}.pqh-wrap{padding:20px 14px 42px}.pqh-mini-list,.pqh-live-session__stats{grid-template-columns:1fr}body.pqh-dashboard-page .pq-comm-panel__sheet{top:auto;right:0;bottom:0;left:0;width:auto;height:min(86vh,720px);border-radius:16px 16px 0 0}body.pqh-dashboard-page .pq-comm-tabs{max-width:none}}
</style>
<main class="pqh-shell">
<div class="pqh-topbar">
  <div class="pqh-brand"><span class="pqh-brand__mark">QA</span><span>Quraan Academy</span></div>
  <div class="pqh-role-nav" aria-label="Dashboard roles">
    ...
    <span class="pqh-role-chip <?php echo $role === 'student' ? 'is-active' : ''; ?>">Students</span>
    <a class="pqh-logout" href="<?php echo (new moodle_url('/login/logout.php', ['sesskey' => sesskey()]))->out(false); ?>">Logout</a>
  </div>
</div>
<div class="pqh-wrap">
  <section class="pqh-hero">
    <div>
      <p class="pqh-kicker"><?php echo s(ucfirst($role)); ?> dashboard</p>
      <h1 class="pqh-title">Assalamu alaikum, <?php echo s(fullname($USER)); ?></h1>
      <p class="pqh-subtitle">A simple home for messages, progress, lessons, and next steps.</p>
    </div>
    <?php if ($children): ?>
      <form method="get">
  ...
  <section class="pqh-quick" aria-label="Quick actions">
    <?php if (($role === 'parent' || $role === 'teacher') && $selectedchild): ?>
      <a class="pqh-quick-card" href="<?php echo pqh_lesson_link((int)$selectedchild['cohortid'], '', false)->out(false); ?>"><strong>Audit Course</strong><span>Review the lesson without changing child progress</span></a>
      ...
      <a class="pqh-quick-card" href="<?php echo pqh_live_sessions_link()->out(false); ?>"><strong>Live Sessions</strong><span>Start or join online review classes</span></a>
    ...
    <?php elseif ($role === 'teacher'): ?>
      ...
      <a class="pqh-quick-card" href="<?php echo pqh_live_teacher_link()->out(false); ?>"><strong>Teacher Workspace</strong><span>Today's classes and post-class reviews</span></a>
      <a class="pqh-quick-card" href="<?php echo pqh_live_sessions_link()->out(false); ?>"><strong>Live Sessions</strong><span>Create and start review classes</span></a>
      ...
      <a class="pqh-quick-card" href="<?php echo pqh_live_schedule_link((int)$USER->id)->out(false); ?>"><strong>Live Schedule</strong><span>Review upcoming class schedule</span></a>
    ...
    <?php else: ?>
      <a class="pqh-quick-card" href="<?php echo pqh_lesson_link(0, '', true)->out(false); ?>"><strong>Lessons</strong><span>Continue learning</span></a>
    ...
    <?php else: ?>
                        ...
                        <div class="pqh-live-session__top">
                          <div>
                            ...
                            <span>
                              ...
                              - <?php echo userdate((int)$session->scheduled_end, get_string('strftimetime')); ?>
      ...
      <section class="pqh-grid" aria-label="<?php echo $role === 'teacher' ? 'Teacher dashboard' : 'Parent dashboard'; ?>">
        <article class="pqh-card pqh-card--wide">
          ...
          <p><?php echo $role === 'teacher' ? 'Review this student, open communications, or audit the course without changing progress.' : 'Audit the course as a parent or open the communication panel for this child.'; ?></p>
          <div class="pqh-actions">
            <a class="pqh-btn" href="<?php echo pqh_lesson_link((int)$selectedchild['cohortid'], '', false)->out(false); ?>">Audit Course</a>
            <a class="pqh-btn pqh-btn--secondary" href="<?php echo pqh_live_parent_trust_link((int)$selectedchild['studentid'])->out(false); ?>">Parent live hub</a>
  ...
  <?php elseif ($role === 'admin'): ?>
    <section class="pqh-grid" aria-label="Admin dashboard">
      ...
      <article class="pqh-card">
        <h3>Create</h3>
        <p>Use guided flows for safer scheduling and conflict prevention.</p>
        <div class="pqh-actions">
  ...
  <?php else: ?>
    <section class="pqh-grid" aria-label="Dashboard shell">
      <article class="pqh-card pqh-card--wide">
        <h3><?php echo s(ucfirst($role)); ?> dashboard</h3>
        <p>Use the quick links above to continue lessons, join live classes, and review teacher feedback when it is available.</p>
      </article>
      <article class="pqh-card">
        <h3>Communications</h3>
        <p>Use the lesson app communication panel while the role dashboard tools are being expanded.</p>
        <div class="pqh-actions">
          <a class="pqh-btn" href="<?php echo pqh_lesson_link(0, 'messages')->out(false); ?>">Open messages</a>
        </div>
      </article>
    </section>
  <?php endif; ?>
...
window.__prequran_ws_endpoint = 'https://quraan.academy/webservice/rest/server.php';
