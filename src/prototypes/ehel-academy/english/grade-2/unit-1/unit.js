...
const STORAGE_KEY = "ehel-english-g2-u1-progress-v1";
...
function renderOverview() {
  ...
  $("#app").innerHTML = `${pageHeader("Grade 2 · Term 1 · Unit 1", course.unit.unitTitle, "Meet new friends, use the calendar, read a school story and share your own ideas.")}
    <div class="overview-grid">
      <div class="section-stack">
        <section class="unit-banner">
          <img src="${course.visual.image}" alt="${escapeHtml(course.visual.alt)}">
          <div class="banner-copy"><span>Your learning journey</span><h2>A warm welcome to English</h2><p>${escapeHtml(course.unit.unitOverview.split(". ").slice(0, 2).join(". "))}.</p><button class="button gold" data-go="lecture" type="button">${icon("play")} Start with Teacher Musa</button></div>
        </section>
...
function renderLecture() {
  $("#app").innerHTML = `${pageHeader("Begin here", "Teacher audiovisual lecture", "Watch and listen before you begin the independent lesson. Captions are available in the player.")}
    <div class="lecture-layout">
      <section class="panel video-shell"><video id="lecture-video" controls preload="metadata" poster="${course.visual.lecturePoster}"><source src="${course.visual.lectureVideo}" type="video/mp4"><track kind="captions" src="${course.visual.lectureCaptions}" srclang="en" label="English" default></video><div class="video-footer"><p id="video-status">Teacher Musa · Unit 1 vocabulary</p><button class="button gold" id="lecture-done" type="button">${icon("check")} Mark watched</button></div></section>
      <div class="section-stack"><section class="panel"><span class="eyebrow">Before you learn</span><h2>Listen, look and repeat</h2><p>Teacher Musa introduces greetings, calendar words, words around us, colours, numbers and dates.</p><ul class="checklist"><li>${icon("ear")} Hear the words in clear British English</li><li>${icon("captions")} Read along with captions</li><li>${icon("message-circle")} Pause and repeat aloud</li></ul></section><section class="panel"><h3>Ready after the video?</h3><p>Open the vocabulary dictionary to hear every word again at normal or slow speed.</p><button class="button primary" id="to-dictionary" type="button">Open vocabulary ${icon("arrow-right")}</button></section></div>
    </div>`;
...
function renderActivities() {
  const calendarStatements = [
    ["There are 12 months in a year.", true], ["There are 8 days in a week.", false], ["January is the first month of the year.", true], ["September has 31 days.", false],
  ];
  $("#app").innerHTML = `${pageHeader("Learn by doing", "Activities", "Try the online calendar check, then complete the local-weather and personal-calendar tasks.")}<div class="task-grid"><section class="panel"><span class="eyebrow">Interactive</span><h2>True or false?</h2><div class="question-list">${calendarStatements.map(([statement], index) => `<div class="question"><strong>${index + 1}. ${statement}</strong><div class="audio-actions"><button class="button secondary" data-fact="${index}" data-value="true" type="button">True</button><button class="button secondary" data-fact="${index}" data-value="false" type="button">False</button></div><div id="fact-${index}"></div></div>`).join("")}</div></section>${course.activities.slice(1).map((activity) => `<article class="panel task-card"><span class="eyebrow">${escapeHtml(activity.deliveryMode)}</span><h3>${escapeHtml(activity.title)}</h3><p class="rule-box">${escapeHtml(activity.instructionsAndItems)}</p><button class="button secondary" data-activity-done="${activity.activityId}" type="button">${icon("check")} Mark complete</button></article>`).join("")}</div><p><button class="button primary" id="activities-done" type="button">Finish activities ${icon("check")}</button></p>`;
  $$('[data-fact]').forEach((button) => button.addEventListener("click", () => {
    const index = Number(button.dataset.fact);
    const correct = String(calendarStatements[index][1]) === button.dataset.value;
    $(`#fact-${index}`).innerHTML = `<p class="feedback ${correct ? "good" : "try"}">${correct ? "Correct!" : index === 1 ? "Try again: a week has 7 days." : "Try again: September has 30 days."}</p>`;
  }));
...
function renderQuiz() {
  ...
  $("#app").innerHTML = `${pageHeader("Unit checkpoint", "Quick quiz", "Answer five questions. You will see feedback after each answer and can try again.")}<section class="panel quiz-shell" id="quiz-shell"></section>`;
  drawQuizQuestion();
}
...
function renderTeacher() {
  ...
  $("#app").innerHTML = `${pageHeader("Teacher view", "Unit 1 teaching resources", "Implementation view for lesson delivery, assessment evidence and curriculum alignment.", "Curriculum approved")}
    <div class="section-stack">
...
async function init() {
  try {
    const [courseResponse, dictionaryResponse] = await Promise.all([fetch("./data/grade2-unit1.json"), fetch("./data/master-dictionary.unit1.json")]);
