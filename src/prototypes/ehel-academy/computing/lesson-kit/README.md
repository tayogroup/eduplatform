...
| `branch` | `branchAlgo` | picks one of two inputs and follows only the branch it takes (`before`, `yes`, `no`, `after`), then the other input; both inputs must run and the arms must differ; `word` labels the fork (default `IF`, and Stage 4 says `Ask:` because the owner left IF to Stage 5, 2026-09-12); `branch_run` decides (4CT.09) |
