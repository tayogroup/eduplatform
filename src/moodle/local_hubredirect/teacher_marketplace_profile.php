...
function pqtmp_application($teacher): array {
    $decoded = json_decode((string)($teacher->application_json ?? ''), true);
    return is_array($decoded) ? $decoded : [];
}

function pqtmp_public_url(array $application, string $field): string {
...
$isadminviewer = $loggedin && pqh_can_manage_academy_operations((int)$USER->id);
...
if ($ready && ($teacherid > 0 || $teacherslug !== '')) {
    ...
    if ($teacher) {
        $marketing = pqtmp_application($teacher);
    }
}
$requestloginurl = new moodle_url('/local/hubredirect/teacher_marketplace_request.php', ['teacherid' => $teacherid] + $consumerparams);
$marketplaceenrollmenturl = new moodle_url('/local/hubredirect/marketplace_enrollment.php', ['teacherid' => $teacherid] + $consumerparams);
$marketplaceadminurl = new moodle_url('/local/hubredirect/teacher_marketplace_admin.php', $consumerparams);
$teacherintakeurl = new moodle_url('/local/hubredirect/teacher_intake.php', ['existing_teacherid' => $teacherid] + $consumerparams);
$admindashboardurl = new moodle_url((int)($consumercontext->workspaceid ?? 0) > 0 ? '/local/hubredirect/workspace_dashboard.php' : '/local/hubredirect/dashboard.php', $consumerparams);
$canonicalprofileurl = $teacher ? pqh_teacher_public_profile_url($teacher, $consumercontext) : new moodle_url('/local/hubredirect/teacher_marketplace_profile.php', ['teacherid' => $teacherid] + $consumerparams);
if ($teacher && $teacherslug === '' && $_SERVER['REQUEST_METHOD'] === 'GET'
        && pqh_consumer_feature_enabled($consumercontext, 'teacher_marketplace')) {
    redirect($canonicalprofileurl);
}
$PAGE->set_context($context);
$PAGE->set_url($canonicalprofileurl);
...
$publicsocialurl = pqtmp_public_url($marketing, 'social_profile_url');
$publicwebsiteurl = pqtmp_public_url($marketing, 'website_or_booking_url');
$publicdemourl = pqtmp_public_url($marketing, 'demo_video_url');
$publicsocialhandle = ltrim(trim((string)($marketing['social_media_handle'] ?? '')), '@');
$marketplaceenrollmentrequired = pqh_consumer_feature_enabled($consumercontext, 'teacher_marketplace');

if ($marketplaceenrollmentrequired && $_SERVER['REQUEST_METHOD'] === 'POST'
        && (optional_param('submit_request', '', PARAM_TEXT) === '1'
            || optional_param('submit_selection', '', PARAM_TEXT) === '1')) {
    redirect($marketplaceenrollmenturl);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && optional_param('submit_request', '', PARAM_TEXT) === '1') {
    ...
    try {
        if ($isadminviewer) {
            throw new invalid_parameter_exception('Admin users should manage teacher requests from the marketplace admin area.');
        }
        if (!confirm_sesskey()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && optional_param('submit_selection', '', PARAM_TEXT) === '1') {
    ...
    try {
        if ($isadminviewer) {
            throw new invalid_parameter_exception('Admin users should manage teacher selections from the marketplace admin area.');
        }
        if (!confirm_sesskey()) {
    ...
    <section class="pqtmp-top pqh-workspace-top">
      ...
      <div class="pqtmp-actions pqh-workspace-actions">
        ...
        <?php if ($isadminviewer): ?>
          <a class="pqtmp-btn pqtmp-btn--light" href="<?php echo $marketplaceadminurl->out(false); ?>">Marketplace admin</a>
          <a class="pqtmp-btn pqtmp-btn--light" href="<?php echo $teacherintakeurl->out(false); ?>">Update intake</a>
          <a class="pqtmp-btn pqtmp-btn--light" href="<?php echo $admindashboardurl->out(false); ?>">Dashboard</a>
        <?php elseif ($loggedin): ?>
        ...
        <div>
          <section class="pqtmp-panel">
            ...
            <div class="pqtmp-meta">
              ...
              <span class="pqtmp-pill pqtmp-pill--gold">Platform reviewed</span>
            </div>
          ...
          <?php foreach ([
            ...
            <?php if (trim($content) !== ''): ?><section class="pqtmp-panel"><h2><?php echo s($heading); ?></h2><p class="pqtmp-text"><?php echo pqtmp_safe_lines($content); ?></p></section><?php endif; ?>
          <?php endforeach; ?>
          <?php if ($publicsocialhandle !== '' || $publicsocialurl !== '' || $publicwebsiteurl !== '' || $publicdemourl !== ''): ?>
            <section class="pqtmp-panel">
              <h2>Online presence</h2>
              <?php if ($publicsocialhandle !== '' || $publicsocialurl !== ''): ?>
                <div class="pqtmp-social">
                  <?php if ($publicsocialhandle !== ''): ?><div class="pqtmp-social-row"><span class="pqtmp-social-label">Social media handle</span><span class="pqtmp-social-value">@<?php echo s($publicsocialhandle); ?></span></div><?php endif; ?>
                  <?php if ($publicsocialurl !== ''): ?><div class="pqtmp-social-row"><span class="pqtmp-social-label">Social media link</span><span class="pqtmp-social-value"><a href="<?php echo s($publicsocialurl); ?>" target="_blank" rel="noopener noreferrer"><?php echo s($publicsocialurl); ?></a></span></div><?php endif; ?>
                </div>
              <?php endif; ?>
              <div class="pqtmp-actions">
                <?php if ($publicwebsiteurl !== ''): ?><a class="pqtmp-btn pqtmp-btn--light" href="<?php echo s($publicwebsiteurl); ?>" target="_blank" rel="noopener noreferrer">Website / booking</a><?php endif; ?>
                <?php if ($publicdemourl !== ''): ?><a class="pqtmp-btn pqtmp-btn--light" href="<?php echo s($publicdemourl); ?>" target="_blank" rel="noopener noreferrer">Introduction video</a><?php endif; ?>
              </div>
            </section>
          <?php endif; ?>
        </div>
        <aside>
          <?php if (!$loggedin): ?>
            <section class="pqtmp-panel" id="request-teacher">
              ...
              <p class="pqtmp-text">Complete the learner enrollment before messaging or selecting this teacher.</p>
              <p><a class="pqtmp-btn" href="<?php echo $requestloginurl->out(false); ?>">Request this teacher</a></p>
            </section>
          <?php elseif ($isadminviewer): ?>
            <section class="pqtmp-panel" id="request-teacher">
              <h2>Admin Preview</h2>
              <p class="pqtmp-text">You are viewing this parent-facing teacher profile as an admin. Use the admin tools to review requests, update intake details, or return to the dashboard.</p>
              <p><a class="pqtmp-btn" href="<?php echo $marketplaceadminurl->out(false); ?>">Marketplace admin</a></p>
              <p><a class="pqtmp-btn pqtmp-btn--light" href="<?php echo $teacherintakeurl->out(false); ?>">Update teacher intake</a></p>
              <p><a class="pqtmp-btn pqtmp-btn--light" href="<?php echo $admindashboardurl->out(false); ?>">Dashboard</a></p>
            </section>
          <?php else: ?>
            <section class="pqtmp-panel" id="request-teacher">
              <h2>Enroll to Request This Teacher</h2>
              <p class="pqtmp-text"><?php echo s($brandname); ?> requires learner goals, level, service needs, and availability before teacher communication is enabled. Existing learner identities are reused.</p>
              <p><a class="pqtmp-btn" href="<?php echo $marketplaceenrollmenturl->out(false); ?>">Complete enrollment</a></p>
            </section>
          <?php endif; ?>
        </aside>
      </div>
