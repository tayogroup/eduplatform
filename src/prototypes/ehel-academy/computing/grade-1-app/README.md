...
## Redeployed 2026-09-11 with the kit's changes for Grade 3

On the owner's instruction ("redeploy Grades 2 and 3, and Grades 1 and 4 with
them"): the pages of `a43aee2ac`. 14 files uploaded, all verified on storage, every
page byte-identical to HEAD, every edge path fresh. All eight lessons were then
...
owner then asked for these three, so every grade serves the same kit again. 15 files
...
one defect. All three are position-safe; the 8 lessons still have the same
...
- **The real block names.** Naming ScratchJr, Scratch and MakeCode closed
  ...
  maps the blocks it actually uses onto ScratchJr's own arrows and speech bubble. Blocks with no equivalent here
