...
const outFile = path.join(root, "dist", "pre_quraan", "units", "openmaic-classroom", "subtraction-missions-standalone.html");
const outputCopy = path.join(root, "outputs", "openmaic", "subtraction-missions-standalone.html");
...
<head>
  ...
  <title>Subtraction Missions</title>
  <style>
    :root {
      --ink: #1f2937;
      --muted: #667085;
    ...
    * { box-sizing: border-box; }
    ...
    .mission-chip:nth-child(2) { color: #166534; background: #dff7ee; }
    .mission-chip:nth-child(3) { color: #92400e; background: #fff0ca; }
    ...
    .mission-goal {
      ...
      color: var(--ink);
      background: #fff;
      border: 2px solid rgba(214, 165, 49, .30);
    ...
    .mastery-star {
      ...
      background: #fff7d6;
      color: #d6a531;
      border: 1px solid rgba(214, 165, 49, .28);
    }

    .mastery-star.on {
      color: #fff;
      background: var(--gold);
      box-shadow: 0 8px 18px rgba(214, 165, 49, .25);
    }
...
<body>
  ...
  <script>
    const pages = [
      {
        kind: "story",
        title: "Mission: Find What Is Left",
        text: ["Today you are a math explorer.", "Your job is to notice what changes when some things go away."],
        count: 5,
        taken: 0,
        type: "star",
        line: "Look. Think. Try. Check.",
        cards: ["Look first", "Make a guess", "Test it"]
      },
      {
        kind: "tap",
        title: "Rule to Discover",
        text: ["Some toys are here.", "Tap a toy. What changed? Now there are fewer toys."],
        count: 4,
        taken: 0,
        type: "block",
        line: "When one goes away, the group gets smaller.",
        cards: ["What changed?", "What stayed?", "What is left?"]
      },
      {
        kind: "story",
      ...
      {
        kind: "activity",
        title: "Mission 1: Toy Test",
        text: "Predict first. Then tap toys away. Check if your prediction was right.",
        start: 5,
        targetTake: 2,
        type: "block",
        label: "toy"
      ...
      {
        kind: "activity",
        title: "Mission 2: Snack Test",
        text: "Start with four snacks. Predict what is left, then test it.",
        start: 4,
        targetTake: 1,
        type: "round",
        label: "cookie"
      ...
      {
        kind: "activity",
        title: "Mission 3: Balloon Pop",
        text: "Five balloons are floating. Predict, pop three, then count.",
        start: 5,
        targetTake: 3,
        type: "star",
        label: "balloon"
      ...
      {
        kind: "activity",
        title: "Mission 4: Apple Basket",
        text: "Four apples are in the basket. Predict, take away two, and check.",
        start: 4,
        targetTake: 2,
        type: "apple",
        label: "apple"
      ...
      {
        kind: "activity",
        title: "Mission 5: Duck Pond",
        text: "Three ducks are in the pond. Predict what happens when one swims away.",
        start: 3,
        targetTake: 1,
        type: "duck",
        label: "duck"
      ...
      },
      {
        ...
        text: ["Subtraction means take away.", "You looked, guessed, tested, and counted what was left."],
        count: 5,
        taken: 0,
        type: "star",
        line: "A math explorer checks their idea.",
        cards: ["I can predict", "I can test", "I can explain"]
      }
    ];
    ...
    function renderActivity(data) {
      ...
      const goal = el("div", "mission-goal", "Start with " + data.start + ". Take away " + data.targetTake + ". What will be left?");
      const predict = el("div", "predict-row");
      const predictionOptions = Array.from(new Set([Math.max(0, targetLeft - 1), targetLeft, Math.min(data.start, targetLeft + 1)])).sort((a, b) => a - b);
      let prediction = null;
      predictionOptions.forEach((value) => {
        const btn = el("button", "predict-btn", String(value));
        btn.type = "button";
        btn.addEventListener("click", () => {
          prediction = value;
          predict.querySelectorAll("button").forEach((button) => button.classList.remove("selected"));
          btn.classList.add("selected");
          feedback.textContent = "Prediction saved. Now test it.";
        });
        predict.appendChild(btn);
      });
      const feedback = el("div", "feedback", "Pick your prediction before you tap.");
      const mastery = el("div", "mastery-row");
      mastery.appendChild(el("span", "", "Mission stars"));
      for (let i = 0; i < 3; i += 1) mastery.appendChild(el("span", "mastery-star", "★"));
      copy.appendChild(goal);
      copy.appendChild(predict);
      copy.appendChild(feedback);
      copy.appendChild(mastery);
      ...
      check.addEventListener("click", () => {
        ...
        mastery.querySelectorAll(".mastery-star").forEach((star) => star.classList.remove("on"));
        if (taken === data.targetTake) {
          const stars = prediction === targetLeft ? 3 : 2;
          mastery.querySelectorAll(".mastery-star").forEach((star, i) => star.classList.toggle("on", i < stars));
          feedback.textContent = prediction === targetLeft
            ? "Excellent. Your prediction matched the test."
            : "Good test. " + data.start + " take away " + taken + " leaves " + left + ".";
        } else if (taken < data.targetTake) {
          mastery.querySelectorAll(".mastery-star")[0].classList.add("on");
      ...
      reset.addEventListener("click", () => {
        ...
        feedback.textContent = "Pick your prediction before you tap.";
      });
    ...
    function pageAudioSrc(pageIndex) {
      return "assets/subtraction-missions/audio/page-" + String(pageIndex + 1).padStart(2, "0") + ".mp3";
    }
