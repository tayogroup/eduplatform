...
ZIP_PATTERN = "Year *-20260714T*.zip"
...
def document_type(name: str) -> str:
    ...
    if "Activity Sheet" in stem:
        return "Activities"
    for value in ("Lesson", "Activities", "Practice", "Reference"):
...
def looks_like_heading(text: str, style: str) -> bool:
    ...
    return bool(
        re.match(
            r"^(unit\s+\d+|lesson|activity|practice|reference|objectives?|learning|about\s+this\s+unit|"
            r"key\s+(?:words|terms|vocabulary|rules)|vocabulary|warm[- ]?up|introduction|"
            r"worked\s+examples?|guided\s+practice|independent\s+practice|examples?|"
            r"step[- ]by[- ]step|common\s+mistakes?|section\s+\d+|"
...
def extract(downloads: Path) -> dict:
    ...
    for zip_path in sorted(downloads.glob(ZIP_PATTERN)):
            ...
            units.append(
                {
                    "grade": grade,
                    "unit": unit_number,
                    "unit_id": f"math-g{grade:02d}-u{unit_number:02d}",
        ...
        grades[grade] = {
            "metadata": {
                "schema_version": "Ehel Mathematics Content Package v1.0",
                "grade": grade,
