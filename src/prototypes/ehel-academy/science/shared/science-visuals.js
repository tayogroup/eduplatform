...
const DIAGRAMS = {
  living: [
    ...
    { caption: "Green leaves make food from sunlight, water and carbon dioxide, and give off oxygen.", labels: ["sunlight", "carbon dioxide", "water", "oxygen"], scene: "photosynthesis",
    ...
    { caption: "A seed germinates: the root grows down, then the shoot grows up towards the light.", labels: ["seed", "root", "shoot", "leaves"], scene: "lifecycle",
    ...
    { caption: "Energy passes along a food chain: the Sun feeds plants, plants feed animals, animals feed predators. Drag to explore.", labels: ["Sun", "plant", "animal", "predator"], scene: "foodchain" },
    { caption: "The heart pumps blood in a loop, carrying oxygen and food to every part of the body. Drag to explore.", labels: ["heart", "blood", "around the body"], scene: "circulation" },
    { caption: "Animals are grouped by their features — some have a backbone (vertebrates), some do not (invertebrates).", labels: ["mammal", "bird", "fish", "insect"],
      art: `<circle cx="60" cy="70" r="24" class="sci-fill-orange"/><text x="60" y="112" class="sci-tiny">mammal</text><path d="M150 54l24 16-24 16-8-16z" class="sci-fill-blue"/><circle cx="150" cy="70" r="18" class="sci-fill-blue"/><text x="150" y="112" class="sci-tiny">bird</text><path d="M226 70q20-20 44 0-24 20-44 0z" class="sci-fill-teal"/><circle cx="240" cy="68" r="3" class="sci-detail"/><text x="248" y="112" class="sci-tiny">fish</text><circle cx="330" cy="70" r="14" class="sci-fill-red"/><path d="M330 56v-10M330 84v10M316 70h-10M344 70h10" class="sci-detail"/><text x="330" y="112" class="sci-tiny">insect</text>` },
    { caption: "Living things are built from cells. A plant cell has a wall and chloroplasts; an animal cell does not. Drag to explore.", labels: ["nucleus", "cytoplasm", "cell membrane", "chloroplast"], scene: "cell",
      art: `<rect x="34" y="42" width="130" height="96" rx="10" class="sci-fill-green"/><circle cx="99" cy="90" r="20" class="sci-fill-navy"/><circle cx="66" cy="66" r="9" class="sci-dot"/><circle cx="134" cy="114" r="9" class="sci-dot"/><text x="99" y="160" class="sci-label">plant cell</text><ellipse cx="278" cy="90" rx="66" ry="48" class="sci-fill-orange"/><circle cx="278" cy="90" r="19" class="sci-fill-navy"/><text x="278" y="160" class="sci-label">animal cell</text><text x="99" y="34" class="sci-tiny">wall + chloroplasts</text><text x="278" y="34" class="sci-tiny">no wall</text>` },
  ],
  earthspace: [
    ...
    { caption: "Day and night: the Earth spins, so one half faces the Sun (day) and one half faces away (night).", labels: ["Sun", "day side", "night side", "spin →"], scene: "daynight",
    ...
    { caption: "The Moon's Sun-facing half is always lit — we see a different slice of it each night, which gives the phases. Drag to explore.", labels: ["Sun", "Earth", "Moon", "lit half"], scene: "moonphases",
      art: `<circle cx="42" cy="90" r="24" class="sci-fill-gold"/><circle cx="180" cy="90" r="26" class="sci-fill-blue"/><circle cx="180" cy="90" r="62" class="sci-ring"/>${[0,1,2,3].map((i)=>{const a=i/4*Math.PI*2,x=180+Math.cos(a)*62,y=90+Math.sin(a)*62;return `<circle cx="${x.toFixed(0)}" cy="${y.toFixed(0)}" r="11" class="sci-fill-gray"/><path d="M${(x-11).toFixed(0)} ${y.toFixed(0)}a11 11 0 0 1 22 0z" class="sci-fill-gold"/>`;}).join("")}<text x="42" y="140" class="sci-label">Sun</text><text x="180" y="140" class="sci-label">Earth</text>` },
    { caption: "The Sun holds the planets in orbit. Each planet travels its own path, and the ones further out take far longer to go round. Drag to explore.", labels: ["Sun", "inner planets", "outer planets", "orbit"], scene: "solarsystem",
      art: `<circle cx="60" cy="90" r="26" class="sci-fill-gold"/>${[52,80,108,136].map((r,i)=>`<path d="M60 90m-${r} 0a${r} ${r*0.55} 0 1 0 ${r*2} 0a${r} ${r*0.55} 0 1 0 -${r*2} 0" class="sci-ring"/><circle cx="${60+r}" cy="90" r="${7-i}" class="${["sci-fill-gray","sci-fill-red","sci-fill-blue","sci-fill-teal"][i]}"/>`).join("")}<text x="60" y="150" class="sci-label">Sun</text>` },
  ...
  matter: [
    ...
    { caption: "Everything is made of tiny atoms. A nucleus sits in the centre with electrons moving around it. Drag to explore.", labels: ["nucleus", "electrons", "atom"], scene: "atom" },
    { caption: "Atoms join to make molecules — the smallest piece of a substance like water. Drag to explore.", labels: ["atom", "bond", "molecule"], scene: "molecule" },
    { caption: "We separate mixtures using sieving, filtering, evaporating and letting things settle.", labels: ["sieve", "filter", "evaporate"],
      art: `<path d="M40 60h70l-10 30h-50z" class="sci-glass"/><path d="M48 90h54" class="sci-detail" stroke-dasharray="3 3"/><circle cx="60" cy="100" r="4" class="sci-fill-gold"/><circle cx="90" cy="102" r="4" class="sci-fill-gold"/><text x="75" y="128" class="sci-tiny">sieve</text><path d="M160 58l40 0-14 34h-12z" class="sci-glass"/><path d="M174 92l0 20" class="sci-ray-line"/><ellipse cx="174" cy="120" rx="18" ry="6" class="sci-fill-blue"/><text x="180" y="140" class="sci-tiny">filter</text><rect x="270" y="80" width="56" height="30" rx="4" class="sci-fill-blue"/><path d="M282 78c0-10 8-10 8-18M300 78c0-10 8-10 8-18M318 78c0-10 8-10 8-18" class="sci-wavearc"/><text x="298" y="130" class="sci-tiny">evaporate</text>` },
  ],
  forces: [
    ...
    { caption: "Magnets attract some metals and have two poles; like poles repel, unlike poles attract.", labels: ["N pole", "S pole", "attract", "repel"], scene: "magnetfield",
    ...
    { caption: "Friction and gravity: gravity pulls down, the surface pushes up, friction opposes sliding.", labels: ["gravity ↓", "support ↑", "friction ⇄"], scene: "friction",
    ...
    { caption: "An object floats when the upward push of the water balances its weight, and sinks when its weight wins. Drag to explore.", labels: ["upthrust ↑", "weight ↓", "floats", "sinks"], scene: "floatsink",
      art: `<rect x="20" y="80" width="340" height="66" rx="4" class="sci-fill-blue"/><path d="M20 80h340" class="sci-detail"/><rect x="82" y="62" width="46" height="34" rx="3" class="sci-fill-gold"/><path d="M105 122v-18m-7 8l7-8 7 8" class="sci-arrow"/><text x="105" y="52" class="sci-label">floats</text><rect x="248" y="112" width="40" height="30" rx="3" class="sci-fill-navy"/><path d="M268 74v26m-7-8l7 8 7-8" class="sci-arrow alt"/><text x="268" y="66" class="sci-label">sinks</text>` },
  ],
  light: [
    { caption: "Light travels in straight rays; drag the wave model to see how it moves.", labels: ["source", "ray", "screen"], scene: "wave" },
    { caption: "Light reflects off objects into our eyes so we can see them.", labels: ["light source", "object", "eye", "reflected ray"], scene: "reflection",
    ...
    { caption: "A shadow forms where an opaque object blocks the light.", labels: ["light", "object", "shadow"], scene: "shadow",
    ...
    { caption: "Light bends when it passes from air into water or glass, and bends back on the way out. That bending is called refraction. Drag to explore.", labels: ["incoming ray", "boundary", "bent ray", "normal"], scene: "refraction",
      art: `<rect x="120" y="70" width="140" height="70" rx="4" class="sci-fill-blue"/><path d="M30 26l90 44" class="sci-ray-line"/><path d="M120 70l70 70" class="sci-ray-line"/><path d="M190 140l80 26" class="sci-ray-line"/><path d="M120 34v72" class="sci-detail" stroke-dasharray="5 5"/><text x="60" y="24" class="sci-tiny">incoming ray</text><text x="196" y="60" class="sci-tiny">water or glass</text><text x="250" y="176" class="sci-tiny">bent ray</text>` },
  ],
  sound: [
    { caption: "Sound is a vibration that travels as a wave. Drag the model to watch it move.", labels: ["vibration", "wave", "ear"], scene: "wave" },
    { caption: "Vibrations travel from a source through the air to your ear.", labels: ["source", "sound waves", "ear"], scene: "soundwave",
  ...
  electricity: [
    ...
    { caption: "A switch opens or closes the circuit. Open = no current; closed = current flows.", labels: ["open — off", "closed — on"], scene: "seriescircuit",
