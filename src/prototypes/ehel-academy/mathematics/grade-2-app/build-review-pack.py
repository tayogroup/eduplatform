...
for r0 in rows:
    ...
    # no key means nothing could have checked it - see this file's docstring
    r0["unverified"] = r0.get("reason", False) or not r0["key"] or any(
        f == r0["file"] and r0["q"].startswith(q[:60].strip()) for f, q in unver)

# A FLOOR FOR THE RECALL BANK, for the reason the reasoning triples have one:
