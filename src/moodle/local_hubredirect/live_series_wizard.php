...
// The SAME timezone the create page parses with (pql_default_schedule_timezone
// in live_sessions.php). The wizard used to compute its preview with
// usergetmidnight() -- the VIEWER's timezone -- so a series could preview
// "no conflicts" and then be conflict-blocked at create time, hours away in
// school time, with no message the wizard user could see. Measured 2026-08-30:
// 22:02 in the viewer's New York was 15:02 school time, inside a running
// afternoon session.
function pqlsw_schedule_timezone(): string {
...
function pqlsw_parse_schedule_start(string $date, string $time): int {
    if (trim($date) === '' || pqlsw_minutes($time) < 0) {
        return 0;
    }
    try {
        $dt = DateTimeImmutable::createFromFormat('!Y-m-d H:i', trim($date) . ' ' . trim($time), new DateTimeZone(pqlsw_schedule_timezone()));
...
$start = pqlsw_parse_schedule_start($sessiondate, $sessiontime);
$until = $untilraw !== '' ? pqlsw_parse_schedule_start($untilraw, '23:59') : ($start > 0 ? $start + (30 * DAYSECS) : 0);
...
// Pre-flight: every one of these mirrors a refusal in live_sessions.php's
// create branch. The wizard used to preview and submit a series that could
// only be refused -- an empty Students list being the case that cost a day:
// the review card showed "Students:" blank and offered the button anyway.
$readiness = [
    ['ok' => $teacherid > 0, 'step' => 1, 'label' => 'Teacher chosen'],
    ['ok' => count($studentids) > 0, 'step' => 2, 'label' => 'At least one student (choose a class group or enter student IDs)'],
    ['ok' => $title !== '', 'step' => 3, 'label' => 'Series title entered'],
    ['ok' => $start > 0, 'step' => 4, 'label' => 'Valid first date and class time'],
    ['ok' => $start > 0 && ($start + max(15, $duration) * MINSECS) > time(), 'step' => 4, 'label' => 'First session is in the future'],
];
$blockedchecks = array_values(array_filter($readiness, static function(array $check): bool {
    return !$check['ok'];
}));
...
<style><?php echo pqh_viewer_chrome_css('.pqlsw-shell'); ?></style>
...
<?php echo pqh_design_shell_html('pqlsw-shell'); ?><div class="pqlsw-wrap">
  ...
  <section class="pqlsw-panel">
    ...
    <?php else: ?>
      ...
      <div class="pqlsw-card"><h3><?php echo s($title); ?></h3><p class="pqlsw-meta">Teacher: <?php echo s(pqlsw_user_name($teacherid, 'Teacher ' . $teacherid)); ?> #<?php echo (int)$teacherid; ?></p><p class="pqlsw-meta">Students: <?php echo s(implode(', ', array_map(static function(int $id): string { return pqlsw_user_name($id, 'Student ' . $id); }, $studentids))); ?></p><p class="pqlsw-meta">Lesson: <?php echo s($lessonid); ?> / <?php echo s($unitid); ?></p><p class="pqlsw-meta">Generated sessions: <?php echo count($starts); ?>, <?php echo s($pattern); ?>, audio recording always enabled; video opt-in and consent-controlled.</p><p class="pqlsw-meta">Class time <?php echo s($sessiontime); ?> is <?php echo s(pqlsw_schedule_timezone()); ?> school time; dates below are shown in your own timezone.</p></div>
      ...
      <form method="post" action="<?php echo (new moodle_url('/local/hubredirect/live_sessions.php', $urlparams))->out(false); ?>"><input type="hidden" name="sesskey" value="<?php echo sesskey(); ?>"><input type="hidden" name="action" value="create"><input type="hidden" name="created_from_wizard" value="1"><input type="hidden" name="timezone" value="<?php echo s(pqlsw_schedule_timezone()); ?>"><input type="hidden" name="recurring_enabled" value="1"><input type="hidden" name="teacherid" value="<?php echo (int)$teacherid; ?>"><input type="hidden" name="groupid" value="<?php echo (int)$groupid; ?>"><input type="hidden" name="studentids_raw" value="<?php echo s(implode(', ', $studentids)); ?>"><input type="hidden" name="title" value="<?php echo s($title); ?>"><input type="hidden" name="lessonid" value="<?php echo s($lessonid); ?>"><input type="hidden" name="unitid" value="<?php echo s($unitid); ?>"><input type="hidden" name="sessiondate" value="<?php echo s($sessiondate); ?>"><input type="hidden" name="sessiontime" value="<?php echo s($sessiontime); ?>"><input type="hidden" name="duration" value="<?php echo (int)$duration; ?>"><input type="hidden" name="recurrence_pattern" value="<?php echo s($pattern); ?>"><input type="hidden" name="recurrence_count" value="<?php echo (int)$count; ?>"><input type="hidden" name="recurrence_until" value="<?php echo s($untilraw); ?>"><input type="hidden" name="recording_enabled" value="1"><?php foreach ($weekdays as $day): ?><input type="hidden" name="recurrence_weekdays[]" value="<?php echo (int)$day; ?>"><?php endforeach; ?><?php if ($conflictcount > 0 && $pqlswisadmin): ?><div class="pqlsw-card"><h3>Admin Conflict Override</h3><label class="pqlsw-check"><input type="checkbox" name="override_conflicts" value="1"> <span>Override conflicts with audit reason</span></label><div class="pqlsw-field"><label for="override_reason">Override Reason</label><input class="pqlsw-input" id="override_reason" name="override_reason" type="text" placeholder="Required if overriding conflicts"></div></div><?php elseif ($conflictcount > 0): ?><div class="pqlsw-alert pqlsw-alert--bad">Resolve the scheduling conflicts before submitting this series.</div><?php endif; ?><div class="pqlsw-actions pqh-workspace-actions"><a class="pqlsw-btn pqlsw-btn--light" href="<?php echo (new moodle_url('/local/hubredirect/live_series_wizard.php', $params + ['step' => 5]))->out(false); ?>">Back</a><button class="pqlsw-btn" type="submit" <?php echo ($blockedchecks || ($conflictcount > 0 && !$pqlswisadmin)) ? 'disabled' : ''; ?>>Create recurring series</button></div></form>
    <?php endif; ?>
  </section>
</div></main>
<?php
