...
for (const grade of process.argv.slice(2)) {
  const sbDir = path.join(ROOT, "src/prototypes/ehel-academy/science/grade-" + grade + "-app/lecture-video");
  const scDir = path.join(ROOT, "tools/lib/film-scenes/science-g" + grade);
  ...
  for (const f of fs.readdirSync(sbDir).filter((x) => x.endsWith(".json"))) {
    ...
    if (bad.length) { console.log("\nG" + grade + "  " + slug); bad.forEach((b) => console.log("   " + b)); }
  }
}
console.log("\n" + total + " declared cues checked, " + unread + " never read by any scene file.");

/* A run that checked nothing reads exactly like a clean one, so it says so
   rather than printing a tick over no work. */
if (!total) {
  console.log("nothing was checked. Name the grades: node tools/check-ehel-film-cues.js 1 2 3 4");
  process.exitCode = 2;
}
