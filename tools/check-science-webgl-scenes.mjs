// Acceptance check for the Science WebGL scene catalogue.
//
// A scene is wired up in two files: science-visuals.js names it on a diagram,
// science-webgl.js builds its geometry. Nothing at runtime complains when those
...
const sharedDir = path.join(here, "..", "src", "prototypes", "ehel-academy", "science", "shared");

const { sceneObjects } = await import(`file://${path.join(sharedDir, "science-webgl.js")}`);

const visualsSource = fs.readFileSync(path.join(sharedDir, "science-visuals.js"), "utf8");
const webglSource = fs.readFileSync(path.join(sharedDir, "science-webgl.js"), "utf8");
...
for (const id of referenced) {
  if (!defined.includes(id)) fail(`scene "${id}" is used by a diagram but has no geometry in science-webgl.js`);
}
...
if (unique.length) {
  console.error(`✗ science WebGL scenes: ${unique.length} problem(s)`);
  for (const message of unique.slice(0, 40)) console.error(`   ${message}`);
  process.exit(1);
}

const totals = defined.map((id) => sceneObjects(id, 1.5).length);
console.log(`✓ science WebGL scenes: ${defined.length} scenes, all referenced and all render geometry`);
