...
const GRADE_DIR = path.join(ROOT, "src", "prototypes", "ehel-academy", "english", "grade-8");
const DATA_DIR = path.join(GRADE_DIR, "data");
const UNIT_DIR = path.join(DATA_DIR, "units");
const SOURCE_FILE = path.join(ROOT, "inputs", "ehel-grade8-source", "grade8-source-extracted.json");
...
// A unit assignment awards 40 and the capstone 72, both against two rubrics
// worth 32 marks together — four writing criteria and four speaking criteria,
// four marks each. Without an itemisation that leaves 112 of the year's 432
// marks with no stated basis, which is where Grades 7 and 8 sat until
// 2026-08-09. Grade 2, the grade baseRubrics is copied from, publishes its own
// capstone division; these follow its wording. Keep the itemisation and the
// marks in step: change one and the other has to change with it.
const UNIT_MARK_GUIDANCE = "Marking guidance for the teacher. Award the 40 marks as follows. Writing, 16 marks: the four writing rubric criteria — ideas, organisation, language and conventions — four marks each. Speaking, 16 marks: the four speaking rubric criteria — clarity, vocabulary, sentence accuracy and interaction — four marks each. Source annotation, 8 marks: 4 marks for attributing every source used, naming what it is and where it came from, and 4 marks for the comparison or evaluation the task asks for.";
const CAPSTONE_MARK_GUIDANCE = "Marking guidance for the teacher. Award the 72 marks as follows. Inquiry product, 16 marks: the four writing rubric criteria, four marks each. Oral defence, 16 marks: the four speaking rubric criteria, four marks each, judged on answering for the work rather than presenting it. Attributed source record, 16 marks: 4 for completeness, 4 for accurate attribution, 4 for evaluating reliability, 4 for showing where each source changed the conclusion. Four annotated portfolio pieces, 16 marks: 4 each, awarded for naming the unit, naming the skill and giving a real reason. Reflection, 8 marks: 4 for an honest account of what changed, 4 for a specific and reachable transition goal. Do not award marks for presentation polish or for the materials a family can afford. Assess the English and the reasoning only, and accept scribed or recorded work where a learner needs it.";
...
const gradeProfile = templateConfig.gradeProfiles.g08;
if (!gradeProfile || gradeProfile.readingWordsPerText !== "600-1300" || gradeProfile.speakingTarget !== "5-6 minutes") throw new Error("Grade 8 neutral profile is missing or invalid.");
...
const visualMap = {
  1: ["unit-8-home.png", "A Grade 8 learner analysing language, communication and truth"],
  2: ["unit-5-measure.png", "Learners evaluating design, packaging and sustainability"],
  3: ["unit-4-big-sky.png", "Learners investigating community and environmental responsibility"],
  4: ["unit-9-city.png", "Learners analysing advertising, persuasion and audience effect"],
  5: ["unit-4-big-sky.png", "Learners evaluating natural resources and sustainable choices"],
  6: ["unit-4-big-sky.png", "Learners exploring natural wonders and conservation decisions"],
  7: ["unit-2-neighbours-jobs.png", "Learners interpreting historical figures, leadership and legacy"],
  8: ["unit-8-home.png", "Learners developing global storytelling and creative craft"],
  9: ["unit-3-ready-steady-go.png", "Learners exploring music as communication across cultures"],
  10: ["capstone-my-english-world.png", "A Grade 8 learner defending a researched English capstone"],
};
...
function unitId(unitNo) {
  return `eng-g08-${termFor(unitNo).id}-u${String(unitNo).padStart(2, "0")}`;
}
...
function doc(unitNo, type) {
  ...
  if (!record) throw new Error(`Missing Grade 8 source: Unit ${unitNo} ${type}`);
  return record;
}
...
const unitTitles = { 1: "Languages and Communication", 2: "Design, Packaging, and Sustainability", 3: "Community and Environment", 4: "Advertising and Persuasion", 5: "Natural Resources and Sustainability", 6: "Natural Wonders and Conservation", 7: "Historical Figures and Legacy", 8: "Global Storytelling and Creativity", 9: "Music as Universal Language" };

function titleFromLesson(paragraph, unitNo) {
  return unitTitles[unitNo] || paragraph.replace(/^.*?Unit \d+\s*[:\-–—]?\s*/, "").trim();
}

function sourcePath(unitNo, type) {
  return `inputs/ehel-grade8-source/Year 8/${doc(unitNo, type).path}`;
}
...
function extractVocabulary(unitNo) {
  ...
  const tables = vocabularyDoc.tables.filter((table) => table[0]?.includes("Word") && table[0]?.includes("Part of Speech") && table[0]?.includes("Definition"));
  const seen = new Set();
  let total = 0;
  return tables.map((table, groupIndex) => ({
    ...
    words: table.slice(1).filter((row) => {
      const wordIndex = table[0].indexOf("Word");
      const key = String(row[wordIndex] || "").trim().toLowerCase();
      if (!key || seen.has(key) || total >= 55) return false;
      seen.add(key);
      total += 1;
      return true;
    }).map((row) => {
      const wordIndex = table[0].indexOf("Word");
      const typeIndex = table[0].indexOf("Part of Speech");
      const meaningIndex = table[0].indexOf("Definition");
      const exampleIndex = table[0].indexOf("Example Sentence");
      return { word: row[wordIndex].trim(), sourceType: row[typeIndex].trim(), type: normalizeType(row[typeIndex]), meaning: row[meaningIndex].trim(), example: row[exampleIndex]?.trim() || `The word ${row[wordIndex]} is important in this unit.` };
    }),
...
function extractReadings(unitNo) {
  ...
  const cleanLines = (paragraphs) => paragraphs.filter((line) => !/^(Year 8 English|After Reading|Understanding the Story|Comprehension|Reflection Questions|Activity \d+|Answer Key|Ask Your AI Tutor|Self-)/i.test(line) && !line.endsWith("?") && line.split(/\s+/).length > 5);
  const chunk = (paragraphs, target = 600) => {
    const chunks = [];
    let current = [];
    let words = 0;
  ...
  const lessonChunks = chunk(cleanLines(lesson.paragraphs.slice(2)), 640).slice(0, 2);
  ...
  const storyChunks = chunk(cleanLines(story.paragraphs.slice(storyStart, storyEnd < 0 ? story.paragraphs.length : storyEnd)), 640).slice(0, 3);
  const readings = [
    ...lessonChunks.map((passage, index) => ({ type: index ? "Close reading" : "Unit reading", title: `${titleFromLesson(lesson.paragraphs[0], unitNo)}: source text ${index + 1}`, passage })),
...
function grammarBlocks(unitNo) {
  ...
  while (blocks.length < 6) {
    ...
    blocks.push([
      ...
      `Practice ${sequence}`,
      "Edit a four-paragraph response using the unit language focus. Mark eight changes and explain how four changes improve clarity and cohesion.",
    ]);
  }
...
function grammarRecord(unitNo, block, sequence) {
  ...
  return {
    ...
    origin: "Authored Grade 8 source",
...
const extensionTasks = {
  speaking: [
    ...
    ["Six-Minute Unit Argument", "Plan and record a six-minute argument using a focused opening, connected claims with synthesized evidence, explicit source evaluation, a counterargument, rebuttal, and purposeful closing."],
  ],
  writing: [
...
function buildTasks(unitNo, kind) {
  ...
  return tasks.slice(0, 6).map(([title, body], index) => {
    ...
    const common = { unitId: unitId(unitNo), sequence, title: `${kind[0].toUpperCase() + kind.slice(1)} ${sequence}: ${title}`, outcomeId: `${unitId(unitNo)}-lo${String(Math.min(sequence, 6)).padStart(2, "0")}`, origin: index < sourceTasks.length ? "Authored Grade 8 source" : "Ehel English Content Template v1.2 curriculum expansion", reviewStatus: "Approved v1.2", sourceFile: index < sourceTasks.length ? sourcePath(unitNo, "Lesson") : "Ehel English Content Template v1.2" };
    if (kind === "speaking") return { speakingId: id, ...common, activityType: sequence <= 2 ? "Guided speaking" : "Speaking practice", instructionsAndModelLines: sequence === 6 ? `${body}\nDevelop this into a sustained five- to six-minute response with synthesized evidence, source evaluation, a counterargument and answers to follow-up questions.` : body, recordingRequired: true, aiTutorPrompt: `Help me practise ${title.toLowerCase()} and give feedback on clarity, vocabulary, evidence, reasoning and audience effect.`, audio: lessonAudio(unitNo, "speaking", id) };
    if (kind === "writing") return { writingId: id, ...common, promptAndInstructions: body, modelText: "A strong Grade 8 response sustains a line of thought across cohesive paragraphs, integrates precise vocabulary and evidence, evaluates sources and shapes language for audience and purpose.", sentenceStarter: "The evidence supports the view that...", expectedLength: sequence <= 2 ? "Five cohesive paragraphs" : gradeProfile.writingExpectation, successCriteria: "I sustained a clear line of thought; shaped writing for audience and purpose; integrated precise vocabulary and attributed evidence; compared viewpoints and source reliability; used cohesive devices and varied structures; revised content and accuracy", support: "Plan a thesis or controlling idea, paragraph sequence, evidence and source notes before drafting.", extension: "Synthesize three sources, qualify claims, address and rebut a counterargument, and refine style for deliberate effect.", rubricId: "rub-writing-v1" };
...
function extractUnitOverview(lesson) {
  const overviewIndex = lesson.paragraphs.findIndex((line) => /^(Unit )?Overview$/i.test(line));
  if (overviewIndex >= 0 && lesson.paragraphs[overviewIndex + 1]) return lesson.paragraphs[overviewIndex + 1];
  return lesson.paragraphs.find((line, index) => index >= 2 && /^(Welcome to (?:Unit|Year)|In this unit|Unit \d+ is called)/i.test(line)) || lesson.paragraphs[2];
}
...
function extractOutcomeText(lesson, title) {
  ...
  const extensions = [
    ...
    `Use Grade 8 vocabulary accurately in discussion and writing.`,
    `Apply the unit language focus in connected sentences and paragraphs.`,
    `Create a cohesive multi-paragraph response for a defined purpose and audience.`,
    `Present a sustained five- to six-minute argument and defend ideas under questioning.`,
  ];
...
function extractLearningPath(lesson) {
  ...
  if (start >= 0) {
    ...
    const end = rows.findIndex((line) => /^(Section \d+:|Part \d+:|Key Vocabulary|Reading(?:\s|:)|Writing Workshop|Activity \d+:)/i.test(line));
    const selected = rows.slice(0, end < 0 ? Math.min(rows.length, 8) : end);
    if (selected.length) return selected.join("\n");
  }
  return "Preview the goals and vocabulary. Read and annotate the source texts. Compare evidence, viewpoint and source reliability. Apply the grammar focus. Draft and revise a cohesive multi-paragraph response. Present, defend, reflect and complete the quiz.";
}

function buildRegularUnit(unitNo, dictionaryEntries) {
  const lesson = doc(unitNo, "Lesson");
  const title = titleFromLesson(lesson.paragraphs[0], unitNo);
  ...
  groups.forEach((group, groupIndex) => {
    ...
    group.words.forEach((word, wordIndex) => {
      ...
      const practiceSentences = [word.example, `The word ${word.word} belongs to our ${group.title.toLowerCase()} vocabulary.`, `I can explain ${word.word} in my own words.`, `I can connect ${word.word} to this unit's texts.`, `I can use ${word.word} in a clear Grade 8 sentence.`];
      dictionaryLinks.push({ vocabularyId, dictionaryEntryId, senseId: `${dictionaryEntryId}-sense-01`, gradeId: "g08", termId: term.id, unitId: id, groupId, groupTitle: group.title, sequence: wordIndex + 1, childMeaning: word.meaning, exampleSentence: word.example, practiceSentences, sentenceAudio: practiceSentences.map((_, sentenceIndex) => pendingAudio(`./unit-${unitNo}/media/audio/vocabulary/${vocabularyId}-sentence-${sentenceIndex + 1}.mp3`)), sentenceStarter: `The ${word.word}`, spellingPractice: word.word.split("").join(" - "), aiTutorPrompt: `Ask me to define '${word.word}', use it in a sentence, and connect it to ${title}.`, reviewStatus: "Approved v1.2" });
  ...
  const extensionQuestions = [
    ...
    "What viewpoint does the writer or narrator communicate, and how is it shaped?",
    "How reliable is this source for the claim it makes? Explain the limits of the evidence.",
    "Synthesize evidence from two texts to reach a wider conclusion.",
    "Evaluate one choice made by a character, writer or speaker and address an alternative view.",
  ];
  ...
  const comprehension = questions.slice(0, 12).map((question, index) => ({ questionId: `${id}-cq${String(index + 1).padStart(3, "0")}`, unitId: id, readingId: readings[index % readings.length].readingId, section: index < 4 ? "Meaning and structure" : index < 8 ? "Evidence, inference and viewpoint" : "Synthesis, source reliability and evaluation", sequence: index + 1, questionType: index < 4 ? "Interpretation and structure" : index < 8 ? "Evidence and viewpoint" : "Synthesis and critical evaluation", question, correctAnswer: "Accept an accurate, well-reasoned response supported by relevant evidence from the selected text or texts.", explanation: "The learner should interpret language and structure, select evidence, compare ideas, consider source reliability and justify the response clearly.", marks: index < 4 ? 1 : index < 8 ? 2 : 3, outcomeId: `${id}-lo${String((index % Math.max(1, outcomeText.length)) + 1).padStart(2, "0")}`, difficulty: index < 4 ? "Foundation" : index < 8 ? "Core" : "Stretch", origin: "Authored question or curriculum-reviewed extension", reviewStatus: "Approved v1.2", sourceFile: "Grade 8 Lesson and Story source" }));
  ...
  return {
    ...
    grade: { id: "g08", label: "Grade 8" }, subject: "English", term,
    unit: { gradeId: "g08", subject: "English", termId: term.id, unitId: id, unitNo, unitTitle: title, unitOverview: overview, learningPath, origin: "Authored Grade 8 source + curriculum review", reviewStatus: "Approved v1.2", sourceFile: sourcePath(unitNo, "Lesson") },
    visual: { image: `../assets/${visualMap[unitNo][0]}`, alt: visualMap[unitNo][1], lectureMode: "guided-launch" },
    vocabularyGroups, dictionaryLinks, rubrics: baseRubrics.map((rubric) => ({ ...rubric, origin: "Ehel Grade 8 approved rubric v1.2" })), outcomes, readings, comprehension, grammar, speaking, writing, activities,
    assignments: [{ assignmentId: `${id}-assignment01`, unitId: id, title: `${title} evidence portfolio`, instructions: `Submit ${writing[0].title}, an annotated source comparison and a recording of ${speaking[0].title}. Improve all pieces using the Grade 8 rubrics.\n${UNIT_MARK_GUIDANCE}`, submissionType: "Writing + source annotation + audio", marks: 40, outcomeIds: `${outcomes[0].outcomeId} | ${outcomes[Math.min(1, outcomes.length - 1)].outcomeId}`, rubricIds: "rub-writing-v1 | rub-speaking-v1", origin: "Curriculum-reviewed unit portfolio", reviewStatus: "Approved v1.2", sourceFile: "Derived from unit writing and speaking tasks" }],
    quizzes, liveSessions,
    teacherNotes: [{ teacherNoteId: `${id}-note01`, unitId: id, noteType: "Delivery and inclusion", note: "Model disciplinary language aloud, teach annotation and source checking explicitly, use mixed-response modes, and give specific feedback on evidence, cohesion, viewpoint and vocabulary precision while increasing learner independence.", visibility: "Teacher", origin: "Curriculum review", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }, { teacherNoteId: `${id}-note02`, unitId: id, noteType: "Source-linked AI tutor support", note: [...doc(unitNo, "Lesson").tables, ...doc(unitNo, "Vocabulary").tables, ...doc(unitNo, "Grammar").tables].flat(2).filter((value) => String(value).includes("Ask Your AI Tutor")).join("\n"), visibility: "Teacher", origin: "Authored Grade 8 source", reviewStatus: "Approved v1.2", sourceFile: "Multiple Grade 8 source files" }],
...
function buildCapstone(regularUnits, dictionaryEntries) {
  ...
  const id = unitId(unitNo);
  const term = termFor(unitNo);
  const selectedLinks = regularUnits.flatMap((unit) => unit.dictionaryLinks.slice(0, 4)).slice(0, 30).map((link, index) => ({ ...link, vocabularyId: `u10-g1-${index + 1}-${slug(dictionaryEntries.get(link.dictionaryEntryId).displayWord)}`, unitId: id, termId: term.id, groupId: "u10-g1-grade-8-review-words", groupTitle: "Grade 8 Review Words", sequence: index + 1 }));
  const title = "My English Voice: Grade 8 Capstone";
  const outcomes = ["Select and synthesize strong evidence from Units 1-9 to explain progress.", "Use Grade 8 vocabulary accurately across speaking and writing.", "Analyse and compare passages through language, structure, viewpoint and source reliability.", "Create an evidence-led inquiry product for a defined audience using responsible source attribution.", "Present and defend conclusions clearly for six minutes, responding thoughtfully to challenges.", "Evaluate progress honestly and set a specific Grade 9 English goal."].map((learningOutcome, index) => ({ outcomeId: `${id}-lo0${index + 1}`, unitId: id, sequence: index + 1, learningOutcome, bloomLevel: index < 2 ? "Apply" : index < 4 ? "Analyse and create" : "Evaluate", evidenceOfLearning: "Capstone portfolio, inquiry product, source record, oral defence and reflection", origin: "Ehel Grade 8 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  const makeSix = (kind, items) => items.map((item, index) => {
    ...
    const common = { unitId: id, sequence: index + 1, title: item[0], outcomeId: outcomes[index].outcomeId, origin: "Ehel Grade 8 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" };
    ...
    if (kind === "writing") return { writingId: itemId, ...common, promptAndInstructions: item[1], modelText: item[2], sentenceStarter: item[3], expectedLength: "Five to seven sustained paragraphs with source evidence", successCriteria: "I addressed my audience; organised connected sections and paragraphs; used precise vocabulary; synthesized and attributed relevant evidence; evaluated source reliability and viewpoint; checked grammar and punctuation; revised from feedback", support: "Use an inquiry organizer, source record, planning frame, word bank and approved source models.", extension: "Evaluate competing source choices, address a counterargument, vary sentence structures, strengthen transitions and write a defensible conclusion.", rubricId: "rub-writing-v1" };
  ...
  const speaking = makeSix("speaking", [["Portfolio evidence conference", "Explain four portfolio choices and support each choice with precise evidence."], ["Comparative reading and interpretation", "Read selected passages with clear phrasing, then compare their viewpoints, reliability and significance."], ["Research vocabulary in context", "Use fifteen review words naturally while explaining your inquiry and conclusions."], ["Six-minute inquiry presentation", "Present a clear opening, organised claims with synthesized and attributed evidence, a considered viewpoint, a counterargument and a strong closing."], ["Question-and-answer defence", "Answer six audience questions with relevant evidence, source awareness and thoughtful clarification."], ["Final capstone presentation", "Deliver your polished six-minute presentation and record your Grade 9 transition reflection."]]);
  const writing = makeSix("writing", [["Portfolio evidence annotations", "Write a precise paragraph about each of four selected pieces and explain what each demonstrates.", "This revised report shows that I can organize evidence, evaluate a viewpoint and explain the effect of my choices.", "This piece demonstrates..."], ["Inquiry proposal", "Explain your question, audience, approved sources, product and plan in four organized paragraphs.", "My inquiry will investigate how communities can use digital tools responsibly by comparing two approved sources.", "My inquiry question is..."], ["Evidence-led first draft", "Write the complete first draft of your capstone product with clear sections, attributed source evidence and a source record.", "Use a focused introduction, cohesive sections, synthesized evidence, a counterargument and a conclusion suited to your audience.", "Taken together, the evidence suggests..."], ["Substantive revision from feedback", "Record four feedback points and rewrite the sections they improve.", "Feedback asked me to compare source reliability, so I distinguished evidence from opinion and explained why one source was stronger.", "One substantial improvement I made was..."], ["Presentation and defence notes", "Prepare concise cue cards for a six-minute presentation and question defence.", "Opening; inquiry question; claims; attributed evidence; viewpoint; counterargument; audience questions; closing.", "Today I will defend..."], ["Grade 8 reflection", "Evaluate your strongest progress, one challenge, and a specific Grade 9 goal.", "I became a stronger critical reader because I learned to synthesize evidence and check source reliability. In Grade 9, I will improve comparative analysis.", "This year I learned..."]]);
  const activities = makeSix("activities", [["Curate and annotate evidence", "Review Units 1-9 and select reading, writing and speaking evidence with reasons."], ["Build a research word map", "Organise 30 review words into conceptual categories and explain relationships."], ["Investigate and verify sources", "Collect, attribute and compare evidence from at least three approved course sources."], ["Peer review studio", "Use the rubric to give and receive four specific improvement suggestions about evidence, cohesion and audience effect."], ["Rehearse with timing", "Practise the six-minute presentation twice and improve pace, emphasis, transitions and responses to challenges."], ["Exhibition and defence", "Present the inquiry product, defend conclusions and record a Grade 9 learning goal."]]);
  ...
  while (comprehension.length < 12) comprehension.push({ ...comprehension[comprehension.length % 10], questionId: `${id}-cq${String(comprehension.length + 1).padStart(3, "0")}`, sequence: comprehension.length + 1, question: comprehension.length === 10 ? "Compare two review texts. What important idea do they share?" : "Which Grade 8 text influenced your capstone most, and why?" });
  const quizzes = regularUnits.flatMap((unit) => unit.quizzes.slice(0, 2)).slice(0, 10).map((quiz, index) => ({ ...quiz, quizId: `${id}-quiz${String(index + 1).padStart(2, "0")}`, questionId: `${id}-quiz${String(index + 1).padStart(2, "0")}-q01`, unitId: id, quizTitle: "Grade 8 capstone review checkpoint", sequence: index + 1 }));
  const liveSessions = ["Launch and portfolio curation", "Reading and vocabulary conference", "Project planning and first draft", "Grammar editing and peer feedback", "Presentation rehearsal", "Capstone exhibition and reflection"].map((title, index) => ({ liveSessionId: `${id}-live0${index + 1}`, unitId: id, week: index < 3 ? 1 : 2, sessionWithinWeek: (index % 3) + 1, sessionNo: index + 1, title, durationMin: 45, beforeSession: "Complete the preceding capstone milestone and bring evidence.", agenda: "5 min check-in; 10 min model; 15 min workshop; 10 min conference or rehearsal; 5 min next step", afterSession: index === 5 ? "Complete the final course quiz." : "Apply feedback and upload the improved milestone.", outcomeIds: outcomes[index].outcomeId, origin: "Ehel Grade 8 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  return { schemaVersion: "Ehel English Runtime v1.2", templateVersion: "Ehel English Content Template v1.2", dictionaryVersion: "Ehel Master Dictionary v1.2", generatedAt: GENERATED_AT, grade: { id: "g08", label: "Grade 8" }, subject: "English", term, unit: { gradeId: "g08", subject: "English", termId: term.id, unitId: id, unitNo, unitTitle: title, unitOverview: "Bring together your strongest vocabulary, reading, grammar, speaking and writing from Units 1-9. Create an evidence-led inquiry product for a defined audience, attribute and evaluate sources, defend conclusions and reflect on your Grade 8 growth.", learningPath: "Curate and annotate evidence. Form an inquiry question. Compare and verify sources. Plan and draft cohesive sections. Revise from feedback. Present and defend. Reflect toward Grade 9. Complete the final course quiz.", origin: "Ehel Grade 8 capstone specification", reviewStatus: "Approved v1.2 capstone", sourceFile: "Ehel English Content Template v1.2" }, visual: { image: "../grade-2/capstone-my-english-world.png", alt: visualMap[10][1], lectureMode: "capstone-launch" }, vocabularyGroups: [{ id: "u10-g1-grade-8-review-words", number: 1, title: "Grade 8 Review Words", vocabularyIds: selectedLinks.map((link) => link.vocabularyId) }], dictionaryLinks: selectedLinks, rubrics: baseRubrics.map((rubric) => ({ ...rubric, origin: "Ehel Grade 8 approved rubric v1.2" })), outcomes, readings, comprehension, grammar, speaking, writing, activities, assignments: [{ assignmentId: `${id}-assignment01`, unitId: id, title: "Grade 8 inquiry capstone and oral defence", instructions: "Submit the evidence-led inquiry product, attributed source record, four annotated portfolio pieces, six-minute oral defence recording and Grade 9 transition reflection.\n" + CAPSTONE_MARK_GUIDANCE, submissionType: "Portfolio + inquiry product + source record + oral defence", marks: 72, outcomeIds: outcomes.map((outcome) => outcome.outcomeId).join(" | "), rubricIds: "rub-writing-v1 | rub-speaking-v1", origin: "Ehel Grade 8 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], quizzes, liveSessions, teacherNotes: [{ teacherNoteId: `${id}-note01`, unitId: id, noteType: "Capstone moderation", note: "Conference at each milestone, verify responsible attribution and source evaluation, preserve learner ownership, assess the inquiry product and six-minute oral defence with shared rubrics, and record actionable Grade 9 transition feedback.", visibility: "Teacher", origin: "Curriculum review", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], answerKey: [...activities.map((activity) => ({ answerId: `${activity.activityId}-answer`, unitId: id, contentId: activity.activityId, contentType: "Capstone milestone", answerOrGuidance: activity.answerSummary, origin: activity.origin, reviewStatus: "Approved v1.2", sourceFile: activity.sourceFile })), ...quizzes.map((quiz) => ({ answerId: `${quiz.questionId}-answer`, unitId: id, contentId: quiz.questionId, contentType: "Quiz", answerOrGuidance: `${quiz.correctAnswer} ${quiz.explanation}`, origin: quiz.origin, reviewStatus: "Approved v1.2", sourceFile: quiz.sourceFile }))], selfAssessment: outcomes.map((outcome, index) => ({ selfAssessmentId: `${id}-self0${index + 1}`, unitId: id, sequence: index + 1, statement: outcome.learningOutcome.replace(/^(Select|Use|Analyse|Create|Present|Evaluate)/, (verb) => `I can ${verb.toLowerCase()}`), scale: "Not yet | With help | By myself", outcomeId: outcome.outcomeId, origin: "Capstone reflection", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" })) };
}

function buildFinalAssessment(units) {
  const selected = units.slice(0, 9).flatMap((unit) => unit.quizzes.slice(0, 4)).slice(0, 30);
  const sections = [
    { sectionId: "grade8-final-sec01", sequence: 1, title: "Words and Meaning", description: "Use Grade 8 vocabulary accurately in familiar and new contexts.", startQuestion: 1, endQuestion: 10, questionCount: 10 },
    { sectionId: "grade8-final-sec02", sequence: 2, title: "Reading and Language", description: "Show reading understanding and control of Grade 8 language patterns.", startQuestion: 11, endQuestion: 20, questionCount: 10 },
    { sectionId: "grade8-final-sec03", sequence: 3, title: "Communication and Application", description: "Apply English to speaking, writing and purposeful communication.", startQuestion: 21, endQuestion: 30, questionCount: 10 },
  ];
  const questions = selected.map((quiz, index) => {
    const sourceUnit = units.find((unit) => unit.unit.unitId === quiz.unitId);
    return { ...quiz, assessmentId: "eng-g08-course-final-quiz-v12", quizId: "eng-g08-course-final-quiz-v12", questionId: `eng-g08-final-q${String(index + 1).padStart(2, "0")}`, sequence: index + 1, sectionId: sections[Math.floor(index / 10)].sectionId, sourceUnitNo: sourceUnit?.unit.unitNo, sourceUnitId: quiz.unitId, sourceUnitTitle: sourceUnit?.unit.unitTitle, curriculumArea: Math.floor(index / 10) === 0 ? "Vocabulary and meaning" : Math.floor(index / 10) === 1 ? "Reading and language" : "Communication and application", reviewRoute: `Review Unit ${sourceUnit?.unit.unitNo || 1}`, audio: pendingAudio(`./media/audio/final-quiz/eng-g08-final-q${String(index + 1).padStart(2, "0")}.mp3`) };
  });
  return { schemaVersion: "Ehel English Course Final Assessment v1.2", assessmentId: "eng-g08-course-final-quiz-v12", gradeId: "g08", subject: "English", title: "Grade 8 English Final Course Quiz", description: "A cumulative assessment of vocabulary, reading, language and communication across Units 1-9.", placement: "After Unit 10 capstone", questionCount: 30, totalMarks: 30, estimatedMinutes: 30, passPercent: 80, attemptsAllowed: "Teacher or school policy", sections, questions, reviewStatus: "Approved v1.2", audioProvider: "ElevenLabs", voiceId: VOICE_ID };
}
...
fs.writeFileSync(path.join(DATA_DIR, "master-dictionary.grade8.json"), JSON.stringify(dictionary, null, 2));
fs.writeFileSync(path.join(DATA_DIR, "course-final-quiz.json"), JSON.stringify(finalAssessment, null, 2));
console.log(`Built Grade 8 English: ${units.length} units, ${dictionary.entryCount} dictionary entries, ${finalAssessment.questionCount}-question final quiz.`);
