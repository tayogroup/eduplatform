...
if ($workspaceid <= 0 || !pqhh_user_can_assign((int)$USER->id, $workspaceid)) {
    pqh_access_denied('Choose a teacher workspace before opening Homework.', new moodle_url('/local/hubredirect/live_teacher.php', $urlparams), 'Homework access required');
}
if (!pqhh_ready()) {
    pqh_access_denied('Homework is waiting for the EduPlatform database upgrade.', new moodle_url('/local/hubredirect/live_teacher.php', $urlparams), 'Homework setup required');
}
...
<main class="pqhh-shell"><div class="pqhh-wrap">
  <header class="pqhh-head"><div><h1>Homework</h1><p>Create course work, review submissions, and publish grades.</p></div><a class="pqhh-btn" href="<?php echo (new moodle_url('/local/hubredirect/live_teacher.php', $urlparams))->out(false); ?>">Teacher workspace</a></header>
