...
$studentid = optional_param('studentid', (int)$USER->id, PARAM_INT);
$urlparams = [];
...
if ($workspaceid <= 0 || $studentid <= 0 || !pqfin_student_in_workspace($studentid, $workspaceid)) {
    pqh_access_denied('Student billing is not available for this workspace.', new moodle_url('/local/hubredirect/dashboard.php'), 'Billing unavailable');
}
if ((int)$USER->id !== $studentid && !pqfin_user_can_manage_workspace_finance((int)$USER->id, $workspaceid)) {
    pqh_access_denied('Only the student or a workspace finance admin can view this billing dashboard.', new moodle_url('/local/hubredirect/dashboard.php'), 'Billing access required');
}
...
$invoices = pqfin_invoice_rows_for_student($workspaceid, $studentid, (int)$USER->id, $consumercontext);
$metrics = pqfin_invoice_amount_metrics($invoices);

$PAGE->set_context(context_system::instance());
$PAGE->set_url(new moodle_url('/local/hubredirect/student_billing.php', $urlparams));
$PAGE->set_pagelayout('standard');
$PAGE->set_title('Student Billing');
$PAGE->set_heading('Student Billing');
...
<style><?php echo pqh_openproject_skin_css('pqb', 'pqbill-page'); ?></style>
<main class="pqb-shell"><div class="pqb-wrap">
  <section class="pqb-top">
    <div><h1 class="pqb-title">Student Billing</h1><p class="pqb-sub"><?php echo s(fullname($student)); ?> / <?php echo s((string)$workspace->name); ?></p></div>
    <nav class="pqb-actions">
      <a class="pqb-btn" href="<?php echo (new moodle_url('/local/hubredirect/workspace_student.php', $urlparams))->out(false); ?>">Student profile</a>
      <a class="pqb-btn" href="<?php echo (new moodle_url('/local/hubredirect/dashboard.php'))->out(false); ?>">Dashboard</a>
    </nav>
  </section>
  <section class="pqb-metrics">
    <div class="pqb-metric"><strong><?php echo (int)$metrics['count']; ?></strong><span>Invoices</span></div>
    <div class="pqb-metric"><strong><?php echo s((string)$metrics['total']); ?></strong><span>Total billed</span></div>
    <div class="pqb-metric"><strong><?php echo s((string)$metrics['paid']); ?></strong><span>Paid or credited</span></div>
    <div class="pqb-metric"><strong><?php echo s((string)$metrics['balance']); ?></strong><span>Balance due</span></div>
  </section>
  <section class="pqb-panel">
    ...
    <?php if ($invoices): ?>
      <table class="pqb-table"><thead><tr><th>Invoice</th><th>Status</th><th>Total</th><th>Paid</th><th>Balance</th><th>Due</th><th>Plan</th><th></th></tr></thead><tbody>
        <?php foreach ($invoices as $invoice): ?>
          <?php $plan = pqfin_active_payment_plan_for_invoice((int)$invoice->id); ?>
          <tr>
            <td data-label="Invoice"><span class="pqb-name"><?php echo s((string)$invoice->invoicenumber); ?></span><span class="pqb-muted"><?php echo s((string)$invoice->workspace_name); ?></span></td>
            <td data-label="Status"><span class="pqb-pill"><?php echo s(pqfin_invoice_status_label((string)$invoice->status)); ?></span></td>
            <td data-label="Total"><?php echo s((string)$invoice->currency . ' ' . (string)$invoice->total); ?></td>
            <td data-label="Paid"><?php echo s((string)$invoice->paidamount); ?></td>
            <td data-label="Balance"><?php echo s((string)$invoice->balancedue); ?></td>
            <td data-label="Due"><?php echo (int)$invoice->dueat > 0 ? s(userdate((int)$invoice->dueat, get_string('strftimedate'))) : 'Not set'; ?></td>
            <td data-label="Plan"><?php echo $plan ? '<span class="pqb-pill">' . s(pqfin_payment_plan_status_label((string)$plan->status)) . '</span><span class="pqb-muted">' . s((string)$plan->plannumber) . '</span>' : '<span class="pqb-muted">No plan</span>'; ?></td>
            <td data-label="Open"><a class="pqb-btn" href="<?php echo (new moodle_url('/local/hubredirect/invoice_view.php', ['invoiceid' => (int)$invoice->id] + $urlparams))->out(false); ?>">View</a></td>
          </tr>
        <?php endforeach; ?>
      </tbody></table>
    <?php endif; ?>
