  ...
  const QS = [
    ...
    /* 2026-09-12: an eighth question, so the pass mark (three quarters, rounded
       up - _app.pass_mark) lands on 6 of 8 = 75% rather than 6 of 7 = 86%. It
       tests "Ten times bigger", which the check had never asked about. */
    () => { const n = rnd(12, 89); return { q: n + " × 10 = ?", opts: [n * 10, n * 100, n + 10], a: n * 10, why: "Every digit moves one place to the left, so " + n + " tens is " + (n * 10) + "." }; },
  ];
;
