...
const GRADE_DIR = path.join(ROOT, "src", "prototypes", "ehel-academy", "english", "grade-4");
const DATA_DIR = path.join(GRADE_DIR, "data");
const UNIT_DIR = path.join(DATA_DIR, "units");
const SOURCE_FILE = path.join(ROOT, "inputs", "ehel-grade4-source", "grade4-source-extracted.json");
...
const gradeProfile = templateConfig.gradeProfiles.g04;
if (!gradeProfile || gradeProfile.writingExpectation !== "1-2 organised paragraphs") throw new Error("Grade 4 neutral profile is missing or invalid.");
...
const visualMap = {
  1: ["unit-1-welcome-calendar.png", "A learner organising a daily routine and practising clear communication"],
  2: ["unit-4-big-sky.png", "Learners investigating weather patterns and changes in nature"],
  3: ["unit-3-ready-steady-go.png", "Children discussing balanced food, health and active choices"],
  4: ["unit-9-city.png", "Children communicating while exploring services in their community"],
  5: ["unit-3-ready-steady-go.png", "Children using action words during active learning"],
  6: ["unit-2-neighbours-jobs.png", "Learners exploring the roles and contributions of people in society"],
  7: ["unit-8-home.png", "Children expressing emotions, behaviour and identity respectfully"],
  8: ["unit-5-measure.png", "Learners investigating tools, machines and useful everyday items"],
  9: ["unit-9-city.png", "Learners planning routes and describing places and people"],
...
function unitId(unitNo) {
  return `eng-g04-${termFor(unitNo).id}-u${String(unitNo).padStart(2, "0")}`;
}

function doc(unitNo, type) {
  const record = sourceByPath.get(`Unit ${unitNo}/Unit ${unitNo} - ${type}.docx`);
  if (!record) throw new Error(`Missing Grade 4 source: Unit ${unitNo} ${type}`);
  return record;
}
...
function sourcePath(unitNo, type) {
  return `inputs/ehel-grade4-source/Year 4/Unit ${unitNo}/Unit ${unitNo} - ${type}.docx`;
}
...
function extractReadings(unitNo) {
  ...
  const headingIndexes = paragraphs.map((line, index) => /^(Reading|Listening)(?: \d+)?\s*[—-]/.test(line) ? index : -1).filter((index) => index >= 0);
...
function grammarBlocks(unitNo) {
  ...
  while (blocks.length < 6) {
    ...
    blocks.push([
      `Lesson ${sequence}: Grade 4 Editing and Application`,
      ...
      `Practice ${sequence}`,
      "Edit a six-sentence paragraph using the unit language focus. Mark four changes and explain how two changes improve clarity.",
    ]);
  }
...
function grammarRecord(unitNo, block, sequence) {
  ...
  return {
    ...
    origin: "Authored Grade 4 source",
...
function buildTasks(unitNo, kind) {
  ...
  return tasks.slice(0, 6).map(([title, body], index) => {
    ...
    const common = { unitId: unitId(unitNo), sequence, title: `${kind[0].toUpperCase() + kind.slice(1)} ${sequence}: ${title}`, outcomeId: `${unitId(unitNo)}-lo${String(Math.min(sequence, 6)).padStart(2, "0")}`, origin: index < sourceTasks.length ? "Authored Grade 4 source" : "Ehel English Content Template v1.2 curriculum expansion", reviewStatus: "Approved v1.2", sourceFile: index < sourceTasks.length ? sourcePath(unitNo, "Lesson") : "Ehel English Content Template v1.2" };
    ...
    if (kind === "writing") return { writingId: id, ...common, promptAndInstructions: body, modelText: `A strong Grade 4 response uses organised paragraphs, precise unit words, relevant evidence or examples, and a purposeful conclusion about ${title.toLowerCase()}.`, sentenceStarter: "I would like to explain...", expectedLength: sequence <= 2 ? "One organised paragraph" : gradeProfile.writingExpectation, successCriteria: "I answered the prompt; organised ideas into paragraphs; used precise Unit vocabulary; supported ideas with details or evidence; checked cohesion and punctuation; revised my work", support: "Plan a topic sentence and supporting details, then use the word bank and a source model.", extension: "Vary sentence openings, compare two ideas, and strengthen the conclusion.", rubricId: "rub-writing-v1" };
...
function buildCapstone(regularUnits, dictionaryEntries) {
  ...
  const id = unitId(unitNo);
  const term = termFor(unitNo);
  const selectedLinks = regularUnits.flatMap((unit) => unit.dictionaryLinks.slice(0, 4)).slice(0, 30).map((link, index) => ({ ...link, vocabularyId: `u10-g1-${index + 1}-${slug(dictionaryEntries.get(link.dictionaryEntryId).displayWord)}`, unitId: id, termId: term.id, groupId: "u10-g1-grade-4-review-words", groupTitle: "Grade 4 Review Words", sequence: index + 1 }));
  const title = "My English Voice: Grade 4 Capstone";
  const outcomes = ["Select strong evidence from Units 1-9 and explain why it shows progress.", "Use Grade 4 vocabulary accurately across speaking and writing.", "Read a chosen passage fluently and answer questions with text evidence.", "Create a connected multi-paragraph English product for a real audience.", "Present ideas clearly, respond to questions and use feedback.", "Reflect honestly and set a specific Grade 5 English goal."].map((learningOutcome, index) => ({ outcomeId: `${id}-lo0${index + 1}`, unitId: id, sequence: index + 1, learningOutcome, bloomLevel: index < 2 ? "Apply" : index < 4 ? "Analyse and create" : "Evaluate", evidenceOfLearning: "Capstone portfolio, product, presentation and reflection", origin: "Ehel Grade 4 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  const makeSix = (kind, items) => items.map((item, index) => {
    ...
    const common = { unitId: id, sequence: index + 1, title: item[0], outcomeId: outcomes[index].outcomeId, origin: "Ehel Grade 4 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" };
  ...
  while (comprehension.length < 12) comprehension.push({ ...comprehension[comprehension.length % 10], questionId: `${id}-cq${String(comprehension.length + 1).padStart(3, "0")}`, sequence: comprehension.length + 1, question: comprehension.length === 10 ? "Compare two review texts. What important idea do they share?" : "Which Grade 4 text influenced your capstone most, and why?" });
  const quizzes = regularUnits.flatMap((unit) => unit.quizzes.slice(0, 2)).slice(0, 10).map((quiz, index) => ({ ...quiz, quizId: `${id}-quiz${String(index + 1).padStart(2, "0")}`, questionId: `${id}-quiz${String(index + 1).padStart(2, "0")}-q01`, unitId: id, quizTitle: "Grade 4 capstone review checkpoint", sequence: index + 1 }));
  const liveSessions = ["Launch and portfolio curation", "Reading and vocabulary conference", "Project planning and first draft", "Grammar editing and peer feedback", "Presentation rehearsal", "Capstone exhibition and reflection"].map((title, index) => ({ liveSessionId: `${id}-live0${index + 1}`, unitId: id, week: index < 3 ? 1 : 2, sessionWithinWeek: (index % 3) + 1, sessionNo: index + 1, title, durationMin: 45, beforeSession: "Complete the preceding capstone milestone and bring evidence.", agenda: "5 min check-in; 10 min model; 15 min workshop; 10 min conference or rehearsal; 5 min next step", afterSession: index === 5 ? "Complete the final course quiz." : "Apply feedback and upload the improved milestone.", outcomeIds: outcomes[index].outcomeId, origin: "Ehel Grade 4 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }));
  return { schemaVersion: "Ehel English Runtime v1.2", templateVersion: "Ehel English Content Template v1.2", dictionaryVersion: "Ehel Master Dictionary v1.2", generatedAt: GENERATED_AT, grade: { id: "g04", label: "Grade 4" }, subject: "English", term, unit: { gradeId: "g04", subject: "English", termId: term.id, unitId: id, unitNo, unitTitle: title, unitOverview: "Bring together your strongest vocabulary, reading, grammar, speaking and writing from Units 1-9. Create a polished English product for a real audience, present it clearly, respond to questions and reflect on your Grade 4 growth.", learningPath: "Choose evidence. Review language. Plan and draft. Improve from feedback. Present. Reflect. Complete the final course quiz.", origin: "Ehel Grade 4 capstone specification", reviewStatus: "Approved v1.2 capstone", sourceFile: "Ehel English Content Template v1.2" }, visual: { image: "../grade-2/capstone-my-english-world.png", alt: visualMap[10][1], lectureMode: "capstone-launch" }, vocabularyGroups: [{ id: "u10-g1-grade-4-review-words", number: 1, title: "Grade 4 Review Words", vocabularyIds: selectedLinks.map((link) => link.vocabularyId) }], dictionaryLinks: selectedLinks, rubrics: baseRubrics.map((rubric) => ({ ...rubric, origin: "Ehel Grade 4 approved rubric v1.2" })), outcomes, readings, comprehension, grammar, speaking, writing, activities, assignments: [{ assignmentId: `${id}-assignment01`, unitId: id, title: "Grade 4 capstone portfolio and presentation", instructions: "Submit the final product, three portfolio pieces, presentation recording and reflection.", submissionType: "Portfolio + product + presentation", marks: 64, outcomeIds: outcomes.map((outcome) => outcome.outcomeId).join(" | "), rubricIds: "rub-writing-v1 | rub-speaking-v1", origin: "Ehel Grade 4 capstone specification", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], quizzes, liveSessions, teacherNotes: [{ teacherNoteId: `${id}-note01`, unitId: id, noteType: "Capstone moderation", note: "Conference at each milestone, preserve learner ownership, assess the final product and presentation with shared rubrics, and record actionable Grade 4 transition feedback.", visibility: "Teacher", origin: "Curriculum review", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" }], answerKey: [...activities.map((activity) => ({ answerId: `${activity.activityId}-answer`, unitId: id, contentId: activity.activityId, contentType: "Capstone milestone", answerOrGuidance: activity.answerSummary, origin: activity.origin, reviewStatus: "Approved v1.2", sourceFile: activity.sourceFile })), ...quizzes.map((quiz) => ({ answerId: `${quiz.questionId}-answer`, unitId: id, contentId: quiz.questionId, contentType: "Quiz", answerOrGuidance: `${quiz.correctAnswer} ${quiz.explanation}`, origin: quiz.origin, reviewStatus: "Approved v1.2", sourceFile: quiz.sourceFile }))], selfAssessment: outcomes.map((outcome, index) => ({ selfAssessmentId: `${id}-self0${index + 1}`, unitId: id, sequence: index + 1, statement: outcome.learningOutcome.replace(/^(Select|Use|Read|Create|Present|Reflect)/, (verb) => `I can ${verb.toLowerCase()}`), scale: "Not yet | With help | By myself", outcomeId: outcome.outcomeId, origin: "Capstone reflection", reviewStatus: "Approved v1.2", sourceFile: "Ehel English Content Template v1.2" })) };
}

function buildFinalAssessment(units) {
  const selected = units.slice(0, 9).flatMap((unit) => unit.quizzes.slice(0, 4)).slice(0, 30);
  const sections = [
    { sectionId: "grade4-final-sec01", sequence: 1, title: "Words and Meaning", description: "Use Grade 4 vocabulary accurately in familiar and new contexts.", startQuestion: 1, endQuestion: 10, questionCount: 10 },
    { sectionId: "grade4-final-sec02", sequence: 2, title: "Reading and Language", description: "Show reading understanding and control of Grade 4 language patterns.", startQuestion: 11, endQuestion: 20, questionCount: 10 },
    { sectionId: "grade4-final-sec03", sequence: 3, title: "Communication and Application", description: "Apply English to speaking, writing and purposeful communication.", startQuestion: 21, endQuestion: 30, questionCount: 10 },
  ];
  const questions = selected.map((quiz, index) => {
    const sourceUnit = units.find((unit) => unit.unit.unitId === quiz.unitId);
    return { ...quiz, assessmentId: "eng-g04-course-final-quiz-v12", quizId: "eng-g04-course-final-quiz-v12", questionId: `eng-g04-final-q${String(index + 1).padStart(2, "0")}`, sequence: index + 1, sectionId: sections[Math.floor(index / 10)].sectionId, sourceUnitNo: sourceUnit?.unit.unitNo, sourceUnitId: quiz.unitId, sourceUnitTitle: sourceUnit?.unit.unitTitle, curriculumArea: Math.floor(index / 10) === 0 ? "Vocabulary and meaning" : Math.floor(index / 10) === 1 ? "Reading and language" : "Communication and application", reviewRoute: `Review Unit ${sourceUnit?.unit.unitNo || 1}`, audio: pendingAudio(`./media/audio/final-quiz/eng-g04-final-q${String(index + 1).padStart(2, "0")}.mp3`) };
  });
  return { schemaVersion: "Ehel English Course Final Assessment v1.2", assessmentId: "eng-g04-course-final-quiz-v12", gradeId: "g04", subject: "English", title: "Grade 4 English Final Course Quiz", description: "A cumulative assessment of vocabulary, reading, language and communication across Units 1-9.", placement: "After Unit 10 capstone", questionCount: 30, totalMarks: 30, estimatedMinutes: 30, passPercent: 80, attemptsAllowed: "Teacher or school policy", sections, questions, reviewStatus: "Approved v1.2", audioProvider: "ElevenLabs", voiceId: VOICE_ID };
}
...
const dictionary = { schemaVersion: "Ehel Master Dictionary v1.2", language: "en-GB", gradeId: "g04", entryCount: dictionaryEntries.size, entries: [...dictionaryEntries.values()] };
const finalAssessment = buildFinalAssessment(units);
const manifest = { schemaVersion: "Ehel Grade 4 English Course Manifest v1.2", grade: { id: "g04", label: "Grade 4" }, subject: "English", defaultUnit: 1, units: units.map((unit) => ({ number: unit.unit.unitNo, id: unit.unit.unitId, termId: unit.term.id, title: unit.unit.unitTitle, data: `./data/units/unit-${unit.unit.unitNo}.json`, vocabularyCount: unit.dictionaryLinks.length, reviewStatus: unit.unit.reviewStatus })), finalAssessment: { id: finalAssessment.assessmentId, title: finalAssessment.title, data: "./data/course-final-quiz.json", placement: finalAssessment.placement, questionCount: 30, passPercent: 80, reviewStatus: finalAssessment.reviewStatus } };
...
fs.writeFileSync(path.join(DATA_DIR, "master-dictionary.grade4.json"), JSON.stringify(dictionary, null, 2));
fs.writeFileSync(path.join(DATA_DIR, "course-final-quiz.json"), JSON.stringify(finalAssessment, null, 2));
console.log(`Built Grade 4 English: ${units.length} units, ${dictionary.entryCount} dictionary entries, ${finalAssessment.questionCount}-question final quiz.`);
