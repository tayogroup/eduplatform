...
const gradeDir = path.join(root, "src", "prototypes", "ehel-academy", "english", "grade-8");
...
const dictionary = JSON.parse(fs.readFileSync(path.join(dataDir, "master-dictionary.grade8.json"), "utf8"));
const finalAssessment = JSON.parse(fs.readFileSync(path.join(dataDir, "course-final-quiz.json"), "utf8"));
const source = JSON.parse(fs.readFileSync(path.join(root, "inputs", "ehel-grade8-source", "grade8-source-extracted.json"), "utf8"));
...
if (source.documentCount !== 36) throw new Error(`Expected 36 Grade 8 source documents; found ${source.documentCount}.`);
if (manifest.units.length !== 10 || manifest.units[0].number !== 1 || manifest.units[9].number !== 10) throw new Error("Grade 8 manifest must contain Units 1-9 and Unit 10 capstone.");
if (ids.size !== dictionary.entries.length || dictionary.entryCount !== dictionary.entries.length) throw new Error("Grade 8 dictionary IDs or count are invalid.");
if (/speechSynthesis|SpeechSynthesisUtterance/.test(uiSource)) throw new Error("Browser-generated voice remains in the Grade 8 UI.");
if (!indexSource.includes('data-grade="8"') || !indexSource.includes("../shared/grade-redirect.js")) throw new Error("Grade 8 shared UI redirect is missing.");
...
for (const summary of manifest.units) {
  ...
  if (summary.number < 10 && (runtime.dictionaryLinks.length < 25 || runtime.dictionaryLinks.length > 55)) throw new Error(`Unit ${summary.number} vocabulary count is outside the Grade 8 profile.`);
  if (summary.number < 10 && runtime.readings.filter((reading) => reading.passageScript.trim().split(/\s+/).length >= 550).length < 3) throw new Error(`Unit ${summary.number} lacks sufficient extended Grade 8 reading.`);
  if (runtime.writing.some((task) => !/paragraph/i.test(task.expectedLength))) throw new Error(`Unit ${summary.number} writing length is not aligned to the Grade 8 profile.`);
  if (summary.number < 10 && !runtime.speaking.some((task) => /six-minute|5-6 minute|five- to six-minute/i.test(`${task.title} ${task.instructionsAndModelLines}`))) throw new Error(`Unit ${summary.number} lacks the Grade 8 speaking target.`);
...
const unitOne = JSON.parse(fs.readFileSync(path.join(dataDir, "units", "unit-1.json"), "utf8"));
if (unitOne.visual.lectureMode !== "video" || unitOne.visual.lectureVoiceId !== "XfNU2rGpBa01ckF309OY") throw new Error("Grade 8 Unit 1 teacher lecture metadata is invalid.");
for (const key of ["lectureVideo", "lecturePoster", "lectureCaptions"]) {
  const lectureAsset = path.resolve(gradeDir, unitOne.visual[key]);
  if (!fs.existsSync(lectureAsset) || fs.statSync(lectureAsset).size === 0) throw new Error(`Missing Grade 8 Unit 1 ${key}: ${lectureAsset}`);
}
...
if (finalAssessment.questionCount !== 30 || finalAssessment.questions.length !== 30 || finalAssessment.sections.length !== 3 || finalAssessment.passPercent !== 80) throw new Error("Grade 8 final quiz structure is invalid.");
...
const workbook = path.join(root, "outputs", "019f5d39-7fcd-7f23-a425-201fe8206eef", "Ehel-English-Content-Template-v1.2-Grade-8-All-Units-Reference.xlsx");
if (!fs.existsSync(workbook) || fs.statSync(workbook).size <= 10_000) throw new Error(`Missing or invalid Grade 8 workbook: ${workbook}`);
