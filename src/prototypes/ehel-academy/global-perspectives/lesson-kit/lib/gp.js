  ...
  const SCENES = {
    ...
    park: { w: 320, h: 240, label: "The park", bg: SKY(240) + GROUND(120) +
      '<path d="M0 220 q160 -60 320 -10 v30 h-320z" fill="#C9B79C"/><circle cx="270" cy="44" r="24" fill="#F4C95D"/>' +
  ...
  function sortBins(o) {
    ...
    function draw() {
      ...
      $(el.stage).innerHTML = '<div class="stagewide">' + (o.chart ? '<p class="phase">' + esc(o.title) + "</p>" : "") +
        ...
        '<div class="bins' + (o.chart ? " chartcols" : "") + '">' + o.bins.map((b) => '<button type="button" class="bin" data-b="' + b.id + '"><span class="bpic" aria-hidden="true">' + small(b.pic) + "</span>" + esc(b.label) + tray(b) + "</button>").join("") + "</div></div>";
  ...
  function questionBuilder(o) {
    ...
    function askIt() {
      ...
      if (word === rd.word && end === rd.end) {
        ...
        const line = "That is a good question: " + question + " But it asks about " + (e.asks || "something else") + ". You want to know " + lower1(noDot(rd.want)) + ". Try again.";
  ...
  function pictureSource(o) {
    ...
    let asked = false, active = null;
    function draw() {
      $(el.stage).innerHTML = '<div class="stagewide"><div class="sim picbox">' + sceneSvg(o.scene, o.spots, found, active) + "</div>" +
        '<p class="sub">' + esc(o.caption || "Tap the things in the picture to find out what it tells us.") + "</p></div>";
      $(el.score).textContent = found.size + " of " + need + " found";
    }
    $(el.stage).addEventListener("click", (e) => {
      const g = e.target.closest("[data-spot]"); if (!g || asked) return;
      ...
      if (found.size >= need && !asked) {
        asked = true;
        setTimeout(() => {
          active = null; draw(); $(el.fb).textContent = "";
          askOnce(o, o.then, (ok) => { reportScore(o.finish, ok ? 1 : 0, 1); endStep(o); }, (c) => !!c.spot);
        }, 2600);
      }
    });
  ...
  function pictogramRead(o) {
    const el = o.el;
    let i = 0, right = 0, lock = false;
    $(el.stage).innerHTML = '<div class="stagewide"><div class="picto"><p class="pictotitle">' + esc(o.title) + "</p>" + o.rows.map((r) =>
      ...
      '<p class="sub">Each picture is one person.</p></div></div>';
    function draw() {
      lock = false;
      const it = o.items[i];
      ...
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      $(el.score).textContent = "Look at the pictogram: question " + (i + 1) + " of " + o.items.length;
      sayHere(o.finish, plain(it.ask) + " Look at the pictogram.");
    }
    $(el.ch).addEventListener("click", (e) => {
      const b = e.target.closest(".choice"); if (!b || lock) return;
      lock = true;
      ...
      $(el.fb).className = "fb " + (ok ? "good" : "bad"); $(el.fb).textContent = (ok ? cheer() + " " : "Count the pictures again. ") + it.why; say($(el.fb).textContent);
      i++;
      setTimeout(() => {
        if (i >= o.items.length) {
          reportScore(o.finish, right, o.items.length);
          endStep(o, "You answered " + right + " of " + o.items.length + " from the pictogram. " + o.done);
        } else draw();
      }, 2700);
    });
    draw();
  ...
  function knowBoard(o) {
    ...
    function draw() {
      $(el.stage).innerHTML = '<div class="stagewide"><div class="board"><p class="phase">What we know about <b>' + esc(o.topic) + "</b> " + small(o.topicPic || "") + "</p>" +
        ...
        (phase === "pick" ? '<div class="cardsgrid">' + cards.filter((c) => !onBoard.includes(c.k)).map((c) => '<button type="button" class="tapcard" data-k="' + c.k + '"><span class="cpic" aria-hidden="true">' + small(c.pic) + "</span>" + esc(c.t) + "</button>").join("") + "</div>" : "") + "</div>";
      $(el.score).textContent = phase === "pick" ? onBoard.length + " of " + need + " on the board" : "Tap a fact on the board to say it";
    }
    $(el.stage).addEventListener("click", (e) => {
      ...
      if (c.about === o.tag) {
        ...
        const line = "I know that " + lower1(noDot(c.say)) + ".";
        ...
        if (onBoard.length >= need) {
          phase = "say";
          setTimeout(() => { draw(); $(el.ask).innerHTML = "Now say one out loud. <b>Tap a fact on the board.</b>"; $(el.fb).textContent = ""; sayHere(o.finish, "Now say one out loud. Tap a fact on the board."); }, 2600);
        }
      } else {
        missed++; b.classList.add("wrong"); SOUND.play("error", 0.3);
        const line = "That is about " + (c.aboutLabel || "something else") + ", not " + o.topic + ". Only things about " + o.topic + " go on the board.";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        setTimeout(() => b.classList.remove("wrong"), 700);
      }
    });
  ...
  function consequences(o) {
    ...
    function reveal() {
      ...
      r++;
      setTimeout(() => {
        if (r >= rounds.length) { reportScore(o.finish, right, rounds.length); endStep(o, "You thought about " + rounds.length + " things you might do, and what would happen to you. " + o.done); }
        else start();
      }, 4200);
    }
    $(el.ch).addEventListener("click", (e) => {
  ...
  function pickSource(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, phase = "pick", lock = false, missed = false, score = 0;
    function draw() {
      const rd = rounds[r];
      $(el.stage).innerHTML = '<div class="stagewide"><div class="wantcard"><span class="cpic" aria-hidden="true">' + small(rd.pic) + '</span><div><p class="phase">Find out about</p><b>' + esc(rd.topic) + "</b></div></div>" +
        '<div class="cardsgrid shelfgrid">' + shuffle(rd.sources).map((s) => '<button type="button" class="tapcard" data-s="' + esc(s.id) + '"' + (phase !== "pick" ? " disabled" : "") + '><span class="cpic" aria-hidden="true">' + small(s.pic) + "</span>" + esc(s.label) + "</button>").join("") + "</div></div>";
      $(el.score).textContent = "Topic " + (r + 1) + " of " + rounds.length;
    }
    function start() {
      phase = "pick"; lock = false; missed = false;
      $(el.ch).innerHTML = ""; $(el.ch).className = "choices"; $(el.fb).textContent = ""; $(el.fb).className = "fb";
      draw();
      $(el.ask).innerHTML = "We want to find out about <b>" + esc(rounds[r].topic) + "</b>. Which one would help? Tap it.";
      sayHere(o.finish, "We want to find out about " + rounds[r].topic + ". Which one would help? Tap it.");
    }
    $(el.stage).addEventListener("click", (e) => {
      ...
      if ((s.about || []).includes(rd.tag)) {
        lock = true; if (!missed) score++;
        b.classList.add("heard", "now"); SOUND.play("ding", 0.4);
        const line = cheer() + " " + s.say;
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        setTimeout(() => {
          phase = "why"; $(el.fb).textContent = "";
          askOnce(o, { ask: "Why is <b>" + esc(s.label) + "</b> a good one for " + esc(rd.topic) + "?", opts: rd.reasons, why: rd.why }, (ok) => {
            if (ok) score++;
            r++;
  ...
  function opinion(o) {
    const el = o.el, rounds = o.rounds;
    let r = 0, stance = null, said = 0, lock = false;
    function draw() {
      const rd = rounds[r];
      $(el.stage).innerHTML = '<div class="stagewide"><div class="wantcard"><span class="cpic" aria-hidden="true">' + small(rd.pic) + '</span><div><p class="phase">What do you think about</p><b>' + esc(rd.topic) + "?</b></div></div>" +
        '<div class="qline" aria-live="polite">' + (stance ? '<span class="chip qword">' + esc(stance.t) + "</span>" : '<span class="hint">I think…</span>') + " " + '<span class="hint">because…</span></div></div>';
      $(el.score).textContent = "Topic " + (r + 1) + " of " + rounds.length;
    }
    function start() {
      const rd = rounds[r];
      stance = null; lock = false;
      $(el.fb).textContent = ""; $(el.fb).className = "fb";
      draw();
    ...
    function reasons() {
      const rd = rounds[r];
      $(el.ask).innerHTML = "You think: <b>" + esc(stance.t) + "</b>. Now say <b>why</b>. Pick a reason that is about " + esc(rd.topic) + ".";
      $(el.ch).className = "choices stack";
      ...
      sayHere(o.finish, "You think: " + stance.t + ". Now say why. Pick a reason that is about " + rd.topic + ".");
    }
    $(el.ch).addEventListener("click", (e) => {
      const b = e.target.closest(".choice"); if (!b || lock) return;
      const rd = rounds[r];
      ...
      if (b.dataset.about === rd.tag) {
        ...
        SOUND.play("tada", 0.4);
        const sentence = stance.t + ", " + b.textContent + ".";
        $(el.stage).querySelector(".qline").innerHTML = '<span class="chip qword">' + esc(stance.t) + '</span> <span class="chip qend">' + esc(b.textContent) + "</span>";
        sayLine(el, "You said: <b>" + esc(sentence) + "</b> That is an opinion with a reason.", "You said: " + sentence + " That is an opinion with a reason.");
  ...
  function teamBuild(o) {
    ...
    logs[o.finish] = log;
    let r = 0, grown = 0, lock = false, missed = false, score = 0;
    ...
    function start() {
      ...
      if (rd.kind === "share") {
      ...
      } else if (rd.kind === "work") {
        $(el.ask).innerHTML = "<b>" + esc(rd.situation) + "</b> What do you say?";
        $(el.ch).className = "choices stack";
        ...
        sayHere(o.finish, rd.situation + " What do you say?");
      } else {
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      if (rd.kind === "share") {
        ...
        if (both) {
          lock = true; if (!missed) score++;
          $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
          log.push({ who: "you", text: rd.log || b.textContent, pic: rd.resource.pic });
        ...
        if (opt.good) {
          lock = true; if (!missed) score++;
          $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
          log.push({ who: "you", text: rd.log || opt.log || opt.t, pic: rd.pic || "\u{1F91D}" });
          grow(cheer() + " " + rd.why);
          next();
        } else {
          missed = true; b.classList.add("wrong"); b.disabled = true; SOUND.play("error", 0.3);
          const line = opt.why || "That would not help " + f.name + ", and the job would stop. What would help?";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
        }
      }
    });
  ...
  function contributions(o) {
    ...
    function draw() {
      ...
      $(el.stage).innerHTML = '<div class="stagewide"><p class="phase">' + (phase === "mine" ? "What did YOU do to help?" : phase === "friend" ? "What did " + esc(target.name) + " do to help?" : "Who did what") + "</p>" +
      ...
      if (phase === "mine") {
        if (c.who === "you") {
          ...
          if (foundMine.size >= mine) {
            ...
            setTimeout(() => {
              lock = false;
              if (friendsIn.length) { phase = "friend"; draw(); $(el.ask).innerHTML = "Now: what did <b>" + esc(friendsIn[0].name) + "</b> do to help the team? Tap it."; sayHere(o.finish, "Now: what did " + friendsIn[0].name + " do to help the team? Tap it."); }
              else finishUp();
            }, 2400);
          }
        } else {
        ...
        if (c.who === f.id) {
          ...
          setTimeout(() => {
            ...
            else { draw(); $(el.ask).innerHTML = "And what did <b>" + esc(friendsIn[fi].name) + "</b> do?"; sayHere(o.finish, "And what did " + friendsIn[fi].name + " do?"); }
          }, 2600);
        } else {
          missed++; b.classList.add("wrong"); SOUND.play("error", 0.3);
  ...
  function lookBack(o) {
    ...
    const got = new Set();
    let phase = "learned", liked = null, missed = 0, lock = false;
    function draw() {
      if (phase === "learned") {
        $(el.stage).innerHTML = '<div class="stagewide"><p class="phase">I learned that…</p><div class="cardsgrid wide">' + pool.map((c) => '<button type="button" class="tapcard learncard' + (got.has(c.k) ? " heard" : "") + '" data-k="' + c.k + '"' + (got.has(c.k) || lock ? " disabled" : "") + '>' + (c.lesson ? '<small>Lesson ' + c.lesson + "</small>" : "") + esc(lower1(c.t)) + "</button>").join("") + "</div></div>";
        $(el.score).textContent = got.size + " of " + pick + " things learned";
      } else if (phase === "liked") {
        $(el.stage).innerHTML = '<div class="stagewide"><p class="phase">I liked…</p><div class="cardsgrid">' + o.liked.map((c, k) => '<button type="button" class="tapcard likecard' + (liked === k ? " now" : "") + '" data-l="' + k + '"><span class="cpic" aria-hidden="true">' + small(c.icon) + "</span>" + esc(c.title) + (c.lesson ? "<small>Lesson " + c.lesson + "</small>" : "") + "</button>").join("") + "</div></div>";
        $(el.score).textContent = "Tap the part you liked best";
      }
    }
    function becauses() {
      $(el.ask).innerHTML = "You liked <b>" + esc(o.liked[liked].title) + "</b>. Why? Tap a reason.";
      $(el.ch).className = "choices stack";
      $(el.ch).innerHTML = shuffle(o.becauses).map((t) => '<button type="button" class="choice text" data-why="' + esc(t) + '">' + esc(t) + "</button>").join("");
      sayHere(o.finish, "You liked " + o.liked[liked].title + ". Why? Tap a reason.");
    }
    $(el.stage).addEventListener("click", (e) => {
      ...
      if (lc && phase === "learned" && !lock) {
        ...
        if (c.ok) {
          ...
          const line = "I learned to " + lower1(noDot(c.t)) + ".";
          $(el.fb).className = "fb good"; $(el.fb).textContent = cheer() + " " + line; say(cheer() + " " + line);
          reportAttempt(o.finish, got.size, pick, "things learned");
          if (got.size >= pick) {
            lock = true;
            setTimeout(() => { lock = false; phase = "liked"; draw(); $(el.fb).textContent = ""; $(el.ask).innerHTML = "Now: which part did you <b>like</b> best? Tap it."; sayHere(o.finish, "Now: which part did you like best? Tap it."); }, 2600);
          }
        } else {
          missed++; lc.classList.add("wrong"); SOUND.play("error", 0.3);
          const line = "We did not do that today. Tap something you really did learn in this lesson.";
          $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
      SOUND.play("tada", 0.4);
      const sentence = "I liked " + o.liked[liked].title + ", " + b.dataset.why + ".";
  ...
  function relevantAnswer(o) {
    ...
    $(el.ch).addEventListener("click", (e) => {
      ...
      if (b.dataset.about === rd.about) {
        ...
        missed = true; b.classList.add("wrong"); b.disabled = true; SOUND.play("error", 0.3);
        const line = "That is true, but it is about " + (b.dataset.about || "something else") + ". " + (rd.asker ? rd.asker.name : asker.name) + " asked about " + rd.about + ". Try another.";
        $(el.fb).className = "fb bad"; $(el.fb).textContent = line; say(line);
      }
    });
    start();
  ...
  function listenAsk(o) {
    ...
    function start() {
      ...
      $(el.ch).innerHTML = ""; $(el.ch).className = "choices"; $(el.fb).textContent = ""; $(el.fb).className = "fb";
      draw();
      $(el.ask).innerHTML = "Listen to <b>" + esc(rounds[r].speaker.name) + "</b>. Then ask a question about what they said.";
      sayHere(o.finish, "Listen to " + rounds[r].speaker.name + ". Then ask a question about what they said. Press Listen.");
    }
    function listen() {
      if (lock) return;
      lock = true;
    ...
    function askPhase() {
      ...
      $(el.ask).innerHTML = "Now ask <b>" + esc(rd.speaker.name) + "</b> a question about what they said. Tap one.";
      $(el.ch).className = "choices stack";
      ...
      sayHere(o.finish, "Now ask " + rd.speaker.name + " a question about what they said. Tap one.");
    }
    $(el.ch).addEventListener("click", (e) => {
      ...
      if ((rd.topics || []).includes(b.dataset.about)) {
        ...
        $(el.ch).querySelectorAll(".choice").forEach((c) => { c.disabled = true; }); b.classList.add("right");
        SOUND.play("ding", 0.4);
        const line = cheer() + " You asked: " + b.textContent + " " + rd.why + (rd.reply ? " " + rd.speaker.name + " says: " + rd.reply : "");
        $(el.fb).className = "fb good"; $(el.fb).textContent = line; say(line);
        r++;
        setTimeout(() => {
          if (r >= rounds.length) { reportScore(o.finish, right, rounds.length); endStep(o, "You listened to " + rounds.length + " classmates and asked each one a question about what they said. " + o.done); }
          else start();
        }, 4000);
      } else {
        missed = true; b.classList.add("wrong"); b.disabled = true; SOUND.play("error", 0.3);
