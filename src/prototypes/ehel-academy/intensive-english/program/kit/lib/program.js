  ...
  /* A conversation as one utterance: a pause between speakers, and each speaker
     ...
     people (a task model can have a colleague AND a receptionist). */
  const PITCHES = ["+6%", "-10%", "+16%", "-18%"];
  function dialogueSSML(rows) {
    const who = [];
    return rows.map(([name, text]) => {
      if (who.indexOf(name) < 0) who.push(name);
      return '<prosody pitch="' + PITCHES[who.indexOf(name) % PITCHES.length] + '">' + VOICE.esc(text) + "</prosody>";
  ...
  function questions(i, el, items, o) {
    ...
    const same = (x, y) => normal(x) === normal(y);
    function settle(ok) {
      if (ok) right += 1;
    ...
    function draw() {
      const it = items[at];
      /* always shuffled: an author writes the right answer first more often
         than not (the pilot did, every time), so the written order teaches
         nothing except "tap the first one" */
      const opts = shuffle(it.options ? it.options : [it.a].concat(it.wrong || []));
  ...
  /* a typed answer, compared as a person would read it: a phone keyboard types
     a curly apostrophe (don’t), and a full stop at the end is not an error */
  function normal(s) {
    return String(s).trim().toLowerCase().replace(/[‘’ʼ`]/g, "'").replace(/[“”]/g, '"')
      .replace(/[.!?,;:]+$/, "").replace(/\s+/g, " ");
  }
  ...
  /* A plan line can name two texts ("a price list and a receipt") and the format
     has one body, so writers join them with a divider line (---, - - - -). Each
     part is its own card, the way the learner meets them in life. An article
     next to a form is not paper: `documentParts` ([false, true]) says which
     parts are documents, and without it every part follows `document`. */
  const docHTML = (t) => '<p class="eyebrow">' + esc(t.type) + "</p><h3>" + esc(t.title) + "</h3>" +
    String(t.body || "").split(/\n\s*(?:[-–—=_*~]\s*){3,}\n/).map((part, k) => {
      const paper = t.documentParts && t.documentParts[k] !== undefined ? t.documentParts[k] : t.document;
      return '<div class="reading' + (paper ? " document" : "") + '">' + para(part) + "</div>";
    }).join("");
  ...
  P.dictation = function (i, id, path) {
    ...
    const go = () => {
      ...
      const ok = normal(box.value) === normal(want);
