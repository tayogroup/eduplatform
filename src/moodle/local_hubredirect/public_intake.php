...
.pqpir-shell .pqh-workspace-top{background:var(--op-header-bg,#162b48)!important;border:1px solid var(--op-header-bg,#162b48)!important;box-shadow:0 2px 10px rgba(22,38,30,.10)!important;padding:28px 32px!important;border-radius:14px!important}
.pqpir-shell .pqh-workspace-title{color:var(--op-header-ink,#fff)!important;font-size:24px!important;font-weight:700!important;letter-spacing:0!important;text-shadow:none!important}
.pqpir-shell .pqh-workspace-sub{color:var(--op-header-ink-soft,rgba(255,255,255,.72))!important;font-size:14.5px!important;font-weight:400!important;opacity:1!important}.pqpir-shell .pqh-workspace-top .pqpir-navbrand{color:var(--op-header-ink,#fff)!important}.pqpir-shell .pqh-workspace-top .pqpir-navname{color:var(--op-header-ink,#fff)}.pqpir-shell .pqh-workspace-top .pqpir-navmark{background:rgba(255,255,255,.16);color:var(--op-header-ink,#fff)}
      ...
      <section class="pqpir-panel pqpir-wizard">
        <form method="post" novalidate>
          ...
          <?php if ($isprimaryeducation): ?>
            ...
            <?php // Asked on the way OUT of this step, and only when no child has been
                  // added. A parent who has skimmed past "Another child?" has no second
                  // chance later -- the rate limiter allows three submissions an hour on
                  // their contact details, so coming back for a forgotten child is what
                  // this whole feature exists to avoid. Rendered by PHP, hidden, so it
                  // cannot drift from the block it belongs to. ?>
            <div class="pqpir-sib-confirm" data-sib-confirm hidden aria-live="polite">
              <p class="pqpir-sib-confirm-q">Is this the only child you are enrolling today?</p>
              <p class="pqpir-muted">If you have another, add them now &mdash; one form covers them all.</p>
              <div class="pqpir-sib-confirm-btns">
                <button type="button" class="pqpir-btn pqpir-btn-ghost" data-sib-confirm-add>No &mdash; add another child</button>
                <button type="button" class="pqpir-btn" data-sib-confirm-ok>Yes, just this one</button>
              </div>
            </div>
            <?php endif; ?>
          <?php endif; ?>
...
(function () {
  ...
  // Confirm on the way out of this step that there really is only one child.
  //
  // The wizard binds its own click handler to the Next button, and that script runs
  // BEFORE this one, so a listener added to the same button would fire second and
  // could not stop it -- at the target, capture and bubble listeners run in
  // registration order. A capture-phase listener on the document runs before any
  // listener on the target, which is why it is bound here and not on the button.
  var panel = document.querySelector('[data-sib-confirm]');
  var page = list.closest ? list.closest('.pqpir-page') : null;
  var confirmed = false;
  function cardCount() { return list.querySelectorAll('[data-sib-card]').length; }

  if (panel && page) {
    document.addEventListener('click', function (e) {
      if (!e.target || !e.target.closest) { return; }
      if (!e.target.closest('[data-wizard-next]')) { return; }
      // Only while THIS step is the one on screen; aria-hidden is what the
      // wizard sets as it moves, so it is the honest test of where we are.
      if (page.getAttribute('aria-hidden') !== 'false') { return; }
      // Nothing to ask once a child has been added, or once already answered.
      if (confirmed || cardCount() > 0) { return; }
      e.preventDefault();
      e.stopPropagation();
    ...
    panel.querySelector('[data-sib-confirm-ok]').addEventListener('click', function () {
      confirmed = true;
      panel.setAttribute('hidden', '');
      document.querySelector('[data-wizard-next]').click();
    });
    panel.querySelector('[data-sib-confirm-add]').addEventListener('click', function () {
      panel.setAttribute('hidden', '');
      add.click();
    });
  }
