...
function pageNarration(data) {
  ...
  if (data.kind === 'quiz') {
    return [
      data.title,
      data.intro,
      ...data.questions.map((question) => question.q),
    ].filter(Boolean).join(' ');
  }
  const text = (data.text || []).join(' ');
  const speech = (data.bubbles || []).map((bubble) => `${bubble.speaker} says, ${bubble.text}`).join(' ');
  return [data.title, text, speech].filter(Boolean).join(' ');
}
...
async function main() {
  ...
  const htmlPath = path.resolve(root, arg('html', 'dist/pre_quraan/units/openmaic-classroom/juniors-big-day-ebook.html'));
  const outDir = path.resolve(root, arg('out-dir', 'dist/pre_quraan/units/openmaic-classroom/assets/juniors-big-day/audio'));
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    const text = pageNarration(pages[index]).replace(/\s+/g, ' ').trim();
